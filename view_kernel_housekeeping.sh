...
# Collects host kernel housekeeping thread placement, workqueue masks and local/IPI interrupt evidence.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_kernel_housekeeping.sh
#   ./view_kernel_housekeeping.sh --file collect_runtime_pressure.json
#   ./view_kernel_housekeeping.sh --help
#
# Output:
...
app_name="view_kernel_housekeeping"
app_version=0.9.2
app_rc=0
app_compact=0
...
usage() {
    cat <<'EOF'
view_kernel_housekeeping.sh - standalone kernel housekeeping / ipi noise view

Usage:
  sudo ./view_kernel_housekeeping.sh
  ./view_kernel_housekeeping.sh --file collect_kernel_housekeeping.json
  sudo ./view_kernel_housekeeping.sh --json > collect_kernel_housekeeping.json
...
collect_kernel_housekeeping() {
    ckh_dir=$_inv_temp_dir/kernel-housekeeping
    mkdir -p "$ckh_dir" || return 1
    ckh_threads="$ckh_dir/threads.jsonl"
    : > "$ckh_threads" || return 1
    for ckh_proc in /proc/[0-9]*; do
        [ -d "$ckh_proc" ] || continue
        ckh_pid=${ckh_proc##*/}
        ckh_comm=$(cat "$ckh_proc/comm" 2>/dev/null) || continue
        case "$ckh_comm" in
            ksoftirqd/*|rcu*|migration/*|watchdog/*|kworker/*|kswapd*|kcompactd*|writeback*|irq/*|cpuhp/*|idle_inject/*)
                ;;
            *) continue ;;
        esac
        ckh_cpus=$(awk -F: '$1=="Cpus_allowed_list"{gsub(/^[ \t]+/,"",$2);print $2}' "$ckh_proc/status" 2>/dev/null)
        ckh_vol=$(awk -F: '$1=="voluntary_ctxt_switches"{gsub(/[ \t]/,"",$2);print $2}' "$ckh_proc/status" 2>/dev/null)
        ckh_invol=$(awk -F: '$1=="nonvoluntary_ctxt_switches"{gsub(/[ \t]/,"",$2);print $2}' "$ckh_proc/status" 2>/dev/null)
        ckh_psr=$(ps -o psr= -p "$ckh_pid" 2>/dev/null | tr -d ' ')
        jq -n --arg pid "$ckh_pid" --arg comm "$ckh_comm" --arg cpus "$ckh_cpus" \
          --arg psr "$ckh_psr" --arg vol "$ckh_vol" --arg invol "$ckh_invol" \
          '{pid:($pid|tonumber),comm:$comm,cpus_allowed_list:$cpus,
            current_cpu:(try ($psr|tonumber) catch null),
            voluntary_context_switches:(try ($vol|tonumber) catch null),
            involuntary_context_switches:(try ($invol|tonumber) catch null)}' >> "$ckh_threads" || return 1
    done
    jq -s '.' "$ckh_threads" > "$ckh_dir/threads.json" || return 1

    awk '/^(NMI|LOC|SPU|PMI|IWI|RTR|RES|CAL|TLB|TRM|THR|DFR|MCE|MCP|HYP|ERR|MIS|PIN|NPI|PIW):/ {print}' \
      /proc/interrupts 2>/dev/null > "$ckh_dir/ipi.txt" || :
    jq -Rn '[inputs]' < "$ckh_dir/ipi.txt" > "$ckh_dir/ipi.json" || return 1

    ckh_wq="$ckh_dir/workqueues.jsonl"; : > "$ckh_wq" || return 1
    for ckh_wqd in /sys/bus/workqueue/devices/*; do
        [ -d "$ckh_wqd" ] || continue
        ckh_name=${ckh_wqd##*/}
        ckh_mask=$(cat "$ckh_wqd/cpumask" 2>/dev/null)
        ckh_req=$(cat "$ckh_wqd/cpumask_requested" 2>/dev/null)
        jq -n --arg name "$ckh_name" --arg mask "$ckh_mask" --arg req "$ckh_req" \
          '{name:$name,cpumask:($mask|select(length>0)),cpumask_requested:($req|select(length>0))}' \
          >> "$ckh_wq" || return 1
    done
    jq -s '.' "$ckh_wq" > "$ckh_dir/workqueues.json" || return 1
    ckh_soft=$(cat /proc/softirqs 2>/dev/null)

    jq -n --arg soft "$ckh_soft" --slurpfile threads "$ckh_dir/threads.json" \
      --slurpfile ipi "$ckh_dir/ipi.json" --slurpfile wq "$ckh_dir/workqueues.json" \
      '{kernel_threads:$threads[0],thread_count:($threads[0]|length),
        ipi_interrupt_lines:$ipi[0],workqueues:$wq[0],softirqs_raw:$soft}' > "$ckh_dir/data.json" || return 1
    inv_strings_json "$ckh_dir/notes.json" \
      "Kernel thread placement and current CPU are point-in-time observations." \
      "Local/IPI interrupt lines include non-device interrupt classes reported by /proc/interrupts." || return 1
    inv_strings_json "$ckh_dir/errors.json" || return 1
    inv_emit kernel-housekeeping "$ckh_dir/data.json" "$ckh_dir/notes.json" "$ckh_dir/errors.json"
}

print_kernel_housekeeping_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "Kernel Housekeeping / IPI Noise View"
    inv_view_kv "Housekeeping threads" "$(jq -r '.data.thread_count // 0' "$f")"
    inv_view_section "Thread classes"
    jq -r '[.data.kernel_threads[]?.comm |
      if startswith("ksoftirqd/") then "ksoftirqd"
      elif startswith("migration/") then "migration"
      elif startswith("watchdog/") then "watchdog"
      elif startswith("kworker/") then "kworker"
      elif startswith("rcu") then "rcu"
      elif startswith("kswapd") then "kswapd"
      else "other" end] | group_by(.)[]? | "\(.[0])=\(length)"' "$f" |
      while IFS= read -r line; do inv_view_item "$line"; done
    inv_view_section "Local / IPI interrupt lines"
    jq -r '.data.ipi_interrupt_lines[]?' "$f" | while IFS= read -r line; do inv_view_item "$line"; done
    inv_view_metadata "$f"
}

# ============================================================
# print_kernel_housekeeping_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_kernel_housekeeping_summary() { print_kernel_housekeeping_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_kernel_housekeeping_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_kernel_housekeeping.summary-source.json
        collect_kernel_housekeeping > "$main_summary_file" || return $?
        print_kernel_housekeeping_view "$main_summary_file"
        return $?
    fi
    collect_kernel_housekeeping
}
