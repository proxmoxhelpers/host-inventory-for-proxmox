...
# Passively samples hardware IRQ and softirq activity to correlate actual CPU-side interrupt work.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_irq_activity.sh
#   ./view_irq_activity.sh --file collect_irq_activity.json
#   ./view_irq_activity.sh --help
#
# Output:
...
usage() {
    cat <<'EOF'
view_irq_activity.sh - standalone IRQ / softirq activity view

Usage:
  sudo ./view_irq_activity.sh
  ./view_irq_activity.sh --file collect_irq_activity.json
  sudo ./view_irq_activity.sh --json > collect_irq_activity.json
...
collect_irq_activity() {
    cia_dir=$_inv_temp_dir/irq-activity
    mkdir -p "$cia_dir" || return 1
    cat /proc/interrupts 2>/dev/null > "$cia_dir/interrupts.0"
    cat /proc/softirqs 2>/dev/null > "$cia_dir/softirqs.0"
    cia_t0=$(date '+%s' 2>/dev/null)
    sleep 1
    cia_t1=$(date '+%s' 2>/dev/null)
    cat /proc/interrupts 2>/dev/null > "$cia_dir/interrupts.1"
    cat /proc/softirqs 2>/dev/null > "$cia_dir/softirqs.1"

    awk '
    NR==FNR {
      if (FNR==1) {n=NF; next}
      if ($1 ~ /^[0-9]+:$/) {irq=$1; sub(":","",irq); for(i=2;i<=n+1;i++) a[irq,i-2]=$i; next}
      next
    }
    FNR==1 {n2=NF; next}
    $1 ~ /^[0-9]+:$/ {
      irq=$1; sub(":","",irq); total=0; csv="";
      for(i=2;i<=n2+1;i++){d=$i-(a[irq,i-2]+0); if(d<0)d=0; total+=d; csv=csv (csv==""?"":",") d}
      src=""; for(i=n2+2;i<=NF;i++) src=src (src==""?"":" ") $i;
      print irq "\t" total "\t" csv "\t" src
    }' "$cia_dir/interrupts.0" "$cia_dir/interrupts.1" > "$cia_dir/irq.tsv"
    cia_stream=$cia_dir/irq.jsonl; : > "$cia_stream"
    while IFS="$(printf '\t')" read -r irq total csv src; do
        [ -n "$irq" ] || continue
        jq -n --arg irq "$irq" --arg total "$total" --arg csv "$csv" --arg source "$src" '{irq:($irq|tonumber),total_delta:($total|tonumber),per_cpu_delta:($csv|split(",")|map(tonumber)),source:$source}' >> "$cia_stream" || return 1
    done < "$cia_dir/irq.tsv"
    jq -s 'sort_by(-.total_delta)' "$cia_stream" > "$cia_dir/irq.json" || return 1

    awk '
    NR==FNR {if(FNR==1){n=NF;next} key=$1; sub(":","",key); for(i=2;i<=n+1;i++) a[key,i-2]=$i; next}
    FNR==1 {n2=NF; next}
    {key=$1; sub(":","",key); total=0; csv=""; for(i=2;i<=n2+1;i++){d=$i-(a[key,i-2]+0); if(d<0)d=0; total+=d; csv=csv (csv==""?"":",") d}; print key "\t" total "\t" csv}
    ' "$cia_dir/softirqs.0" "$cia_dir/softirqs.1" > "$cia_dir/softirq.tsv"
    cia_sstream=$cia_dir/softirq.jsonl; : > "$cia_sstream"
    while IFS="$(printf '\t')" read -r name total csv; do
        [ -n "$name" ] || continue
        jq -n --arg name "$name" --arg total "$total" --arg csv "$csv" '{name:$name,total_delta:($total|tonumber),per_cpu_delta:($csv|split(",")|map(tonumber))}' >> "$cia_sstream" || return 1
    done < "$cia_dir/softirq.tsv"
    jq -s 'sort_by(-.total_delta)' "$cia_sstream" > "$cia_dir/softirq.json" || return 1

    cia_i0=$(cat "$cia_dir/interrupts.0" 2>/dev/null); cia_i1=$(cat "$cia_dir/interrupts.1" 2>/dev/null)
    cia_s0=$(cat "$cia_dir/softirqs.0" 2>/dev/null); cia_s1=$(cat "$cia_dir/softirqs.1" 2>/dev/null)
    cia_duration=$((cia_t1-cia_t0)); [ "$cia_duration" -gt 0 ] || cia_duration=1
    jq -n --arg duration "$cia_duration" --slurpfile irqs "$cia_dir/irq.json" --slurpfile soft "$cia_dir/softirq.json" --arg i0 "$cia_i0" --arg i1 "$cia_i1" --arg s0 "$cia_s0" --arg s1 "$cia_s1" '{sample_seconds:($duration|tonumber),irqs:$irqs[0],softirqs:$soft[0],raw:{interrupts_t0:$i0,interrupts_t1:$i1,softirqs_t0:$s0,softirqs_t1:$s1}}' > "$cia_dir/data.json" || return 1
    inv_strings_json "$cia_dir/notes.json" "This is a passive point-in-time activity sample; no synthetic workload is generated." "Zero delta means no activity was observed during the sample, not that the IRQ can never fire." || return 1
    inv_strings_json "$cia_dir/errors.json" || return 1
    inv_emit irq-activity "$cia_dir/data.json" "$cia_dir/notes.json" "$cia_dir/errors.json"
}

print_irq_activity_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "IRQ / SoftIRQ Activity View"
    inv_view_kv "Passive sample" "$(jq -r '.data.sample_seconds|tostring' "$f") seconds"
    inv_view_section "Top hardware IRQ deltas"
    jq -r '.data.irqs[0:20][]?|[.irq,.total_delta,.source]|@tsv' "$f" | while IFS="$(printf '\t')" read -r irq total src; do inv_view_item "IRQ $irq delta=$total source=$src"; done
    inv_view_section "SoftIRQ deltas"
    jq -r '.data.softirqs[]?|[.name,.total_delta]|@tsv' "$f" | while IFS="$(printf '\t')" read -r n total; do inv_view_item "$n delta=$total"; done
    inv_view_section "Per-CPU total hardware IRQ delta"
    jq -r '[.data.irqs[].per_cpu_delta] | if length==0 then [] else transpose|map(add) end | to_entries[] | [.key,.value]|@tsv' "$f" | while IFS="$(printf '\t')" read -r cpu total; do inv_view_item "CPU$cpu delta=$total"; done
    inv_view_metadata "$f"
}

# ============================================================
# print_irq_activity_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_irq_activity_summary() { print_irq_activity_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_irq_activity_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_irq_activity.summary-source.json
        collect_irq_activity > "$main_summary_file" || return $?
        print_irq_activity_view "$main_summary_file"
        return $?
    fi
    collect_irq_activity
}
