...
# Collects x86 speculative-execution mitigation states, related CPU capabilities and boot-policy evidence without altering mitigations.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_security_mitigations.sh
#   ./view_security_mitigations.sh --file collect_runtime_pressure.json
#   ./view_security_mitigations.sh --help
#
# Output:
...
app_name="view_security_mitigations"
app_version=0.9.2
app_rc=0
app_compact=0
...
usage() {
    cat <<'EOF'
view_security_mitigations.sh - collect security-mitigation states and performance-relevant mitigation policy evidence

Usage:
  ./view_security_mitigations.sh [--compact|--summary|--view]
  ./view_security_mitigations.sh --help
  ./view_security_mitigations.sh --version
...
collect_security_mitigations() {
    csm_dir=$_inv_temp_dir/security-mitigations; mkdir -p "$csm_dir" || return 1
    csm_stream=$csm_dir/vuln.jsonl; : > "$csm_stream" || return 1
    for csm_file in /sys/devices/system/cpu/vulnerabilities/*; do
        [ -f "$csm_file" ] || continue
        csm_name=${csm_file##*/}; csm_value=$(cat "$csm_file" 2>/dev/null)
        jq -n --arg name "$csm_name" --arg value "$csm_value" '{name:$name,state:$value}' >> "$csm_stream" || return 1
    done
    jq -s 'sort_by(.name)' "$csm_stream" > "$csm_dir/vuln.json" || return 1
    csm_cmd=$(cat /proc/cmdline 2>/dev/null)
    csm_flags=$(awk -F: '/^flags[[:space:]]*:/{sub(/^[[:space:]]+/,"",$2);print $2;exit}' /proc/cpuinfo 2>/dev/null)
    csm_smt_active=$(cat /sys/devices/system/cpu/smt/active 2>/dev/null); csm_smt_control=$(cat /sys/devices/system/cpu/smt/control 2>/dev/null)
    csm_policy=$(printf '%s\n' "$csm_cmd" | tr ' ' '\n' | grep -E '^(mitigations|spectre_v2|spectre_v1|spec_store_bypass_disable|l1tf|mds|tsx_async_abort|retbleed|spec_rstack_overflow|gather_data_sampling|reg_file_data_sampling|nospectre_v1|nospectre_v2|nopti|pti)=' 2>/dev/null | paste -sd ' ' -)
    jq -n --arg cmd "$csm_cmd" --arg flags "$csm_flags" --arg policy "$csm_policy" --arg smta "$csm_smt_active" --arg smtc "$csm_smt_control" --slurpfile vuln "$csm_dir/vuln.json" '{vulnerabilities:$vuln[0],kernel_cmdline:$cmd,explicit_mitigation_arguments:(if $policy=="" then null else $policy end),smt:{active:(try ($smta|tonumber) catch null),control:(if $smtc=="" then null else $smtc end)},cpu_capabilities:{ibrs:($flags|test("(^| )ibrs( |$)")),ibpb:($flags|test("(^| )ibpb( |$)")),stibp:($flags|test("(^| )stibp( |$)")),ssbd:($flags|test("(^| )ssbd( |$)")),spec_ctrl:($flags|test("(^| )spec_ctrl( |$)"))}}' > "$csm_dir/data.json" || return 1
    inv_strings_json "$csm_dir/notes.json" "Mitigation state is security evidence; this collector makes no recommendation to weaken protections for performance." || return 1
    inv_strings_json "$csm_dir/errors.json" || return 1
    inv_emit security-mitigations "$csm_dir/data.json" "$csm_dir/notes.json" "$csm_dir/errors.json"
}
print_security_mitigations_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "Security Mitigations / Performance Dimension View"
    inv_view_kv "SMT active" "$(jq -r '.data.smt.active // "unknown"' "$f")"
    inv_view_kv "Explicit mitigation args" "$(jq -r '.data.explicit_mitigation_arguments // "none"' "$f")"
    inv_view_section "Kernel vulnerability states"
    jq -r '.data.vulnerabilities[]?|"\(.name): \(.state)"' "$f" | while IFS= read -r line; do inv_view_item "$line"; done
    inv_view_metadata "$f"
}

# ============================================================
# print_security_mitigations_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_security_mitigations_summary() { print_security_mitigations_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_security_mitigations_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_security_mitigations.summary-source.json
        collect_security_mitigations > "$main_summary_file" || return $?
        print_security_mitigations_view "$main_summary_file"
        return $?
    fi
    collect_security_mitigations
}
