...
# Collects passive thermal sensors, hwmon data, power caps and a short turbostat snapshot.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_thermal_power.sh
#   ./view_thermal_power.sh --file collect_thermal_power.json
#   ./view_thermal_power.sh --help
#
# Output:
...
app_name="view_thermal_power"
app_version=0.9.2
app_rc=0
app_compact=0
app_package_specs="jq:jq:required sensors:lm-sensors:optional turbostat:linux-cpupower:optional"
...
usage() {
    cat <<'EOF'
view_thermal_power.sh - standalone thermal power inventory view

Usage:
  sudo ./view_thermal_power.sh
  ./view_thermal_power.sh --file collect_thermal_power.json
  sudo ./view_thermal_power.sh --json > collect_thermal_power.json

Default behavior:
  With no arguments, collect the thermal power aspect live and
  render its colorized human-readable view.

View input:
  --file FILE       Render a previously saved collect_thermal_power.json envelope.
...
# Collects passive thermal/power telemetry without load generation.
#
# Version:
#   1.0.0
#
...
# ============================================================
collect_thermal_power() {
    ctp_dir=$_inv_temp_dir/thermal
    ctp_thermal_stream=$ctp_dir/thermal.jsonl
    ctp_hwmon_stream=$ctp_dir/hwmon.jsonl
    ctp_power_stream=$ctp_dir/powercap.jsonl
    mkdir -p "$ctp_dir" || return 1
    : > "$ctp_thermal_stream"
    : > "$ctp_hwmon_stream"
    : > "$ctp_power_stream"
    for ctp_zone in /sys/class/thermal/thermal_zone*; do
        [ -d "$ctp_zone" ] || continue
        jq -n --arg zone "${ctp_zone##*/}" --arg type "$(cat "$ctp_zone/type" 2>/dev/null)" --arg temp_mC "$(cat "$ctp_zone/temp" 2>/dev/null)" --arg mode "$(cat "$ctp_zone/mode" 2>/dev/null)" --arg policy "$(cat "$ctp_zone/policy" 2>/dev/null)" '{zone:$zone,type:$type,temp_mC:$temp_mC,mode:$mode,policy:$policy}' >> "$ctp_thermal_stream" || return 1
    done
    jq -s '.' "$ctp_thermal_stream" > "$ctp_dir/thermal.json" || return 1
    for ctp_hwmon in /sys/class/hwmon/hwmon*; do
        [ -d "$ctp_hwmon" ] || continue
        inv_flat_dir_json "$ctp_dir/hwmon-values.json" "$ctp_hwmon" '^(temp[0-9]+_(input|max|crit|label)|fan[0-9]+_input|power[0-9]+_(input|cap|max)|in[0-9]+_input)$' || return 1
        jq -n --arg hwmon "${ctp_hwmon##*/}" --arg name "$(cat "$ctp_hwmon/name" 2>/dev/null)" --slurpfile values "$ctp_dir/hwmon-values.json" '{hwmon:$hwmon,name:$name,values:$values[0]}' >> "$ctp_hwmon_stream" || return 1
    done
    jq -s '.' "$ctp_hwmon_stream" > "$ctp_dir/hwmon.json" || return 1
    for ctp_power in /sys/class/powercap/*; do
        [ -d "$ctp_power" ] || continue
        inv_flat_dir_json "$ctp_dir/power-values.json" "$ctp_power" '^(name|enabled|energy_uj|max_energy_range_uj|constraint_.*|power_uw|max_power_range_uw)$' || return 1
        jq -n --arg domain "${ctp_power##*/}" --slurpfile values "$ctp_dir/power-values.json" '{domain:$domain,values:$values[0]}' >> "$ctp_power_stream" || return 1
    done
    jq -s '.' "$ctp_power_stream" > "$ctp_dir/powercap.json" || return 1
    inv_command_json "$ctp_dir/sensors.json" 10 sensors -j || return 1
    inv_command_json "$ctp_dir/turbostat.json" 8 turbostat --quiet --show Busy%,Bzy_MHz,TSC_MHz,IRQ,SMI,PkgTmp,PkgWatt,CorWatt,GFXWatt,RAMWatt --interval 1 --num_iterations 1 || return 1
    jq -n --slurpfile thermal "$ctp_dir/thermal.json" --slurpfile hwmon "$ctp_dir/hwmon.json" --slurpfile power "$ctp_dir/powercap.json" --slurpfile sensors "$ctp_dir/sensors.json" --slurpfile turbostat "$ctp_dir/turbostat.json" '{thermal_zones:$thermal[0],hwmon:$hwmon[0],powercap:$power[0],raw:{sensors_json:$sensors[0],turbostat_snapshot:$turbostat[0]}}' > "$ctp_dir/data.json" || return 1
    inv_strings_json "$ctp_dir/notes.json" "The turbostat probe is a short observation only; no synthetic CPU load is generated." || return 1
    inv_strings_json "$ctp_dir/errors.json" || return 1
    inv_emit thermal-power "$ctp_dir/data.json" "$ctp_dir/notes.json" "$ctp_dir/errors.json"
}



# ============================================================
# print_thermal_power_view
# Presents a colorized thermal / power view.
#
# Version:
#   1.1.0
# ============================================================
print_thermal_power_view() {
    ptpv_file=$1
    inv_summary_validate_file "$ptpv_file" || return $?
    inv_view_header "$ptpv_file" "Thermal / Power View"

    inv_view_section "Overview"
    inv_view_kv "Thermal zones" "$(jq -r '.data.thermal_zones|length' "$ptpv_file")"
    inv_view_kv "hwmon devices" "$(jq -r '.data.hwmon|length' "$ptpv_file")"
    inv_view_kv "Powercap domains" "$(jq -r '.data.powercap|length' "$ptpv_file")"
    inv_view_kv "sensors" "rc=$(jq -r '.data.raw.sensors_json.rc // "unavailable"' "$ptpv_file")"
    inv_view_kv "turbostat" "rc=$(jq -r '.data.raw.turbostat_snapshot.rc // "unavailable"' "$ptpv_file")"

    inv_view_section "Temperature sensors"
    jq -r '.data.hwmon[]
        | .name as $dev
        | .values as $v
        | $v | to_entries[]
        | select(.key|test("^temp[0-9]+_input$"))
        | (.key|capture("^temp(?<n>[0-9]+)_input$").n) as $n
        | [$dev, ($v["temp"+$n+"_label"] // ("temp"+$n)), (try (((.value|tonumber)/1000)|tostring) catch .value)]
        | @tsv' "$ptpv_file" |
    while IFS="$(printf '\t')" read -r ptpv_dev ptpv_label ptpv_temp; do
        inv_view_item "$ptpv_dev / $ptpv_label = ${ptpv_temp} C"
    done

    inv_view_section "Fans"
    ptpv_fans=$(jq -r '.data.hwmon[]
        | .name as $dev
        | .values | to_entries[]
        | select(.key|test("^fan[0-9]+_input$"))
        | [$dev,.key,.value] | @tsv' "$ptpv_file")
    if [ -n "$ptpv_fans" ]; then
        printf '%s\n' "$ptpv_fans" |
        while IFS="$(printf '\t')" read -r ptpv_dev ptpv_name ptpv_rpm; do
            inv_view_item "$ptpv_dev / $ptpv_name = $ptpv_rpm RPM"
        done
    else
        inv_view_item "No fan RPM inputs reported."
    fi

    inv_view_section "Voltage inputs"
    ptpv_volts=$(jq -r '.data.hwmon[]
        | .name as $dev
        | .values | to_entries[]
        | select(.key|test("^in[0-9]+_input$"))
        | [$dev,.key,(try (((.value|tonumber)/1000)|tostring) catch .value)] | @tsv' "$ptpv_file")
    if [ -n "$ptpv_volts" ]; then
        printf '%s\n' "$ptpv_volts" |
        while IFS="$(printf '\t')" read -r ptpv_dev ptpv_name ptpv_volts_value; do
            inv_view_item "$ptpv_dev / $ptpv_name = $ptpv_volts_value V"
        done
    else
        inv_view_item "No voltage inputs reported."
    fi

    inv_view_section "Powercap / energy counters"
    jq -r '.data.powercap[] | [.domain,(if (.values.name//"")=="" then "unnamed" else .values.name end),(if (.values.enabled//"")=="" then "?" else .values.enabled end),(if (.values.energy_uj//"")=="" then "n/a" else .values.energy_uj end)] | @tsv' "$ptpv_file" |
    while IFS="$(printf '\t')" read -r ptpv_domain ptpv_name ptpv_enabled ptpv_energy; do
        inv_view_item "$ptpv_domain ($ptpv_name) enabled=$ptpv_enabled energy_uj=$ptpv_energy"
    done

    inv_view_section "turbostat snapshot"
    ptpv_turbo=$(jq -r '.data.raw.turbostat_snapshot.stdout // "" | split("\n") | .[0:2] | join(" | ")' "$ptpv_file")
    [ -n "$ptpv_turbo" ] && inv_view_item "$ptpv_turbo"

    inv_view_metadata "$ptpv_file"
}

# ============================================================
# print_thermal_power_summary
# Presents thermal zones, hwmon and host power telemetry.
#
# Version:
#   1.1.0
#
# Usage:
#   print_thermal_power_summary collectorEnvelope.json
#
# Returns:
#   0 on success
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_thermal_power_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_thermal_power.summary-source.json
        collect_thermal_power > "$main_summary_file" || return $?
        print_thermal_power_view "$main_summary_file"
        return $?
    fi
    collect_thermal_power
}
