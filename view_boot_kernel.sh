...
# ============================================================
# view_boot_kernel.sh
# Collects bootloader, kernel command line, kernel capabilities and clocksource.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_boot_kernel.sh
#   ./view_boot_kernel.sh --file collect_boot_kernel.json
#   ./view_boot_kernel.sh --help
#
# Output:
...
app_name="view_boot_kernel"
app_version=0.9.2
app_rc=0
app_compact=0
app_package_specs="jq:jq:required efibootmgr:efibootmgr:optional gzip:gzip:optional"
...
usage() {
    cat <<'EOF'
view_boot_kernel.sh - standalone boot kernel inventory view

Usage:
  sudo ./view_boot_kernel.sh
  ./view_boot_kernel.sh --file collect_boot_kernel.json
  sudo ./view_boot_kernel.sh --json > collect_boot_kernel.json

Default behavior:
  With no arguments, collect the boot kernel aspect live and
  render its colorized human-readable view.

View input:
  --file FILE       Render a previously saved collect_boot_kernel.json envelope.
...
# ============================================================
# collect_boot_kernel
# Collects cmdline, bootloader state, selected kernel config and clocksource.
#
# Version:
#   1.0.0
#
...
# ============================================================
collect_boot_kernel() {
    cbk_dir=$_inv_temp_dir/boot-kernel
    mkdir -p "$cbk_dir" || return 1
    cbk_release=$(uname -r)
    cbk_config=/boot/config-$cbk_release
    cbk_config_raw=
    if [ -r "$cbk_config" ]; then cbk_config_raw=$(cat "$cbk_config"); elif [ -r /proc/config.gz ] && command -v gzip >/dev/null 2>&1; then cbk_config_raw=$(gzip -cd /proc/config.gz 2>/dev/null); fi
    cbk_selected=$_inv_temp_dir/kernel-config-selected.$$.jsonl
    : > "$cbk_selected" || return 1
    for cbk_key in CONFIG_IOMMU_SUPPORT CONFIG_INTEL_IOMMU CONFIG_AMD_IOMMU CONFIG_IRQ_REMAP CONFIG_VFIO CONFIG_VFIO_PCI CONFIG_VFIO_IOMMU_TYPE1 CONFIG_NO_HZ_FULL CONFIG_RCU_NOCB_CPU CONFIG_HUGETLBFS CONFIG_TRANSPARENT_HUGEPAGE CONFIG_KSM CONFIG_NUMA CONFIG_NUMA_BALANCING CONFIG_PREEMPT CONFIG_PREEMPT_DYNAMIC CONFIG_PREEMPT_RT; do
        cbk_value=$(printf '%s\n' "$cbk_config_raw" | sed -n "s/^$cbk_key=//p" | head -n 1)
        if [ -z "$cbk_value" ] && printf '%s\n' "$cbk_config_raw" | grep -q "^# $cbk_key is not set$"; then cbk_value=n; fi
        if [ -n "$cbk_value" ]; then jq -n --arg key "$cbk_key" --arg value "$cbk_value" '{($key):$value}' >> "$cbk_selected"; else jq -n --arg key "$cbk_key" '{($key):null}' >> "$cbk_selected"; fi
    done
    jq -s 'reduce .[] as $item ({}; . * $item)' "$cbk_selected" > "$cbk_dir/kernel_config.json" || return 1
    inv_command_json "$cbk_dir/proxmox_boot.json" 12 proxmox-boot-tool status || return 1
    inv_command_json "$cbk_dir/efibootmgr.json" 10 efibootmgr -v || return 1
    cbk_cmdline=$(cat /proc/cmdline 2>/dev/null)
    cbk_grub=$(cat /etc/default/grub 2>/dev/null)
    cbk_pve_cmdline=$(cat /etc/kernel/cmdline 2>/dev/null)
    cbk_clock_current=$(cat /sys/devices/system/clocksource/clocksource0/current_clocksource 2>/dev/null)
    cbk_clock_available=$(cat /sys/devices/system/clocksource/clocksource0/available_clocksource 2>/dev/null)
    cbk_preempt=$(cat /sys/kernel/debug/sched/preempt 2>/dev/null)
    jq -n --arg cmdline "$cbk_cmdline" --arg grub "$cbk_grub" --arg pve_cmdline "$cbk_pve_cmdline" --arg clock_current "$cbk_clock_current" --arg clock_available "$cbk_clock_available" --arg preempt "$cbk_preempt" --arg config_source "$cbk_config" --slurpfile config "$cbk_dir/kernel_config.json" --slurpfile boot "$cbk_dir/proxmox_boot.json" --slurpfile efi "$cbk_dir/efibootmgr.json" '{cmdline_raw:$cmdline,kernel_config_source:$config_source,kernel_config_selected:$config[0],bootloader:{proxmox_boot_tool:$boot[0],efibootmgr:$efi[0],grub_default_raw:$grub,kernel_cmdline_file_raw:$pve_cmdline},runtime:{clocksource_current:$clock_current,clocksource_available:$clock_available,preempt:$preempt}}' > "$cbk_dir/data.json" || return 1
    inv_strings_json "$cbk_dir/notes.json" "Kernel configuration is filtered to capabilities relevant to virtualization and latency work." || return 1
    inv_strings_json "$cbk_dir/errors.json" || return 1
    inv_emit boot-kernel "$cbk_dir/data.json" "$cbk_dir/notes.json" "$cbk_dir/errors.json"
}



# ============================================================
# print_boot_kernel_view
# Presents a colorized boot/kernel view.
#
# Version:
#   1.1.0
# ============================================================
print_boot_kernel_view() {
    pbv_file=$1
    inv_summary_validate_file "$pbv_file" || return $?
    inv_view_header "$pbv_file" "Boot / Kernel View"

    inv_view_section "Runtime overview"
    inv_view_kv "Kernel command line" "$(jq -r '.data.cmdline_raw // "unknown"' "$pbv_file")"
    inv_view_kv "Kernel config source" "$(jq -r '.data.kernel_config_source // "unknown"' "$pbv_file")"
    inv_view_kv "Clocksource" "$(jq -r '.data.runtime.clocksource_current // "unknown"' "$pbv_file")"
    inv_view_kv "Available clocks" "$(jq -r '.data.runtime.clocksource_available // "unknown"' "$pbv_file")"
    inv_view_kv "Preemption" "$(jq -r '.data.runtime.preempt // "unknown"' "$pbv_file")"

    inv_view_section "Kernel capabilities"
    jq -r '.data.kernel_config_selected | to_entries[] | [.key,(.value // "unknown")] | @tsv' "$pbv_file" |
    while IFS="$(printf '\t')" read -r pbv_key pbv_value; do
        inv_view_item "$pbv_key = $pbv_value"
    done

    inv_view_section "Boot tooling"
    pbv_pbt_rc=$(jq -r '.data.bootloader.proxmox_boot_tool.rc // "unavailable"' "$pbv_file")
    pbv_pbt_detail=$(jq -r '(.data.bootloader.proxmox_boot_tool.stderr // .data.bootloader.proxmox_boot_tool.stdout // "") | split("\n") | map(select(length>0)) | first // ""' "$pbv_file")
    inv_view_kv "proxmox-boot-tool" "rc=$pbv_pbt_rc${pbv_pbt_detail:+ - $pbv_pbt_detail}"

    pbv_efi_rc=$(jq -r '.data.bootloader.efibootmgr.rc // "unavailable"' "$pbv_file")
    inv_view_kv "efibootmgr" "rc=$pbv_efi_rc"
    pbv_boot_current=$(jq -r '(.data.bootloader.efibootmgr.stdout // "") | split("\n") | map(select(startswith("BootCurrent:"))) | first // ""' "$pbv_file")
    pbv_boot_order=$(jq -r '(.data.bootloader.efibootmgr.stdout // "") | split("\n") | map(select(startswith("BootOrder:"))) | first // ""' "$pbv_file")
    [ -n "$pbv_boot_current" ] && inv_view_item "$pbv_boot_current"
    [ -n "$pbv_boot_order" ] && inv_view_item "$pbv_boot_order"

    inv_view_metadata "$pbv_file"
}

# ============================================================
# print_boot_kernel_summary
# Presents boot and kernel capability inventory.
#
# Version:
#   1.1.0
#
# Usage:
#   print_boot_kernel_summary collectorEnvelope.json
#
# Returns:
#   0 on success
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_boot_kernel_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_boot_kernel.summary-source.json
        collect_boot_kernel > "$main_summary_file" || return $?
        print_boot_kernel_view "$main_summary_file"
        return $?
    fi
    collect_boot_kernel
}
