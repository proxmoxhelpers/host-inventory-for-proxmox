...
# Collects clocksources, clockevents, watchdog/NMI settings and timing-platform evidence.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./collect_timers_watchdogs.sh [--compact]
#   ./collect_timers_watchdogs.sh --help
#   ./collect_timers_watchdogs.sh --version
#
# Output:
...
app_name="collect_timers_watchdogs"
app_version=0.9.2
app_rc=0
app_compact=0
...
usage() {
    cat <<'EOF'
collect_timers_watchdogs.sh - collect clocksource, clockevent, watchdog and timing-platform evidence

Usage:
  ./collect_timers_watchdogs.sh [--compact|--summary|--view]
  ./collect_timers_watchdogs.sh --help
  ./collect_timers_watchdogs.sh --version
...
collect_timers_watchdogs() {
    ctw_dir=$_inv_temp_dir/timers-watchdogs
    mkdir -p "$ctw_dir" || return 1
    inv_file_map_json "$ctw_dir/clocksource.json" "current=/sys/devices/system/clocksource/clocksource0/current_clocksource" "available=/sys/devices/system/clocksource/clocksource0/available_clocksource" || return 1
    ctw_stream=$ctw_dir/clockevents.jsonl; : > "$ctw_stream"
    for ctw_ev in /sys/devices/system/clockevents/clockevent*; do
        [ -d "$ctw_ev" ] || continue
        ctw_name=${ctw_ev##*/}; ctw_dev=$(cat "$ctw_ev/current_device" 2>/dev/null)
        jq -n --arg name "$ctw_name" --arg device "$ctw_dev" '{name:$name,current_device:(if $device=="" then null else $device end)}' >> "$ctw_stream" || return 1
    done
    jq -s '.' "$ctw_stream" > "$ctw_dir/clockevents.json" || return 1
    inv_file_map_json "$ctw_dir/watchdog.json" "nmi_watchdog=/proc/sys/kernel/nmi_watchdog" "watchdog=/proc/sys/kernel/watchdog" "watchdog_thresh=/proc/sys/kernel/watchdog_thresh" "watchdog_cpumask=/proc/sys/kernel/watchdog_cpumask" "hardlockup_panic=/proc/sys/kernel/hardlockup_panic" "softlockup_panic=/proc/sys/kernel/softlockup_panic" || return 1
    ctw_hpet=false; [ -e /dev/hpet ] && ctw_hpet=true
    ctw_rtc=$ctw_dir/rtc.jsonl; : > "$ctw_rtc"
    for ctw_r in /sys/class/rtc/rtc*; do [ -d "$ctw_r" ] || continue; ctw_name=${ctw_r##*/}; ctw_rname=$(cat "$ctw_r/name" 2>/dev/null); jq -n --arg rtc "$ctw_name" --arg name "$ctw_rname" '{rtc:$rtc,name:(if $name=="" then null else $name end)}' >> "$ctw_rtc"; done
    jq -s '.' "$ctw_rtc" > "$ctw_dir/rtc-array.json" || return 1
    inv_command_json "$ctw_dir/dmesg.json" 8 sh -c 'dmesg 2>/dev/null | grep -Ei "clocksource|clockevent|TSC|HPET|APIC timer|x2apic|watchdog|NMI watchdog" || true' || return 1
    jq -n --slurpfile cs "$ctw_dir/clocksource.json" --slurpfile ce "$ctw_dir/clockevents.json" --slurpfile wd "$ctw_dir/watchdog.json" --slurpfile rtc "$ctw_dir/rtc-array.json" --slurpfile dmesg "$ctw_dir/dmesg.json" --argjson hpet "$ctw_hpet" '{clocksource:$cs[0],clockevents:$ce[0],watchdog:$wd[0],hpet_device_present:$hpet,rtc:$rtc[0],dmesg_relevant:$dmesg[0]}' > "$ctw_dir/data.json" || return 1
    inv_strings_json "$ctw_dir/notes.json" "Watchdog state is observed because NMI/perf watchdog activity can add periodic host work; no watchdog setting is changed." || return 1
    inv_strings_json "$ctw_dir/errors.json" || return 1
    inv_emit timers-watchdogs "$ctw_dir/data.json" "$ctw_dir/notes.json" "$ctw_dir/errors.json"
}

print_timers_watchdogs_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "Timers / Watchdogs View"
    inv_view_section "Clocksource"
    inv_view_kv "Current" "$(jq -r '.data.clocksource.current // "unknown"' "$f")"
    inv_view_kv "Available" "$(jq -r '.data.clocksource.available // "unknown"' "$f")"
    inv_view_section "Clockevents"
    jq -r '.data.clockevents[]|[.name,(.current_device//"unknown")]|@tsv' "$f" | while IFS="$(printf '\t')" read -r n d; do inv_view_item "$n -> $d"; done
    inv_view_section "Watchdogs"
    jq -r '.data.watchdog|to_entries[]|[.key,.value]|@tsv' "$f" | while IFS="$(printf '\t')" read -r k v; do inv_view_item "$k=$v"; done
    inv_view_kv "HPET device" "$(jq -r 'if .data.hpet_device_present then "present" else "not observed" end' "$f")"
    inv_view_metadata "$f"
}

# ============================================================
# print_timers_watchdogs_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_timers_watchdogs_summary() { print_timers_watchdogs_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_timers_watchdogs_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_timers_watchdogs.summary-source.json
        collect_timers_watchdogs > "$main_summary_file" || return $?
        print_timers_watchdogs_view "$main_summary_file"
        return $?
    fi
    collect_timers_watchdogs
}
