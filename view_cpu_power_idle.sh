...
# Collects cpufreq, governors, EPP/P-state controls and CPU idle states.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_cpu_power_idle.sh
#   ./view_cpu_power_idle.sh --file collect_cpu_power_idle.json
#   ./view_cpu_power_idle.sh --help
#
# Output:
...
app_name="view_cpu_power_idle"
app_version=0.9.2
app_rc=0
app_compact=0
app_package_specs="jq:jq:required cpupower:linux-cpupower:optional"
...
usage() {
    cat <<'EOF'
view_cpu_power_idle.sh - standalone cpu power idle inventory view

Usage:
  sudo ./view_cpu_power_idle.sh
  ./view_cpu_power_idle.sh --file collect_cpu_power_idle.json
  sudo ./view_cpu_power_idle.sh --json > collect_cpu_power_idle.json

Default behavior:
  With no arguments, collect the cpu power idle aspect live and
  render its colorized human-readable view.

View input:
  --file FILE       Render a previously saved collect_cpu_power_idle.json envelope.
...
# ============================================================
# collect_cpu_power_idle
# Collects cpufreq policies, CPU idle states and vendor P-state controls.
#
# Version:
#   1.0.0
#
...
# ============================================================
collect_cpu_power_idle() {
    cpi_dir=$_inv_temp_dir/cpu-power-idle
    cpi_policy_stream=$cpi_dir/policies.jsonl
    cpi_idle_stream=$cpi_dir/idle.jsonl
    mkdir -p "$cpi_dir" || return 1
    : > "$cpi_policy_stream"
    : > "$cpi_idle_stream"
    for cpi_path in /sys/devices/system/cpu/cpufreq/policy*; do
        [ -d "$cpi_path" ] || continue
        inv_flat_dir_json "$cpi_dir/policy.json" "$cpi_path" '^(affected_cpus|related_cpus|scaling_.*|cpuinfo_.*|energy_performance_.*|bios_limit|base_frequency)$' || return 1
        jq -n --arg policy "${cpi_path##*/}" --slurpfile values "$cpi_dir/policy.json" '{policy:$policy,values:$values[0]}' >> "$cpi_policy_stream" || return 1
    done
    jq -s '.' "$cpi_policy_stream" > "$cpi_dir/policies.json" || return 1
    for cpi_cpu in /sys/devices/system/cpu/cpu[0-9]*; do
        [ -d "$cpi_cpu/cpuidle" ] || continue
        cpi_state_stream=$cpi_dir/states.${cpi_cpu##*cpu}.jsonl
        : > "$cpi_state_stream"
        for cpi_state in "$cpi_cpu"/cpuidle/state*; do
            [ -d "$cpi_state" ] || continue
            jq -n --arg state "${cpi_state##*/}" --arg name "$(cat "$cpi_state/name" 2>/dev/null)" --arg desc "$(cat "$cpi_state/desc" 2>/dev/null)" --arg latency_us "$(cat "$cpi_state/latency" 2>/dev/null)" --arg residency_us "$(cat "$cpi_state/residency" 2>/dev/null)" --arg disabled "$(cat "$cpi_state/disable" 2>/dev/null)" --arg usage "$(cat "$cpi_state/usage" 2>/dev/null)" --arg time "$(cat "$cpi_state/time" 2>/dev/null)" '{state:$state,name:$name,desc:$desc,latency_us:$latency_us,residency_us:$residency_us,disabled:$disabled,usage:$usage,time:$time}' >> "$cpi_state_stream" || return 1
        done
        jq -s '.' "$cpi_state_stream" > "$cpi_dir/states.json" || return 1
        jq -n --arg cpu "${cpi_cpu##*cpu}" --slurpfile states "$cpi_dir/states.json" '{cpu:($cpu|tonumber),states:$states[0]}' >> "$cpi_idle_stream" || return 1
    done
    jq -s '.' "$cpi_idle_stream" > "$cpi_dir/idle.json" || return 1
    inv_flat_dir_json "$cpi_dir/intel_pstate.json" /sys/devices/system/cpu/intel_pstate || return 1
    inv_flat_dir_json "$cpi_dir/amd_pstate.json" /sys/devices/system/cpu/amd_pstate || return 1
    inv_command_json "$cpi_dir/cpupower_frequency.json" 10 cpupower frequency-info || return 1
    inv_command_json "$cpi_dir/cpupower_idle.json" 10 cpupower idle-info || return 1
    jq -n --arg boost "$(cat /sys/devices/system/cpu/cpufreq/boost 2>/dev/null)" --arg idle_driver "$(cat /sys/devices/system/cpu/cpuidle/current_driver 2>/dev/null)" --arg idle_governor "$(cat /sys/devices/system/cpu/cpuidle/current_governor_ro 2>/dev/null)" --arg idle_governors "$(cat /sys/devices/system/cpu/cpuidle/available_governors 2>/dev/null)" --slurpfile policies "$cpi_dir/policies.json" --slurpfile idle "$cpi_dir/idle.json" --slurpfile intel "$cpi_dir/intel_pstate.json" --slurpfile amd "$cpi_dir/amd_pstate.json" --slurpfile cpf "$cpi_dir/cpupower_frequency.json" --slurpfile cpi "$cpi_dir/cpupower_idle.json" '{cpufreq:{boost:$boost,policies:$policies[0],intel_pstate:$intel[0],amd_pstate:$amd[0]},cpuidle:{driver:$idle_driver,governor:$idle_governor,available_governors:$idle_governors,per_cpu:$idle[0]},raw:{cpupower_frequency_info:$cpf[0],cpupower_idle_info:$cpi[0]}}' > "$cpi_dir/data.json" || return 1
    inv_strings_json "$cpi_dir/notes.json" || return 1
    inv_strings_json "$cpi_dir/errors.json" || return 1
    inv_emit cpu-power-idle "$cpi_dir/data.json" "$cpi_dir/notes.json" "$cpi_dir/errors.json"
}



# ============================================================
# print_cpu_power_idle_view
# Presents a colorized CPU power/idle view.
#
# Version:
#   1.1.0
# ============================================================
print_cpu_power_idle_view() {
    ...
    inv_view_header "$ppiv_file" "CPU Power / Idle View"

    inv_view_section "Power overview"
    inv_view_kv "Boost" "$(jq -r '.data.cpufreq.boost // "unknown"' "$ppiv_file")"
    inv_view_kv "Scaling driver(s)" "$(jq -r '[.data.cpufreq.policies[].values.scaling_driver // empty]|unique|join(", ") // "unknown"' "$ppiv_file")"
    inv_view_kv "Governor(s)" "$(jq -r '[.data.cpufreq.policies[].values.scaling_governor // empty]|unique|join(", ") // "unknown"' "$ppiv_file")"
    inv_view_kv "Available governors" "$(jq -r '[.data.cpufreq.policies[].values.scaling_available_governors // empty]|unique|join(" | ")|gsub("\\s+$";"") // "unknown"' "$ppiv_file")"
    inv_view_kv "Idle driver" "$(jq -r '.data.cpuidle.driver // "unknown"' "$ppiv_file")"
    inv_view_kv "Idle governor" "$(jq -r '.data.cpuidle.governor // "unknown"' "$ppiv_file")"

    inv_view_section "Frequency policies"
    jq -r '.data.cpufreq.policies
        | sort_by((.policy|sub("^policy";"")|tonumber))[]
        | [.policy,
           (.values.affected_cpus//"?"),
           (try (((.values.scaling_min_freq|tonumber)/1000)|floor|tostring) catch "?"),
           (try (((.values.scaling_max_freq|tonumber)/1000)|floor|tostring) catch "?"),
           (try (((.values.scaling_cur_freq|tonumber)/1000)|floor|tostring) catch "?"),
           (.values.scaling_governor//"?")]
        | @tsv' "$ppiv_file" |
    while IFS="$(printf '\t')" read -r ppiv_pol ppiv_cpus ppiv_min ppiv_max ppiv_cur ppiv_gov; do
        inv_view_item "$ppiv_pol CPUs=$ppiv_cpus min=${ppiv_min}MHz max=${ppiv_max}MHz current=${ppiv_cur}MHz governor=$ppiv_gov"
    done

    inv_view_section "CPU0 idle states"
    ppiv_rows=$(jq -r '(first(.data.cpuidle.per_cpu[] | select(.cpu==0)) // {}) | .states[]? | [.name,.latency_us,.residency_us,.disabled] | @tsv' "$ppiv_file") || return 1
    if [ -n "$ppiv_rows" ]; then
        printf '%s\n' "$ppiv_rows" |
        while IFS="$(printf '\t')" read -r ppiv_name ppiv_lat ppiv_res ppiv_dis; do
            inv_view_item "$ppiv_name latency=${ppiv_lat}us target=${ppiv_res}us disabled=$ppiv_dis"
        done
    else
        inv_view_item "No CPU0 cpuidle states were reported."
    fi
...
# ============================================================
# print_cpu_power_idle_summary
# Presents CPU frequency and idle-state configuration.
#
# Version:
#   1.1.0
#
# Usage:
#   print_cpu_power_idle_summary collectorEnvelope.json
#
# Returns:
#   0 on success
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_cpu_power_idle_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_cpu_power_idle.summary-source.json
        collect_cpu_power_idle > "$main_summary_file" || return $?
        print_cpu_power_idle_view "$main_summary_file"
        return $?
    fi
    collect_cpu_power_idle
}
