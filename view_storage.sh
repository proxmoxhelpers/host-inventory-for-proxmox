...
# ============================================================
# view_storage.sh
# Collects block queues, NVMe controllers, LVM/ZFS, filesystems and Proxmox storage.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_storage.sh
#   ./view_storage.sh --file collect_storage.json
#   ./view_storage.sh --help
#
# Output:
...
app_name="view_storage"
app_version=0.9.2
app_rc=0
app_compact=0
app_package_specs="jq:jq:required lsblk:util-linux:optional findmnt:util-linux:optional blkid:util-linux:optional nvme:nvme-cli:optional lvs:lvm2:optional vgs:lvm2:optional pvs:lvm2:optional zpool:zfsutils-linux:optional zfs:zfsutils-linux:optional iostat:sysstat:optional"
...
usage() {
    cat <<'EOF'
view_storage.sh - standalone storage inventory view

Usage:
  sudo ./view_storage.sh
  ./view_storage.sh --file collect_storage.json
  sudo ./view_storage.sh --json > collect_storage.json

Default behavior:
  With no arguments, collect the storage aspect live and
  render its colorized human-readable view.

View input:
  --file FILE       Render a previously saved collect_storage.json envelope.
...
# ============================================================
# collect_storage
# Collects block queue, device ancestry, NVMe, LVM/ZFS and Proxmox storage state.
#
# Version:
#   1.1.0
#
...
# ============================================================
collect_storage() {
    cst_dir=$_inv_temp_dir/storage
    cst_queue_stream=$cst_dir/queues.jsonl
    cst_nvme_stream=$cst_dir/nvme.jsonl
    mkdir -p "$cst_dir" || return 1
    : > "$cst_queue_stream"
    : > "$cst_nvme_stream"
    for cst_block in /sys/block/*; do
        [ -d "$cst_block/queue" ] || continue
        inv_file_map_json "$cst_dir/queue-values.json" "scheduler=$cst_block/queue/scheduler" "nr_requests=$cst_block/queue/nr_requests" "read_ahead_kb=$cst_block/queue/read_ahead_kb" "rotational=$cst_block/queue/rotational" "logical_block_size=$cst_block/queue/logical_block_size" "physical_block_size=$cst_block/queue/physical_block_size" "minimum_io_size=$cst_block/queue/minimum_io_size" "optimal_io_size=$cst_block/queue/optimal_io_size" "discard_granularity=$cst_block/queue/discard_granularity" "discard_max_bytes=$cst_block/queue/discard_max_bytes" "write_cache=$cst_block/queue/write_cache" "rq_affinity=$cst_block/queue/rq_affinity" "nomerges=$cst_block/queue/nomerges" "io_poll=$cst_block/queue/io_poll" "io_poll_delay=$cst_block/queue/io_poll_delay" "wbt_lat_usec=$cst_block/queue/wbt_lat_usec" || return 1
        cst_devpath=$(inv_readlink "$cst_block/device" 2>/dev/null)
        jq -n --arg block "${cst_block##*/}" --arg device_path "$cst_devpath" --slurpfile values "$cst_dir/queue-values.json" '{block:$block,device_path:(if $device_path=="" then null else $device_path end),queue:$values[0]}' >> "$cst_queue_stream" || return 1
    done
    jq -s '.' "$cst_queue_stream" > "$cst_dir/queues.json" || return 1
    for cst_ctrl in /sys/class/nvme/nvme[0-9]*; do
        [ -d "$cst_ctrl" ] || continue
        cst_devpath=$(inv_readlink "$cst_ctrl/device" 2>/dev/null)
        jq -n --arg controller "${cst_ctrl##*/}" --arg model "$(cat "$cst_ctrl/model" 2>/dev/null)" --arg serial "$(cat "$cst_ctrl/serial" 2>/dev/null)" --arg firmware_rev "$(cat "$cst_ctrl/firmware_rev" 2>/dev/null)" --arg state "$(cat "$cst_ctrl/state" 2>/dev/null)" --arg transport "$(cat "$cst_ctrl/transport" 2>/dev/null)" --arg subsysnqn "$(cat "$cst_ctrl/subsysnqn" 2>/dev/null)" --arg device_path "$cst_devpath" '{controller:$controller,model:$model,serial:$serial,firmware_rev:$firmware_rev,state:$state,transport:$transport,subsysnqn:$subsysnqn,device_path:$device_path}' >> "$cst_nvme_stream" || return 1
    done
    jq -s '.' "$cst_nvme_stream" > "$cst_dir/nvme.json" || return 1
    inv_command_json "$cst_dir/lsblk.json" 12 lsblk -J -O -b || return 1
    inv_command_json "$cst_dir/findmnt.json" 12 findmnt -J -R || return 1
    inv_command_json "$cst_dir/blkid.json" 10 blkid || return 1
    inv_command_json "$cst_dir/nvme_list.json" 12 nvme list -o json || return 1
    inv_command_json "$cst_dir/nvme_subsys.json" 12 nvme list-subsys -o json || return 1
    inv_command_json "$cst_dir/lvs.json" 15 lvs --reportformat json -a -o +devices,segtype,lv_size,data_percent,metadata_percent,cache_mode || return 1
    inv_command_json "$cst_dir/vgs.json" 12 vgs --reportformat json -o +vg_size,vg_free || return 1
    inv_command_json "$cst_dir/pvs.json" 12 pvs --reportformat json -o +pv_size,pv_free || return 1
    inv_command_json "$cst_dir/zpool_status.json" 20 zpool status -v || return 1
    inv_command_json "$cst_dir/zpool_get.json" 20 zpool get all || return 1
    inv_command_json "$cst_dir/zfs_get.json" 25 zfs get -r all || return 1
    inv_command_json "$cst_dir/pvesm.json" 20 pvesm status --format 1 || return 1
    inv_command_json "$cst_dir/iostat.json" 6 iostat -x -d 1 2 || return 1
    jq -n --arg storage_cfg "$(cat /etc/pve/storage.cfg 2>/dev/null)" --arg mdstat "$(cat /proc/mdstat 2>/dev/null)" --slurpfile queues "$cst_dir/queues.json" --slurpfile nvme "$cst_dir/nvme.json" --slurpfile lsblk "$cst_dir/lsblk.json" --slurpfile findmnt "$cst_dir/findmnt.json" --slurpfile blkid "$cst_dir/blkid.json" --slurpfile nvl "$cst_dir/nvme_list.json" --slurpfile nvs "$cst_dir/nvme_subsys.json" --slurpfile lvs "$cst_dir/lvs.json" --slurpfile vgs "$cst_dir/vgs.json" --slurpfile pvs "$cst_dir/pvs.json" --slurpfile zps "$cst_dir/zpool_status.json" --slurpfile zpg "$cst_dir/zpool_get.json" --slurpfile zfg "$cst_dir/zfs_get.json" --slurpfile pvesm "$cst_dir/pvesm.json" --slurpfile iostat "$cst_dir/iostat.json" '{block_queues:$queues[0],nvme_sysfs:$nvme[0],proxmox_storage:{storage_cfg_raw:$storage_cfg,pvesm_status:$pvesm[0]},raw:{lsblk:$lsblk[0],findmnt:$findmnt[0],blkid:$blkid[0],nvme_list:$nvl[0],nvme_list_subsys:$nvs[0],lvs:$lvs[0],vgs:$vgs[0],pvs:$pvs[0],zpool_status:$zps[0],zpool_get:$zpg[0],zfs_get:$zfg[0],mdstat:$mdstat,iostat_devices:$iostat[0]}}' > "$cst_dir/data.json" || return 1
    inv_strings_json "$cst_dir/notes.json" "No benchmark or write workload is executed by this collector." "Block device_path values are resolved sysfs ancestry evidence and are used by the harmonizer to correlate non-NVMe devices with PCI controllers when a BDF is present in the path." || return 1
    inv_strings_json "$cst_dir/errors.json" || return 1
    inv_emit storage "$cst_dir/data.json" "$cst_dir/notes.json" "$cst_dir/errors.json"
}


# ============================================================
# print_storage_view
# Presents a colorized storage view.
#
# Version:
#   1.1.0
# ============================================================
print_storage_view() {
    psv_file=$1
    inv_summary_validate_file "$psv_file" || return $?
    inv_view_header "$psv_file" "Storage View"

    inv_view_section "Storage overview"
    inv_view_kv "Block queue objects" "$(jq -r '.data.block_queues|length' "$psv_file")"
    inv_view_kv "Physical block devices" "$(jq -r '[.data.block_queues[] | select((.block|startswith("dm-")|not) and (.block|startswith("loop")|not))] | length' "$psv_file")"
    inv_view_kv "Device-mapper devices" "$(jq -r '[.data.block_queues[] | select(.block|startswith("dm-"))] | length' "$psv_file")"
    inv_view_kv "Loop devices" "$(jq -r '[.data.block_queues[] | select(.block|startswith("loop"))] | length' "$psv_file")"
    inv_view_kv "NVMe controllers" "$(jq -r '.data.nvme_sysfs|length' "$psv_file")"
    inv_view_kv "iostat" "rc=$(jq -r '.data.raw.iostat_devices.rc // "unavailable"' "$psv_file")"
    inv_view_kv "PVE storage" "rc=$(jq -r '.data.proxmox_storage.pvesm_status.rc // "unavailable"' "$psv_file")"

    inv_view_section "Physical block devices"
    jq -r '.data.block_queues
        | map(select((.block|startswith("dm-")|not) and (.block|startswith("loop")|not)))
        | sort_by(.block)[]
        | [.block,(.queue.scheduler//"?"),(.queue.nr_requests//"?"),(.queue.read_ahead_kb//"?"),(.queue.write_cache//"?"),(.queue.rq_affinity//"?"),(.queue.rotational//"?"),(.device_path//"n/a")]
        | @tsv' "$psv_file" |
    while IFS="$(printf '\t')" read -r psv_block psv_sched psv_nr psv_ra psv_wc psv_rq psv_rot psv_path; do
        inv_view_item "$psv_block scheduler=$psv_sched nr_requests=$psv_nr read_ahead=${psv_ra}KB write_cache=$psv_wc rq_affinity=$psv_rq rotational=$psv_rot"
        [ "$psv_path" = "n/a" ] || inv_view_item "$psv_block sysfs-device=$psv_path"
    done

    inv_view_section "Device-mapper queue profiles"
    jq -r '.data.block_queues
        | map(select(.block|startswith("dm-")))
        | map({block:.block, read_ahead:(.queue.read_ahead_kb//"?"), write_cache:(.queue.write_cache//"?"), scheduler:(.queue.scheduler//"n/a")})
        | group_by([.read_ahead,.write_cache,.scheduler])
        | sort_by(.[0].read_ahead)[]
        | [(length|tostring), .[0].read_ahead, .[0].write_cache, .[0].scheduler]
        | @tsv' "$psv_file" |
    while IFS="$(printf '\t')" read -r psv_count psv_ra psv_wc psv_sched; do
        inv_view_item "$psv_count device(s): read_ahead=${psv_ra}KB write_cache=$psv_wc scheduler=$psv_sched"
    done

    inv_view_section "Loop-device queue profile"
    jq -r '.data.block_queues
        | map(select(.block|startswith("loop")))
        | if length==0 then empty else
          [(length|tostring),(.[0].queue.scheduler//"?"),(.[0].queue.nr_requests//"?"),(.[0].queue.read_ahead_kb//"?"),(.[0].queue.write_cache//"?")]
          | @tsv end' "$psv_file" |
    while IFS="$(printf '\t')" read -r psv_count psv_sched psv_nr psv_ra psv_wc; do
        inv_view_item "$psv_count loop device(s): scheduler=$psv_sched nr_requests=$psv_nr read_ahead=${psv_ra}KB write_cache=$psv_wc"
    done

    inv_view_section "NVMe controllers"
    jq -r '.data.nvme_sysfs[] | [.controller,.model,.firmware_rev,.state,.transport] | @tsv' "$psv_file" |
    while IFS="$(printf '\t')" read -r psv_ctrl psv_model psv_fw psv_state psv_transport; do
        inv_view_item "$psv_ctrl $psv_model firmware=$psv_fw state=$psv_state transport=$psv_transport"
    done

    inv_view_section "Proxmox storage status"
    psv_pve=$(jq -r '.data.proxmox_storage.pvesm_status.stdout // ""' "$psv_file")
    if [ -n "$psv_pve" ]; then
        printf '%s\n' "$psv_pve" | while IFS= read -r psv_line; do
            [ -n "$psv_line" ] && printf '    %s\n' "$psv_line"
        done
    else
        inv_view_item "PVE storage status unavailable."
    fi

    inv_view_metadata "$psv_file"
}

# ============================================================
# print_storage_summary
# Presents block queues, NVMe and host storage-stack inventory.
#
# Version:
#   1.1.0
#
# Usage:
#   print_storage_summary collectorEnvelope.json
#
# Returns:
#   0 on success
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_storage_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_storage.summary-source.json
        collect_storage > "$main_summary_file" || return $?
        print_storage_view "$main_summary_file"
        return $?
    fi
    collect_storage
}
