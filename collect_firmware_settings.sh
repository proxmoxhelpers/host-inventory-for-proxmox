...
# Collects firmware-attributes and fwupd-reported BIOS setting evidence when firmware exposes it.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./collect_firmware_settings.sh [--compact]
#   ./collect_firmware_settings.sh --help
#   ./collect_firmware_settings.sh --version
#
# Output:
...
app_name="collect_firmware_settings"
app_version=0.9.2
app_rc=0
app_compact=0
...
usage() {
    cat <<'EOF'
collect_firmware_settings.sh - collect firmware-reported BIOS settings and firmware attributes

Usage:
  ./collect_firmware_settings.sh [--compact|--summary|--view]
  ./collect_firmware_settings.sh --help
  ./collect_firmware_settings.sh --version
...
collect_firmware_settings() {
    cfs_dir=$_inv_temp_dir/firmware-settings
    mkdir -p "$cfs_dir" || return 1
    cfs_stream=$cfs_dir/attrs.jsonl
    : > "$cfs_stream" || return 1
    if [ -d /sys/class/firmware-attributes ]; then
        find -L /sys/class/firmware-attributes -maxdepth 8 -type f -readable 2>/dev/null |
        while IFS= read -r cfs_file; do
            cfs_value=$(cat "$cfs_file" 2>/dev/null) || continue
            cfs_rel=${cfs_file#/sys/class/firmware-attributes/}
            jq -n --arg path "$cfs_rel" --arg value "$cfs_value" '{path:$path,value:$value}' >> "$cfs_stream"
        done
    fi
    jq -s '.' "$cfs_stream" > "$cfs_dir/attrs.json" || return 1
    inv_command_json "$cfs_dir/fwupd-bios.json" 20 fwupdmgr get-bios-settings --json || return 1
    inv_command_json "$cfs_dir/fwupd-devices.json" 20 fwupdmgr get-devices --json || return 1
    inv_command_json "$cfs_dir/dmesg.json" 8 sh -c 'dmesg 2>/dev/null | grep -Ei "BIOS|UEFI|firmware|SVM|IOMMU|SME|SEV|Above 4G|Resizable BAR|ReBAR|ACPI" || true' || return 1
    cfs_efivars=0; [ -d /sys/firmware/efi/efivars ] && cfs_efivars=$(find /sys/firmware/efi/efivars -maxdepth 1 -type f 2>/dev/null | wc -l | tr -d ' ')
    cfs_bios_vendor=$(cat /sys/class/dmi/id/bios_vendor 2>/dev/null)
    cfs_bios_version=$(cat /sys/class/dmi/id/bios_version 2>/dev/null)
    cfs_bios_date=$(cat /sys/class/dmi/id/bios_date 2>/dev/null)
    jq -n --slurpfile attrs "$cfs_dir/attrs.json" --slurpfile fw "$cfs_dir/fwupd-bios.json" --slurpfile devices "$cfs_dir/fwupd-devices.json" --slurpfile dmesg "$cfs_dir/dmesg.json" --arg efivars "$cfs_efivars" --arg vendor "$cfs_bios_vendor" --arg version "$cfs_bios_version" --arg date "$cfs_bios_date" '{bios:{vendor:$vendor,version:$version,date:$date},firmware_attributes:$attrs[0],fwupd_bios_settings:$fw[0],fwupd_devices:$devices[0],efi_variable_count:($efivars|tonumber? // $efivars),dmesg_relevant:$dmesg[0]}' > "$cfs_dir/data.json" || return 1
    inv_strings_json "$cfs_dir/notes.json" "Literal BIOS setting values are reported only when firmware exposes them through Linux firmware attributes or fwupd." "Functional OS evidence must not be relabeled as a literal firmware toggle state." || return 1
    inv_strings_json "$cfs_dir/errors.json" || return 1
    inv_emit firmware-settings "$cfs_dir/data.json" "$cfs_dir/notes.json" "$cfs_dir/errors.json"
}

print_firmware_settings_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "Firmware / BIOS Settings View"
    inv_view_section "Firmware identity"
    inv_view_kv "BIOS" "$(jq -r '[.data.bios.vendor,.data.bios.version,.data.bios.date]|map(select(.!=""))|join(" / ")' "$f")"
    inv_view_kv "EFI variables" "$(jq -r '.data.efi_variable_count|tostring' "$f")"
    inv_view_section "Firmware attributes"
    inv_view_kv "Exposed attribute files" "$(jq -r '.data.firmware_attributes|length' "$f")"
    jq -r '.data.firmware_attributes[0:30][]?|[.path,.value]|@tsv' "$f" | while IFS="$(printf '\t')" read -r p v; do inv_view_item "$p = $v"; done
    inv_view_section "fwupd BIOS settings"
    inv_view_kv "Probe rc" "$(jq -r '.data.fwupd_bios_settings.rc // "unavailable"' "$f")"
    inv_view_kv "Captured chars" "$(jq -r '(.data.fwupd_bios_settings.stdout // "")|length' "$f")"
    inv_view_metadata "$f"
}

# ============================================================
# print_firmware_settings_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_firmware_settings_summary() { print_firmware_settings_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_firmware_settings_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_firmware_settings.summary-source.json
        collect_firmware_settings > "$main_summary_file" || return $?
        print_firmware_settings_view "$main_summary_file"
        return $?
    fi
    collect_firmware_settings
}
