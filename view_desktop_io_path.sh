...
# Collects host display, USB/HID/audio, network-path policy and active VM device-model evidence relevant to interactive desktop I/O.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_desktop_io_path.sh
#   ./view_desktop_io_path.sh --file collect_runtime_pressure.json
#   ./view_desktop_io_path.sh --help
#
# Output:
...
app_name="view_desktop_io_path"
app_version=0.9.2
app_rc=0
app_compact=0
app_package_specs="jq:jq:required ip:iproute2:optional tc:iproute2:optional"
...
usage() {
    cat <<'EOF'
view_desktop_io_path.sh - standalone desktop i/o path / interactive device view

Usage:
  sudo ./view_desktop_io_path.sh
  ./view_desktop_io_path.sh --file collect_desktop_io_path.json
  sudo ./view_desktop_io_path.sh --json > collect_desktop_io_path.json
...
collect_desktop_io_path() {
    cdi_dir=$_inv_temp_dir/desktop-io-path
    mkdir -p "$cdi_dir" || return 1

    cdi_drm="$cdi_dir/drm.jsonl"; : > "$cdi_drm" || return 1
    for cdi_node in /sys/class/drm/card* /sys/class/drm/card*-*; do
        [ -e "$cdi_node" ] || continue
        cdi_name=${cdi_node##*/}
        cdi_status=$(cat "$cdi_node/status" 2>/dev/null)
        cdi_enabled=$(cat "$cdi_node/enabled" 2>/dev/null)
        cdi_dpms=$(cat "$cdi_node/dpms" 2>/dev/null)
        cdi_path=$(readlink -f "$cdi_node/device" 2>/dev/null)
        cdi_bdf=$(printf '%s\n' "$cdi_path" | grep -Eo '[0-9a-fA-F]{4}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}\.[0-7]' | tail -1)
        jq -n --arg name "$cdi_name" --arg status "$cdi_status" --arg enabled "$cdi_enabled" \
          --arg dpms "$cdi_dpms" --arg bdf "$cdi_bdf" \
          '{name:$name,status:($status|select(length>0)),enabled:($enabled|select(length>0)),
            dpms:($dpms|select(length>0)),pci_bdf:($bdf|select(length>0))}' >> "$cdi_drm" || return 1
    done
    jq -s '.' "$cdi_drm" > "$cdi_dir/drm.json" || return 1

    cdi_vt="$cdi_dir/vt.jsonl"; : > "$cdi_vt" || return 1
    for cdi_vtd in /sys/class/vtconsole/vtcon*; do
        [ -d "$cdi_vtd" ] || continue
        cdi_vtn=${cdi_vtd##*/}
        cdi_bind=$(cat "$cdi_vtd/bind" 2>/dev/null)
        cdi_vname=$(cat "$cdi_vtd/name" 2>/dev/null)
        jq -n --arg vt "$cdi_vtn" --arg bind "$cdi_bind" --arg name "$cdi_vname" \
          '{vt:$vt,bound:($bind=="1"),name:$name}' >> "$cdi_vt" || return 1
    done
    jq -s '.' "$cdi_vt" > "$cdi_dir/vt.json" || return 1

    cdi_fb=$(cat /proc/fb 2>/dev/null)

    cdi_usb="$cdi_dir/usb.jsonl"; : > "$cdi_usb" || return 1
    for cdi_ud in /sys/bus/usb/devices/*; do
        [ -d "$cdi_ud" ] || continue
        [ -r "$cdi_ud/idVendor" ] || continue
        cdi_uname=${cdi_ud##*/}
        cdi_vid=$(cat "$cdi_ud/idVendor" 2>/dev/null)
        cdi_pid=$(cat "$cdi_ud/idProduct" 2>/dev/null)
        cdi_product=$(cat "$cdi_ud/product" 2>/dev/null)
        cdi_speed=$(cat "$cdi_ud/speed" 2>/dev/null)
        cdi_power_control=$(cat "$cdi_ud/power/control" 2>/dev/null)
        cdi_auto=$(cat "$cdi_ud/power/autosuspend" 2>/dev/null)
        [ -n "$cdi_auto" ] || cdi_auto=$(cat "$cdi_ud/power/autosuspend_delay_ms" 2>/dev/null)
        cdi_wakeup=$(cat "$cdi_ud/power/wakeup" 2>/dev/null)
        cdi_usb2_lpm=$(cat "$cdi_ud/power/usb2_hardware_lpm" 2>/dev/null)
        cdi_u1=$(cat "$cdi_ud/power/usb3_hardware_lpm_u1" 2>/dev/null)
        cdi_u2=$(cat "$cdi_ud/power/usb3_hardware_lpm_u2" 2>/dev/null)
        cdi_path=$(readlink -f "$cdi_ud" 2>/dev/null)
        cdi_bdf=$(printf '%s\n' "$cdi_path" | grep -Eo '[0-9a-fA-F]{4}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}\.[0-7]' | tail -1)

        cdi_eps="$cdi_dir/eps.$$.jsonl"; : > "$cdi_eps"
        find "$cdi_ud" -maxdepth 4 -type d -name 'ep_*' 2>/dev/null | while IFS= read -r cdi_ep; do
            cdi_epn=${cdi_ep##*/}
            cdi_addr=$(cat "$cdi_ep/bEndpointAddress" 2>/dev/null)
            cdi_attr=$(cat "$cdi_ep/bmAttributes" 2>/dev/null)
            cdi_int=$(cat "$cdi_ep/bInterval" 2>/dev/null)
            cdi_interval=$(cat "$cdi_ep/interval" 2>/dev/null)
            cdi_mps=$(cat "$cdi_ep/wMaxPacketSize" 2>/dev/null)
            cdi_eff_us=
            case "$cdi_int:$cdi_speed" in
                [1-9]*:*)
                    if awk "BEGIN{exit !($cdi_speed >= 480)}" 2>/dev/null; then
                        cdi_eff_us=$(awk -v b="$cdi_int" 'BEGIN{x=125; for(i=1;i<b;i++) x*=2; print x}')
                    else
                        cdi_eff_us=$(awk -v b="$cdi_int" 'BEGIN{print b*1000}')
                    fi ;;
            esac
            jq -n --arg endpoint "$cdi_epn" --arg address "$cdi_addr" --arg attributes "$cdi_attr" \
              --arg binterval "$cdi_int" --arg interval "$cdi_interval" --arg max_packet "$cdi_mps" --arg effective_us "$cdi_eff_us" \
              '{endpoint:$endpoint,address:($address|select(length>0)),
                attributes:($attributes|select(length>0)),bInterval:($binterval|select(length>0)),
                interval:($interval|select(length>0)),effective_interval_us:(try ($effective_us|tonumber) catch null),max_packet_size:($max_packet|select(length>0))}'
        done > "$cdi_eps"
        jq -s '.' "$cdi_eps" > "$cdi_dir/eps.$$.json" || return 1
        jq -n --arg device "$cdi_uname" --arg vendor "$cdi_vid" --arg product_id "$cdi_pid" \
          --arg product "$cdi_product" --arg speed "$cdi_speed" --arg control "$cdi_power_control" \
          --arg autosuspend "$cdi_auto" --arg wakeup "$cdi_wakeup" --arg usb2 "$cdi_usb2_lpm" \
          --arg u1 "$cdi_u1" --arg u2 "$cdi_u2" --arg bdf "$cdi_bdf" \
          --slurpfile eps "$cdi_dir/eps.$$.json" \
          '{device:$device,vendor_id:$vendor,product_id:$product_id,product:($product|select(length>0)),
            speed_mbps:(try ($speed|tonumber) catch null),controller_bdf:($bdf|select(length>0)),
            power:{control:($control|select(length>0)),autosuspend:($autosuspend|select(length>0)),
              wakeup:($wakeup|select(length>0)),usb2_hardware_lpm:($usb2|select(length>0)),
              usb3_hardware_lpm_u1:($u1|select(length>0)),usb3_hardware_lpm_u2:($u2|select(length>0))},
            endpoints:$eps[0]}' >> "$cdi_usb" || return 1
    done
    jq -s '.' "$cdi_usb" > "$cdi_dir/usb.json" || return 1

    cdi_hda="$cdi_dir/hda.jsonl"; : > "$cdi_hda" || return 1
    if [ -d /sys/module/snd_hda_intel/parameters ]; then
        for cdi_hp in /sys/module/snd_hda_intel/parameters/*; do
            [ -f "$cdi_hp" ] || continue
            cdi_hn=${cdi_hp##*/}
            cdi_hv=$(cat "$cdi_hp" 2>/dev/null)
            jq -n --arg name "$cdi_hn" --arg value "$cdi_hv" '{name:$name,value:$value}' >> "$cdi_hda" || return 1
        done
    fi
    jq -s '.' "$cdi_hda" > "$cdi_dir/hda.json" || return 1
    cdi_asound=$(cat /proc/asound/cards 2>/dev/null)

    cdi_busy_read=$(cat /proc/sys/net/core/busy_read 2>/dev/null)
    cdi_busy_poll=$(cat /proc/sys/net/core/busy_poll 2>/dev/null)
    cdi_rfs=$(cat /proc/sys/net/core/rps_sock_flow_entries 2>/dev/null)
    cdi_bridge="$cdi_dir/bridge.jsonl"; : > "$cdi_bridge" || return 1
    for cdi_bp in /proc/sys/net/bridge/*; do
        [ -f "$cdi_bp" ] || continue
        cdi_bn=${cdi_bp##*/}; cdi_bv=$(cat "$cdi_bp" 2>/dev/null)
        jq -n --arg key "$cdi_bn" --arg value "$cdi_bv" '{key:$key,value:$value}' >> "$cdi_bridge" || return 1
    done
    jq -s 'map({(.key):.value})|add//{}' "$cdi_bridge" > "$cdi_dir/bridge.json" || return 1

    cdi_napi="$cdi_dir/napi.jsonl"; : > "$cdi_napi" || return 1
    for cdi_ifd in /sys/class/net/*; do
        [ -d "$cdi_ifd" ] || continue
        cdi_if=${cdi_ifd##*/}
        cdi_threaded=$(cat "$cdi_ifd/threaded" 2>/dev/null)
        cdi_gro=$(cat "$cdi_ifd/gro_flush_timeout" 2>/dev/null)
        cdi_defer=$(cat "$cdi_ifd/napi_defer_hard_irqs" 2>/dev/null)
        jq -n --arg interface "$cdi_if" --arg threaded "$cdi_threaded" --arg gro "$cdi_gro" \
          --arg defer "$cdi_defer" \
          '{interface:$interface,threaded:($threaded|select(length>0)),
            gro_flush_timeout:(try ($gro|tonumber) catch null),
            napi_defer_hard_irqs:(try ($defer|tonumber) catch null)}' >> "$cdi_napi" || return 1
    done
    jq -s '.' "$cdi_napi" > "$cdi_dir/napi.json" || return 1

    inv_command_json "$cdi_dir/ip_link.json" 10 ip -j -d link show || return 1
    inv_command_json "$cdi_dir/tc_qdisc.json" 10 tc -j qdisc show || return 1
    inv_command_json "$cdi_dir/tc_filter_ingress.json" 10 tc -j filter show dev vmbr0 ingress || return 1
    inv_command_json "$cdi_dir/nft.json" 10 nft -j list ruleset || return 1

    cdi_vm="$cdi_dir/vms.jsonl"; : > "$cdi_vm" || return 1
    if command -v qm >/dev/null 2>&1; then
        qm list 2>/dev/null | awk 'NR>1 && $1 ~ /^[0-9]+$/ {print $1}' | while IFS= read -r cdi_vmid; do
            cdi_cfg=$(qm config "$cdi_vmid" 2>/dev/null)
            cdi_status=$(qm status "$cdi_vmid" 2>/dev/null | awk '{print $2}')
            jq -n --arg vmid "$cdi_vmid" --arg status "$cdi_status" --arg config "$cdi_cfg" \
              '{vmid:$vmid,status:$status,config_raw:$config}' 
        done > "$cdi_vm"
    fi
    jq -s '.' "$cdi_vm" > "$cdi_dir/vms.json" || return 1

    jq -n --arg fb "$cdi_fb" --arg asound "$cdi_asound" \
      --arg busy_read "$cdi_busy_read" --arg busy_poll "$cdi_busy_poll" --arg rfs "$cdi_rfs" \
      --slurpfile drm "$cdi_dir/drm.json" --slurpfile vt "$cdi_dir/vt.json" \
      --slurpfile usb "$cdi_dir/usb.json" --slurpfile hda "$cdi_dir/hda.json" \
      --slurpfile bridge "$cdi_dir/bridge.json" --slurpfile napi "$cdi_dir/napi.json" \
      --slurpfile iplink "$cdi_dir/ip_link.json" --slurpfile qdisc "$cdi_dir/tc_qdisc.json" \
      --slurpfile tcfilter "$cdi_dir/tc_filter_ingress.json" --slurpfile nft "$cdi_dir/nft.json" \
      --slurpfile vms "$cdi_dir/vms.json" '
      {display:{drm:$drm[0],vtconsoles:$vt[0],framebuffers_raw:$fb},
       usb:$usb[0],audio:{snd_hda_intel_parameters:$hda[0],asound_cards_raw:$asound},
       network_policy:{busy_read:(try ($busy_read|tonumber) catch null),
         busy_poll:(try ($busy_poll|tonumber) catch null),
         rps_sock_flow_entries:(try ($rfs|tonumber) catch null),
         bridge_netfilter:$bridge[0],napi:$napi[0],
         ip_link:$iplink[0],tc_qdisc:$qdisc[0],tc_filter_ingress:$tcfilter[0],nft_ruleset:$nft[0]},
       qemu_configs:$vms[0]}' > "$cdi_dir/data.json" || return 1
    inv_strings_json "$cdi_dir/notes.json" \
      "USB endpoint and power evidence is read from host sysfs; guest-side HID/audio latency is not inferred." \
      "Network policy records XDP/tc/nft/NAPI state only when the host exposes the relevant interfaces/tools." || return 1
    inv_strings_json "$cdi_dir/errors.json" || return 1
    inv_emit desktop-io-path "$cdi_dir/data.json" "$cdi_dir/notes.json" "$cdi_dir/errors.json"
}

print_desktop_io_path_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "Desktop I/O Path / Interactive Device View"
    inv_view_section "Display / host console"
    inv_view_kv "DRM records" "$(jq -r '.data.display.drm|length' "$f")"
    inv_view_kv "Bound VT consoles" "$(jq -r '[.data.display.vtconsoles[]?|select(.bound==true)]|length' "$f")"
    inv_view_section "USB / HID path"
    inv_view_kv "USB devices" "$(jq -r '.data.usb|length' "$f")"
    inv_view_kv "Periodic endpoints" "$(jq -r '[.data.usb[]?.endpoints[]?|select(.bInterval!=null)]|length' "$f")"
    inv_view_section "Audio"
    inv_view_kv "snd_hda parameters" "$(jq -r '.data.audio.snd_hda_intel_parameters|length' "$f")"
    inv_view_section "Interactive network policy"
    inv_view_kv "busy_read" "$(jq -r '.data.network_policy.busy_read // "unknown"' "$f")"
    inv_view_kv "busy_poll" "$(jq -r '.data.network_policy.busy_poll // "unknown"' "$f")"
    inv_view_kv "RFS table entries" "$(jq -r '.data.network_policy.rps_sock_flow_entries // "unknown"' "$f")"
    inv_view_kv "VM configs" "$(jq -r '.data.qemu_configs|length' "$f")"
    inv_view_metadata "$f"
}

# ============================================================
# print_desktop_io_path_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_desktop_io_path_summary() { print_desktop_io_path_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_desktop_io_path_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_desktop_io_path.summary-source.json
        collect_desktop_io_path > "$main_summary_file" || return $?
        print_desktop_io_path_view "$main_summary_file"
        return $?
    fi
    collect_desktop_io_path
}
