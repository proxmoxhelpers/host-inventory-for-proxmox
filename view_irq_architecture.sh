...
# Collects Linux IRQ-domain, hardware-vector, irqchip and special-interrupt architecture evidence in addition to affinity.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_irq_architecture.sh
#   ./view_irq_architecture.sh --file collect_runtime_pressure.json
#   ./view_irq_architecture.sh --help
#
# Output:
...
app_name="view_irq_architecture"
app_version=0.9.2
app_rc=0
app_compact=0
...
usage() {
    cat <<'EOF'
view_irq_architecture.sh - collect IRQ hardware identifiers, irqchip/domain evidence and special interrupt/IPI topology

Usage:
  ./view_irq_architecture.sh [--compact|--summary|--view]
  ./view_irq_architecture.sh --help
  ./view_irq_architecture.sh --version
...
collect_irq_architecture() {
    cia_dir=$_inv_temp_dir/irq-architecture; mkdir -p "$cia_dir" || return 1
    cia_stream=$cia_dir/irqs.jsonl; : > "$cia_stream" || return 1
    for cia_path in /sys/kernel/irq/[0-9]*; do
        [ -d "$cia_path" ] || continue
        cia_irq=${cia_path##*/}
        cia_hwirq=$(cat "$cia_path/hwirq" 2>/dev/null)
        cia_chip=$(cat "$cia_path/chip_name" 2>/dev/null)
        cia_name=$(cat "$cia_path/name" 2>/dev/null)
        cia_actions=$(cat "$cia_path/actions" 2>/dev/null)
        cia_type=$(cat "$cia_path/type" 2>/dev/null)
        cia_count=$(cat "$cia_path/per_cpu_count" 2>/dev/null)
        cia_node=$(cat "/proc/irq/$cia_irq/node" 2>/dev/null)
        cia_hint=$(cat "/proc/irq/$cia_irq/affinity_hint" 2>/dev/null)
        cia_smp=$(cat "/proc/irq/$cia_irq/smp_affinity_list" 2>/dev/null)
        cia_eff=$(cat "/proc/irq/$cia_irq/effective_affinity_list" 2>/dev/null)
        cia_desc=$(grep -E "^[[:space:]]*$cia_irq:" /proc/interrupts 2>/dev/null | sed -E 's/^[^:]+:[[:space:]]*//' )
        cia_ir=$(printf '%s' "$cia_desc" | grep -q 'IR-PCI-MSI' && printf true || printf false)
        cia_msi=$(printf '%s' "$cia_desc" | grep -Eq 'PCI-MSI|PCI-MSIX|MSI' && printf true || printf false)
        jq -n --arg irq "$cia_irq" --arg hwirq "$cia_hwirq" --arg chip "$cia_chip" --arg name "$cia_name" --arg actions "$cia_actions" --arg typ "$cia_type" --arg pc "$cia_count" --arg node "$cia_node" --arg hint "$cia_hint" --arg smp "$cia_smp" --arg eff "$cia_eff" --arg desc "$cia_desc" --argjson ir "$cia_ir" --argjson msi "$cia_msi" '{irq:($irq|tonumber),hwirq:(if $hwirq=="" then null else $hwirq end),chip_name:(if $chip=="" then null else $chip end),name:(if $name=="" then null else $name end),actions:(if $actions=="" then null else $actions end),type:(if $typ=="" then null else $typ end),per_cpu_count_raw:(if $pc=="" then null else $pc end),numa_node:(try ($node|tonumber) catch null),affinity_hint:(if $hint=="" then null else $hint end),smp_affinity_list:(if $smp=="" then null else $smp end),effective_affinity_list:(if $eff=="" then null else $eff end),interrupt_descriptor:(if $desc=="" then null else $desc end),interrupt_remapped_msi:$ir,msi_like:$msi,managed_status:"unknown"}' >> "$cia_stream" || return 1
    done
    jq -s 'sort_by(.irq)' "$cia_stream" > "$cia_dir/irqs.json" || return 1
    cia_special=$(grep -E '^[[:space:]]*(NMI|LOC|SPU|PMI|IWI|RTR|RES|CAL|TLB|TRM|THR|DFR|MCE|MCP|HYP|ERR|MIS|PIN|NPI):' /proc/interrupts 2>/dev/null)
    jq -n --arg special "$cia_special" --slurpfile irqs "$cia_dir/irqs.json" '{irqs:$irqs[0],special_interrupts_raw:$special,managed_irq_detection:"unknown-unless-explicitly-exposed"}' > "$cia_dir/data.json" || return 1
    inv_strings_json "$cia_dir/notes.json" "Managed-IRQ status stays unknown unless an explicit kernel interface exposes it; affinity shape is not treated as proof." "IR-PCI-MSI descriptor evidence indicates interrupt-remapped PCI MSI where present." || return 1
    inv_strings_json "$cia_dir/errors.json" || return 1
    inv_emit irq-architecture "$cia_dir/data.json" "$cia_dir/notes.json" "$cia_dir/errors.json"
}
print_irq_architecture_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "IRQ Architecture View"
    inv_view_kv "IRQ records" "$(jq -r '.data.irqs|length' "$f")"
    inv_view_kv "Interrupt-remapped MSI" "$(jq -r '[.data.irqs[]|select(.interrupt_remapped_msi==true)]|length' "$f")"
    inv_view_section "IRQ hardware mapping"
    jq -r '.data.irqs[]?|select(.hwirq!=null or .chip_name!=null)|"IRQ \(.irq) hwirq=\(.hwirq // "n/a") chip=\(.chip_name // "n/a") effective=\(.effective_affinity_list // "n/a") actions=\(.actions // "n/a")"' "$f" | head -40 | while IFS= read -r line; do inv_view_item "$line"; done
    inv_view_section "Special interrupts / IPIs"
    jq -r '.data.special_interrupts_raw // "not exposed"' "$f" | while IFS= read -r line; do [ -n "$line" ] && inv_view_item "$line"; done
    inv_view_metadata "$f"
}

# ============================================================
# print_irq_architecture_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_irq_architecture_summary() { print_irq_architecture_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_irq_architecture_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_irq_architecture.summary-source.json
        collect_irq_architecture > "$main_summary_file" || return $?
        print_irq_architecture_view "$main_summary_file"
        return $?
    fi
    collect_irq_architecture
}
