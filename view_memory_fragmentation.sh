...
# Collects buddy allocator, pageblock, compaction, THP and HugeTLB feasibility evidence without allocating or compacting memory.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_memory_fragmentation.sh
#   ./view_memory_fragmentation.sh --file collect_runtime_pressure.json
#   ./view_memory_fragmentation.sh --help
#
# Output:
...
app_name="view_memory_fragmentation"
app_version=0.9.2
app_rc=0
app_compact=0
...
usage() {
    cat <<'EOF'
view_memory_fragmentation.sh - collect memory fragmentation, compaction and hugepage-feasibility evidence

Usage:
  ./view_memory_fragmentation.sh [--compact|--summary|--view]
  ./view_memory_fragmentation.sh --help
  ./view_memory_fragmentation.sh --version
...
collect_memory_fragmentation() {
    cmf_dir=$_inv_temp_dir/memory-fragmentation; mkdir -p "$cmf_dir" || return 1
    cmf_buddy=$(cat /proc/buddyinfo 2>/dev/null)
    cmf_page=$(cat /proc/pagetypeinfo 2>/dev/null)
    cmf_zone=$(cat /proc/zoneinfo 2>/dev/null)
    cmf_vm=$(grep -E '^(compact_|thp_|pgscan_|pgsteal_|kswapd_|allocstall|nr_free_pages|nr_anon_transparent_hugepages)' /proc/vmstat 2>/dev/null)
    cmf_hpmd=$(cat /sys/kernel/mm/transparent_hugepage/hpage_pmd_size 2>/dev/null)
    cmf_thp_enabled=$(cat /sys/kernel/mm/transparent_hugepage/enabled 2>/dev/null)
    cmf_thp_defrag=$(cat /sys/kernel/mm/transparent_hugepage/defrag 2>/dev/null)
    cmf_compaction=$(cat /proc/sys/vm/compaction_proactiveness 2>/dev/null)
    cmf_extfrag=$(cat /proc/sys/vm/extfrag_threshold 2>/dev/null)
    cmf_minfree=$(cat /proc/sys/vm/min_free_kbytes 2>/dev/null)
    cmf_wscale=$(cat /proc/sys/vm/watermark_scale_factor 2>/dev/null)
    cmf_wboost=$(cat /proc/sys/vm/watermark_boost_factor 2>/dev/null)
    cmf_zreclaim=$(cat /proc/sys/vm/zone_reclaim_mode 2>/dev/null)
    cmf_huge_stream=$cmf_dir/huge.jsonl; : > "$cmf_huge_stream" || return 1
    for cmf_hp in /sys/kernel/mm/hugepages/hugepages-*; do
        [ -d "$cmf_hp" ] || continue
        cmf_name=${cmf_hp##*/hugepages-}; cmf_total=$(cat "$cmf_hp/nr_hugepages" 2>/dev/null); cmf_free=$(cat "$cmf_hp/free_hugepages" 2>/dev/null); cmf_resv=$(cat "$cmf_hp/resv_hugepages" 2>/dev/null); cmf_surplus=$(cat "$cmf_hp/surplus_hugepages" 2>/dev/null)
        jq -n --arg size "$cmf_name" --arg total "$cmf_total" --arg free "$cmf_free" --arg resv "$cmf_resv" --arg surplus "$cmf_surplus" '{size:$size,total:(try ($total|tonumber) catch null),free:(try ($free|tonumber) catch null),reserved:(try ($resv|tonumber) catch null),surplus:(try ($surplus|tonumber) catch null)}' >> "$cmf_huge_stream" || return 1
    done
    jq -s '.' "$cmf_huge_stream" > "$cmf_dir/huge.json" || return 1
    printf '%s\n' "$cmf_buddy" | awk 'NF>4{zone=$4; gsub(/,/,"",zone); for(i=5;i<=NF;i++) if($i+0>0) last=i-5; print $2"\t"zone"\t"last}' > "$cmf_dir/buddy-high.tsv" 2>/dev/null
    jq -R -s 'split("\n")|map(select(length>0)|split("\t")|{node:.[0],zone:.[1],highest_order_with_free:(.[2]|tonumber)})' "$cmf_dir/buddy-high.tsv" > "$cmf_dir/buddy-high.json" || return 1
    jq -n --arg buddy "$cmf_buddy" --arg pagetype "$cmf_page" --arg zone "$cmf_zone" --arg vm "$cmf_vm" --arg hpmd "$cmf_hpmd" --arg thpe "$cmf_thp_enabled" --arg thpd "$cmf_thp_defrag" --arg comp "$cmf_compaction" --arg ext "$cmf_extfrag" --arg minfree "$cmf_minfree" --arg ws "$cmf_wscale" --arg wb "$cmf_wboost" --arg zr "$cmf_zreclaim" --slurpfile huge "$cmf_dir/huge.json" --slurpfile bh "$cmf_dir/buddy-high.json" '{buddyinfo_raw:$buddy,pagetypeinfo_raw:$pagetype,zoneinfo_raw:$zone,vmstat_fragmentation_raw:$vm,buddy_highest_order:$bh[0],transparent_hugepage:{hpage_pmd_size:(try ($hpmd|tonumber) catch null),enabled:$thpe,defrag:$thpd},hugetlb:$huge[0],vm_tunables:{compaction_proactiveness:(try ($comp|tonumber) catch null),extfrag_threshold:(try ($ext|tonumber) catch null),min_free_kbytes:(try ($minfree|tonumber) catch null),watermark_scale_factor:(try ($ws|tonumber) catch null),watermark_boost_factor:(try ($wb|tonumber) catch null),zone_reclaim_mode:(try ($zr|tonumber) catch null)}}' > "$cmf_dir/data.json" || return 1
    inv_strings_json "$cmf_dir/notes.json" "Hugepage feasibility is observational only; the collector never allocates, reserves or compacts memory." || return 1
    inv_strings_json "$cmf_dir/errors.json" || return 1
    inv_emit memory-fragmentation "$cmf_dir/data.json" "$cmf_dir/notes.json" "$cmf_dir/errors.json"
}
print_memory_fragmentation_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "Memory Fragmentation / Hugepage Feasibility View"
    inv_view_kv "THP" "$(jq -r '.data.transparent_hugepage.enabled // "unknown"' "$f")"
    inv_view_kv "THP defrag" "$(jq -r '.data.transparent_hugepage.defrag // "unknown"' "$f")"
    inv_view_section "HugeTLB pools"
    jq -r '.data.hugetlb[]?|"\(.size) total=\(.total // "n/a") free=\(.free // "n/a") reserved=\(.reserved // "n/a")"' "$f" | while IFS= read -r line; do inv_view_item "$line"; done
    inv_view_section "Buddy highest free orders"
    jq -r '.data.buddy_highest_order[]?|"node=\(.node) zone=\(.zone) highest-order=\(.highest_order_with_free)"' "$f" | while IFS= read -r line; do inv_view_item "$line"; done
    inv_view_metadata "$f"
}

# ============================================================
# print_memory_fragmentation_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_memory_fragmentation_summary() { print_memory_fragmentation_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_memory_fragmentation_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_memory_fragmentation.summary-source.json
        collect_memory_fragmentation > "$main_summary_file" || return $?
        print_memory_fragmentation_view "$main_summary_file"
        return $?
    fi
    collect_memory_fragmentation
}
