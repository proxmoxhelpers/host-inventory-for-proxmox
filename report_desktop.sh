...
usage() {
cat <<'EOF'
report_desktop.sh - Hypervisor Desktop — latency / stutter / jitter evidence

Usage:
  ./report_desktop.sh --from-run RUN_DIR [--short|--dense-summary|--dense]
  ./report_desktop.sh --file host-map.json [--short|--dense-summary|--dense]
...
printf '%s%s%s\n' "$c_title" "Hypervisor Desktop — latency / stutter / jitter evidence" "$c_reset"
printf '%s%s  %s → %s%s\n' "$c_dim" "$app_host" "$app_start" "$app_end" "$c_reset"

if [ "$app_style" = dense_summary ]; then
    jq -r '.reports.desktop.dense_summary[] | [.title, ([.rows[] | (.label+"="+.value)] | join("  │  "))] | @tsv' "$app_model" |
...
if [ "$app_style" = short ]; then app_path='.reports.desktop.short'; else app_path='.reports.desktop.dense'; fi
