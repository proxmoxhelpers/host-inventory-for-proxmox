...
# ============================================================
# view_network.sh
# Collects physical/virtual interfaces, bridges, queues, offloads and coalescing settings.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_network.sh
#   ./view_network.sh --file collect_network.json
#   ./view_network.sh --help
#
# Output:
...
app_name="view_network"
app_version=0.9.2
app_rc=0
app_compact=0
app_package_specs="jq:jq:required ip:iproute2:optional ethtool:ethtool:optional"
...
usage() {
    cat <<'EOF'
view_network.sh - standalone network inventory view

Usage:
  sudo ./view_network.sh
  ./view_network.sh --file collect_network.json
  sudo ./view_network.sh --json > collect_network.json

Default behavior:
  With no arguments, collect the network aspect live and
  render its colorized human-readable view.

View input:
  --file FILE       Render a previously saved collect_network.json envelope.
...
# ============================================================
# collect_network
# Collects interface topology and latency-relevant NIC queue settings.
#
# Version:
#   1.0.0
#
...
# ============================================================
collect_network() {
    cnw_dir=$_inv_temp_dir/network
    cnw_stream=$cnw_dir/interfaces.jsonl
    mkdir -p "$cnw_dir" || return 1
    : > "$cnw_stream"
    for cnw_if in /sys/class/net/*; do
        [ -d "$cnw_if" ] || continue
        cnw_name=${cnw_if##*/}
        [ "$cnw_name" = lo ] && continue
        cnw_queue_stream=$cnw_dir/queues.jsonl
        : > "$cnw_queue_stream"
        for cnw_q in "$cnw_if"/queues/*; do
            [ -d "$cnw_q" ] || continue
            inv_flat_dir_json "$cnw_dir/qvalues.json" "$cnw_q" '^(rps_cpus|rps_flow_cnt|xps_cpus|xps_rxqs)$' || return 1
            jq -n --arg queue "${cnw_q##*/}" --slurpfile values "$cnw_dir/qvalues.json" '{queue:$queue,values:$values[0]}' >> "$cnw_queue_stream" || return 1
        done
        jq -s '.' "$cnw_queue_stream" > "$cnw_dir/queues.json" || return 1
        inv_command_json "$cnw_dir/ethtool.json" 8 ethtool "$cnw_name" || return 1
        inv_command_json "$cnw_dir/channels.json" 8 ethtool -l "$cnw_name" || return 1
        inv_command_json "$cnw_dir/coalesce.json" 8 ethtool -c "$cnw_name" || return 1
        inv_command_json "$cnw_dir/features.json" 8 ethtool -k "$cnw_name" || return 1
        inv_command_json "$cnw_dir/ring.json" 8 ethtool -g "$cnw_name" || return 1
        cnw_devpath=$(inv_readlink "$cnw_if/device" 2>/dev/null)
        cnw_driver=$(inv_readlink "$cnw_if/device/driver" 2>/dev/null)
        jq -n --arg name "$cnw_name" --arg operstate "$(cat "$cnw_if/operstate" 2>/dev/null)" --arg mtu "$(cat "$cnw_if/mtu" 2>/dev/null)" --arg address "$(cat "$cnw_if/address" 2>/dev/null)" --arg numa_node "$(cat "$cnw_if/device/numa_node" 2>/dev/null)" --arg device_path "$cnw_devpath" --arg driver "$cnw_driver" --slurpfile queues "$cnw_dir/queues.json" --slurpfile ethtool "$cnw_dir/ethtool.json" --slurpfile channels "$cnw_dir/channels.json" --slurpfile coalesce "$cnw_dir/coalesce.json" --slurpfile features "$cnw_dir/features.json" --slurpfile ring "$cnw_dir/ring.json" '{name:$name,operstate:$operstate,mtu:$mtu,address:$address,numa_node:$numa_node,device_path:$device_path,driver:$driver,queues:$queues[0],ethtool:$ethtool[0],channels:$channels[0],coalesce:$coalesce[0],features:$features[0],ring:$ring[0]}' >> "$cnw_stream" || return 1
    done
    jq -s '.' "$cnw_stream" > "$cnw_dir/interfaces.json" || return 1
    inv_command_json "$cnw_dir/ip_link.json" 10 ip -j -details link show || return 1
    inv_command_json "$cnw_dir/ip_addr.json" 10 ip -j address show || return 1
    inv_command_json "$cnw_dir/ip_route.json" 10 ip -j route show table all || return 1
    inv_command_json "$cnw_dir/bridge_link.json" 10 bridge -j -details link show || return 1
    inv_command_json "$cnw_dir/bridge_vlan.json" 10 bridge -j vlan show || return 1
    inv_command_json "$cnw_dir/tc.json" 10 tc -s qdisc show || return 1
    jq -n --arg interfaces_cfg "$(cat /etc/network/interfaces 2>/dev/null)" --slurpfile interfaces "$cnw_dir/interfaces.json" --slurpfile ip_link "$cnw_dir/ip_link.json" --slurpfile ip_addr "$cnw_dir/ip_addr.json" --slurpfile ip_route "$cnw_dir/ip_route.json" --slurpfile bridge_link "$cnw_dir/bridge_link.json" --slurpfile bridge_vlan "$cnw_dir/bridge_vlan.json" --slurpfile tc "$cnw_dir/tc.json" '{interfaces:$interfaces[0],bridge_config_raw:$interfaces_cfg,raw:{ip_link:$ip_link[0],ip_addr:$ip_addr[0],ip_route:$ip_route[0],bridge_link:$bridge_link[0],bridge_vlan:$bridge_vlan[0],tc_qdisc:$tc[0]}}' > "$cnw_dir/data.json" || return 1
    inv_strings_json "$cnw_dir/notes.json" || return 1
    inv_strings_json "$cnw_dir/errors.json" || return 1
    inv_emit network "$cnw_dir/data.json" "$cnw_dir/notes.json" "$cnw_dir/errors.json"
}
...
# ============================================================
# print_network_view
# Presents a colorized network view.
#
# Version:
#   1.1.0
# ============================================================
print_network_view() {
    pnv_file=$1
    inv_summary_validate_file "$pnv_file" || return $?
    inv_view_header "$pnv_file" "Network View"

    inv_view_section "Network overview"
    inv_view_kv "Interfaces" "$(jq -r '.data.interfaces|length' "$pnv_file")"
    inv_view_kv "Physical interfaces" "$(jq -r '[.data.interfaces[] | select((.driver//"")!="")] | length' "$pnv_file")"
    inv_view_kv "Host/Firewall bridges" "$(jq -r '[.data.interfaces[] | select(.name|test("^(vmbr|fwbr)"))] | length' "$pnv_file")"
    inv_view_kv "Guest TAP interfaces" "$(jq -r '[.data.interfaces[] | select(.name|startswith("tap"))] | length' "$pnv_file")"
    inv_view_kv "Container veth interfaces" "$(jq -r '[.data.interfaces[] | select(.name|startswith("veth"))] | length' "$pnv_file")"

    inv_view_section "Physical interfaces"
    pnv_rows=$(jq -r '.data.interfaces
        | map(select((.driver//"")!=""))
        | sort_by(.name)[]
        | [.name, (.operstate // "unknown"), (.mtu // "?"), (.address // "?"),
           (if (.numa_node // "") == "" then "n/a" else .numa_node end),
           .driver, (.queues|length|tostring),
           ((.channels.rc // "unavailable")|tostring),
           ((.coalesce.rc // "unavailable")|tostring)]
        | @tsv' "$pnv_file") || return 1
    printf '%s\n' "$pnv_rows" |
    while IFS="$(printf '\t')" read -r pnv_name pnv_state pnv_mtu pnv_mac pnv_node pnv_driver pnv_queues pnv_ch pnv_co; do
        pnv_driver=${pnv_driver##*/}
        inv_view_item "$pnv_name state=$pnv_state mtu=$pnv_mtu mac=$pnv_mac node=$pnv_node driver=$pnv_driver queues=$pnv_queues channels-rc=$pnv_ch coalesce-rc=$pnv_co"
    done

    inv_view_section "Host / firewall bridges"
    jq -r '.data.interfaces
        | map(select(.name|test("^(vmbr|fwbr)")))
        | sort_by(.name)[]
        | [.name,(.operstate//"unknown"),(.mtu//"?"),(.address//"?")]
        | @tsv' "$pnv_file" |
    while IFS="$(printf '\t')" read -r pnv_name pnv_state pnv_mtu pnv_mac; do
        inv_view_item "$pnv_name state=$pnv_state mtu=$pnv_mtu mac=$pnv_mac"
    done

    inv_view_section "Guest / container virtual interfaces"
    inv_view_kv "TAP" "$(jq -r '[.data.interfaces[] | select(.name|startswith("tap")) | .name] | join(", ") | if .=="" then "none" else . end' "$pnv_file")"
    inv_view_kv "veth" "$(jq -r '[.data.interfaces[] | select(.name|startswith("veth")) | .name] | join(", ") | if .=="" then "none" else . end' "$pnv_file")"
    inv_view_kv "Firewall links" "$(jq -r '[.data.interfaces[] | select(.name|test("^(fwln|fwpr)")) | .name] | join(", ") | if .=="" then "none" else . end' "$pnv_file")"

    inv_view_section "Configured host networking"
    pnv_cfg=$(jq -r '.data.bridge_config_raw // ""' "$pnv_file")
    if [ -n "$pnv_cfg" ]; then
        printf '%s\n' "$pnv_cfg" | sed -e '/^[[:space:]]*#/d' -e '/^[[:space:]]*$/d' |
        while IFS= read -r pnv_line; do
            printf '    %s\n' "$pnv_line"
        done
    else
        inv_view_item "No /etc/network/interfaces content captured."
    fi

    inv_view_metadata "$pnv_file"
}

# ============================================================
# print_network_summary
# Presents host NIC, queue and bridge-related inventory.
#
# Version:
#   1.1.0
#
# Usage:
#   print_network_summary collectorEnvelope.json
#
# Returns:
#   0 on success
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_network_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_network.summary-source.json
        collect_network > "$main_summary_file" || return $?
        print_network_view "$main_summary_file"
        return $?
    fi
    collect_network
}
