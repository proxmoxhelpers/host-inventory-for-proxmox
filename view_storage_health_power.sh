...
# Collects storage health, power-management, NVMe feature and advanced block queue evidence.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_storage_health_power.sh
#   ./view_storage_health_power.sh --file collect_storage_health_power.json
#   ./view_storage_health_power.sh --help
#
# Output:
...
app_name="view_storage_health_power"
app_version=0.9.2
app_rc=0
app_compact=0
app_package_specs="jq:jq:required nvme:nvme-cli:optional lsblk:util-linux:optional"
...
usage() {
    cat <<'EOF'
view_storage_health_power.sh - standalone storage health / power view

Usage:
  sudo ./view_storage_health_power.sh
  ./view_storage_health_power.sh --file collect_storage_health_power.json
  sudo ./view_storage_health_power.sh --json > collect_storage_health_power.json
...
collect_storage_health_power() {
    shp_dir=$_inv_temp_dir/storage-health-power
    mkdir -p "$shp_dir" || return 1
    shp_stream=$shp_dir/disks.jsonl; : > "$shp_stream" || return 1
    for shp_path in /sys/block/*; do
        [ -d "$shp_path" ] || continue
        shp_name=${shp_path##*/}
        case "$shp_name" in loop*|dm-*|ram*|zram*) continue ;; esac
        shp_q=$shp_dir/q-$shp_name.json
        inv_file_map_json "$shp_q" "scheduler=$shp_path/queue/scheduler" "nr_requests=$shp_path/queue/nr_requests" "rq_affinity=$shp_path/queue/rq_affinity" "nomerges=$shp_path/queue/nomerges" "read_ahead_kb=$shp_path/queue/read_ahead_kb" "wbt_lat_usec=$shp_path/queue/wbt_lat_usec" "max_sectors_kb=$shp_path/queue/max_sectors_kb" "max_hw_sectors_kb=$shp_path/queue/max_hw_sectors_kb" "io_poll=$shp_path/queue/io_poll" "io_poll_delay=$shp_path/queue/io_poll_delay" || return 1
        shp_devpath=$(readlink -f "$shp_path/device" 2>/dev/null)
        shp_power_control=$(cat "$shp_path/device/power/control" 2>/dev/null)
        shp_runtime=$(cat "$shp_path/device/power/runtime_status" 2>/dev/null)
        shp_smart=$shp_dir/smart-$shp_name.json
        inv_command_json "$shp_smart" 20 smartctl -x "/dev/$shp_name" || return 1
        jq -n --arg name "$shp_name" --arg devpath "$shp_devpath" --arg pctl "$shp_power_control" --arg runtime "$shp_runtime" --slurpfile queue "$shp_q" --slurpfile smart "$shp_smart" '{name:$name,device_path:(if $devpath=="" then null else $devpath end),queue:$queue[0],power:{control:(if $pctl=="" then null else $pctl end),runtime_status:(if $runtime=="" then null else $runtime end)},smartctl:$smart[0]}' >> "$shp_stream" || return 1
    done
    jq -s 'sort_by(.name)' "$shp_stream" > "$shp_dir/disks.json" || return 1

    shp_nvme_stream=$shp_dir/nvme.jsonl; : > "$shp_nvme_stream" || return 1
    for shp_ctrl_path in /sys/class/nvme/nvme[0-9]*; do
        [ -d "$shp_ctrl_path" ] || continue
        shp_ctrl=${shp_ctrl_path##*/}; shp_dev=/dev/$shp_ctrl
        inv_command_json "$shp_dir/$shp_ctrl-smart.json" 12 nvme smart-log "$shp_dev" -o json || return 1
        inv_command_json "$shp_dir/$shp_ctrl-id.json" 12 nvme id-ctrl "$shp_dev" -o json || return 1
        inv_command_json "$shp_dir/$shp_ctrl-apst.json" 12 nvme get-feature "$shp_dev" -f 0x0c -H || return 1
        inv_command_json "$shp_dir/$shp_ctrl-coal.json" 12 nvme get-feature "$shp_dev" -f 0x08 -H || return 1
        inv_command_json "$shp_dir/$shp_ctrl-queues.json" 12 nvme get-feature "$shp_dev" -f 0x07 -H || return 1
        inv_command_json "$shp_dir/$shp_ctrl-pm.json" 12 nvme get-feature "$shp_dev" -f 0x02 -H || return 1
        jq -n --arg controller "$shp_ctrl" --slurpfile smart "$shp_dir/$shp_ctrl-smart.json" --slurpfile id "$shp_dir/$shp_ctrl-id.json" --slurpfile apst "$shp_dir/$shp_ctrl-apst.json" --slurpfile coal "$shp_dir/$shp_ctrl-coal.json" --slurpfile queues "$shp_dir/$shp_ctrl-queues.json" --slurpfile pm "$shp_dir/$shp_ctrl-pm.json" '{controller:$controller,smart_log:$smart[0],identify_controller:$id[0],features:{apst:$apst[0],interrupt_coalescing:$coal[0],number_of_queues:$queues[0],power_management:$pm[0]}}' >> "$shp_nvme_stream" || return 1
    done
    jq -s '.' "$shp_nvme_stream" > "$shp_dir/nvme.json" || return 1

    shp_hosts=$shp_dir/scsi-hosts.jsonl; : > "$shp_hosts"
    for shp_host in /sys/class/scsi_host/host*; do
        [ -d "$shp_host" ] || continue
        shp_name=${shp_host##*/}; shp_policy=$(cat "$shp_host/link_power_management_policy" 2>/dev/null); shp_proc=$(cat "$shp_host/proc_name" 2>/dev/null)
        jq -n --arg host "$shp_name" --arg policy "$shp_policy" --arg proc "$shp_proc" '{host:$host,driver:(if $proc=="" then null else $proc end),link_power_management_policy:(if $policy=="" then null else $policy end)}' >> "$shp_hosts" || return 1
    done
    jq -s '.' "$shp_hosts" > "$shp_dir/scsi-hosts.json" || return 1
    jq -n --slurpfile disks "$shp_dir/disks.json" --slurpfile nvme "$shp_dir/nvme.json" --slurpfile hosts "$shp_dir/scsi-hosts.json" '{disks:$disks[0],nvme:$nvme[0],scsi_hosts:$hosts[0]}' > "$shp_dir/data.json" || return 1
    inv_strings_json "$shp_dir/notes.json" "SMART, NVMe feature and queue data are read-only observations; no self-test or benchmark is started." || return 1
    inv_strings_json "$shp_dir/errors.json" || return 1
    inv_emit storage-health-power "$shp_dir/data.json" "$shp_dir/notes.json" "$shp_dir/errors.json"
}

print_storage_health_power_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "Storage Health / Power View"
    inv_view_section "Physical disks"
    jq -r '.data.disks[]|[.name,(.queue.scheduler//"?"),(.queue.rq_affinity//"?"),(.queue.wbt_lat_usec//"?"),(.power.runtime_status//"n/a"),(.smartctl.rc//"unavailable"|tostring)]|@tsv' "$f" | while IFS="$(printf '\t')" read -r n sched rq wbt pwr src; do inv_view_item "$n scheduler=$sched rq_affinity=$rq wbt_lat_usec=$wbt power=$pwr smartctl-rc=$src"; done
    inv_view_section "NVMe health / features"
    jq -r '.data.nvme[]|[.controller,(.smart_log.rc//"unavailable"|tostring),(.features.apst.rc//"unavailable"|tostring),(.features.interrupt_coalescing.rc//"unavailable"|tostring),(.features.number_of_queues.rc//"unavailable"|tostring)]|@tsv' "$f" | while IFS="$(printf '\t')" read -r n s a c q; do inv_view_item "$n smart-rc=$s apst-rc=$a coalescing-rc=$c queues-rc=$q"; done
    inv_view_section "SCSI/SATA link power policy"
    jq -r '.data.scsi_hosts[]|[.host,(.driver//"?"),(.link_power_management_policy//"not-exposed")]|@tsv' "$f" | while IFS="$(printf '\t')" read -r h d p; do inv_view_item "$h driver=$d policy=$p"; done
    inv_view_metadata "$f"
}

# ============================================================
# print_storage_health_power_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_storage_health_power_summary() { print_storage_health_power_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_storage_health_power_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_storage_health_power.summary-source.json
        collect_storage_health_power > "$main_summary_file" || return $?
        print_storage_health_power_view "$main_summary_file"
        return $?
    fi
    collect_storage_health_power
}
