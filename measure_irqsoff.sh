...
# ============================================================
# measure_irqsoff.sh
# Runs the kernel irqsoff tracer for a bounded interval after explicit acknowledgement.
#
# Version:
#   0.9.2
#
...
usage() {
    cat <<'EOF'
measure_irqsoff.sh - trace long IRQ-disabled sections

Usage:
  ./measure_irqsoff.sh [options]
...
main() {
    [ "$app_ack_tracing" -eq 1 ] || { printf 'ERROR: --ack-tracing is required for irqsoff tracing.\n' >&2; return 2; }
    if [ "$app_dry_run" -eq 1 ]; then
        printf 'DRY-RUN: tracefs=<auto> tracer=irqsoff duration=%ss\n' "$app_duration"
        return 0
    fi
    find_tracefs || return $?
    [ "$(id -u)" -eq 0 ] || { printf 'ERROR: irqsoff tracing requires root.\n' >&2; return 2; }
