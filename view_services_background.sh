...
# Collects Proxmox/systemd services, timers, backup/replication and cron evidence.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_services_background.sh
#   ./view_services_background.sh --file collect_services_background.json
#   ./view_services_background.sh --help
#
# Output:
...
app_name="view_services_background"
app_version=0.9.2
app_rc=0
app_compact=0
...
usage() {
    cat <<'EOF'
view_services_background.sh - standalone services background inventory view

Usage:
  sudo ./view_services_background.sh
  ./view_services_background.sh --file collect_services_background.json
  sudo ./view_services_background.sh --json > collect_services_background.json

Default behavior:
  With no arguments, collect the services background aspect live and
  render its colorized human-readable view.

View input:
  --file FILE       Render a previously saved collect_services_background.json envelope.
...
# ============================================================
# collect_services_background
# Collects service affinity/state and scheduled background-work evidence.
#
# Version:
#   1.0.0
#
...
# ============================================================
collect_services_background() {
    csb_dir=$_inv_temp_dir/services
    csb_stream=$csb_dir/units.jsonl
    mkdir -p "$csb_dir" || return 1
    : > "$csb_stream"
    for csb_unit in pvestatd.service pvedaemon.service pveproxy.service pve-firewall.service pve-ha-lrm.service pve-ha-crm.service pvescheduler.service irqbalance.service ksmtuned.service smartmontools.service fstrim.timer apt-daily.timer apt-daily-upgrade.timer; do
        inv_command_json "$csb_dir/unit.json" 6 systemctl show "$csb_unit" -p LoadState -p ActiveState -p SubState -p UnitFileState -p CPUAccounting -p AllowedCPUs -p EffectiveCPUs || return 1
        jq -n --arg unit "$csb_unit" --slurpfile state "$csb_dir/unit.json" '{unit:$unit,state:$state[0]}' >> "$csb_stream" || return 1
    done
    jq -s '.' "$csb_stream" > "$csb_dir/units.json" || return 1
    inv_command_json "$csb_dir/root_crontab.json" 6 sh -c 'if [ -r /var/spool/cron/crontabs/root ]; then cat /var/spool/cron/crontabs/root; else exit 1; fi' || return 1
    inv_command_json "$csb_dir/cron_d.json" 8 sh -c "grep -RnsH . /etc/cron.d 2>/dev/null || true" || return 1
    inv_command_json "$csb_dir/timers.json" 12 systemctl list-timers --all --no-pager || return 1
    inv_command_json "$csb_dir/running_services.json" 12 systemctl list-units --type=service --state=running --no-pager || return 1
    inv_command_json "$csb_dir/failed.json" 10 systemctl --failed --no-pager || return 1
    inv_command_json "$csb_dir/qm.json" 10 qm list || return 1
    inv_command_json "$csb_dir/pct.json" 10 pct list || return 1
    inv_command_json "$csb_dir/ha.json" 10 ha-manager status || return 1
    jq -n --arg jobs "$(cat /etc/pve/jobs.cfg 2>/dev/null)" --arg vzdump "$(cat /etc/vzdump.conf 2>/dev/null)" --arg replication "$(cat /etc/pve/replication.cfg 2>/dev/null)" --slurpfile units "$csb_dir/units.json" --slurpfile cron "$csb_dir/root_crontab.json" --slurpfile crond "$csb_dir/cron_d.json" --slurpfile timers "$csb_dir/timers.json" --slurpfile running "$csb_dir/running_services.json" --slurpfile failed "$csb_dir/failed.json" --slurpfile qm "$csb_dir/qm.json" --slurpfile pct "$csb_dir/pct.json" --slurpfile ha "$csb_dir/ha.json" '{important_units:$units[0],proxmox_jobs_cfg_raw:$jobs,vzdump_cfg_raw:$vzdump,replication_cfg_raw:$replication,cron:{root_crontab:$cron[0],cron_d:$crond[0]},raw:{systemd_timers:$timers[0],running_services:$running[0],failed_units:$failed[0],qm_list:$qm[0],pct_list:$pct[0],ha_status:$ha[0]}}' > "$csb_dir/data.json" || return 1
    inv_strings_json "$csb_dir/notes.json" "No service is stopped, masked, enabled or disabled by this collector." || return 1
    inv_strings_json "$csb_dir/errors.json" || return 1
    inv_emit services-background "$csb_dir/data.json" "$csb_dir/notes.json" "$csb_dir/errors.json"
}



# ============================================================
# print_services_background_view
# Presents a colorized services / background-work view.
#
# Version:
#   1.1.0
# ============================================================
print_services_background_view() {
    psbv_file=$1
    inv_summary_validate_file "$psbv_file" || return $?
    inv_view_header "$psbv_file" "Services / Background Work View"

    inv_view_section "Overview"
    inv_view_kv "Important units" "$(jq -r '.data.important_units|length' "$psbv_file")"
    inv_view_kv "Timer probe" "rc=$(jq -r '.data.raw.systemd_timers.rc // "unavailable"' "$psbv_file")"
    inv_view_kv "Failed-unit probe" "rc=$(jq -r '.data.raw.failed_units.rc // "unavailable"' "$psbv_file")"
    inv_view_kv "PVE jobs config" "$(jq -r 'if (.data.proxmox_jobs_cfg_raw//"") == "" then "no/empty" else "present" end' "$psbv_file")"
    inv_view_kv "Replication config" "$(jq -r 'if (.data.replication_cfg_raw//"") == "" then "no/empty" else "present" end' "$psbv_file")"

    inv_view_section "Important services"
    jq -r '.data.important_units[]
        | select(.unit|endswith(".service"))
        | [.unit,
           ((.state.stdout//"") | split("\n") | map(select(startswith("ActiveState="))) | first // "ActiveState=unknown"),
           ((.state.stdout//"") | split("\n") | map(select(startswith("EffectiveCPUs="))) | first // "EffectiveCPUs=unknown"),
           ((.state.stdout//"") | split("\n") | map(select(startswith("AllowedCPUs="))) | first // "AllowedCPUs=")]
        | @tsv' "$psbv_file" |
    while IFS="$(printf '\t')" read -r psbv_unit psbv_active psbv_effective psbv_allowed; do
        inv_view_item "$psbv_unit ${psbv_active#ActiveState=} ${psbv_effective} ${psbv_allowed}"
    done

    inv_view_section "Important timers"
    jq -r '.data.important_units[]
        | select(.unit|endswith(".timer"))
        | [.unit,
           ((.state.stdout//"") | split("\n") | map(select(startswith("ActiveState="))) | first // "ActiveState=unknown"),
           ((.state.stdout//"") | split("\n") | map(select(startswith("SubState="))) | first // "SubState=unknown")]
        | @tsv' "$psbv_file" |
    while IFS="$(printf '\t')" read -r psbv_unit psbv_active psbv_sub; do
        inv_view_item "$psbv_unit ${psbv_active#ActiveState=} ${psbv_sub#SubState=}"
    done

    inv_view_metadata "$psbv_file"
}

# ============================================================
# print_services_background_summary
# Presents important services, timers and scheduled-work evidence.
#
# Version:
#   1.1.0
#
# Usage:
#   print_services_background_summary collectorEnvelope.json
#
# Returns:
#   0 on success
...
# ============================================================
print_services_background_summary() {
    print_services_background_view "$1"
}
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_services_background_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_services_background.summary-source.json
        collect_services_background > "$main_summary_file" || return $?
        print_services_background_view "$main_summary_file"
        return $?
    fi
    collect_services_background
}
