...
# Collects CPU cache topology, NUMA distance and resctrl/RDT/AMD-QoS capability evidence without mounting or configuring resctrl.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_cache_resource_qos.sh
#   ./view_cache_resource_qos.sh --file collect_runtime_pressure.json
#   ./view_cache_resource_qos.sh --help
#
# Output:
...
app_name="view_cache_resource_qos"
app_version=0.9.2
app_rc=0
app_compact=0
app_package_specs="jq:jq:required findmnt:util-linux:optional"
...
usage() {
    cat <<'EOF'
view_cache_resource_qos.sh - collect cache topology, NUMA/fabric distance and resctrl resource-control capabilities

Usage:
  ./view_cache_resource_qos.sh [--compact|--summary|--view]
  ./view_cache_resource_qos.sh --help
  ./view_cache_resource_qos.sh --version

Read-only observation. Unsupported interfaces remain unavailable rather than
being inferred or enabled automatically.
EOF
    inv_print_common_usage
    return 0
}

collect_cache_resource_qos() {
    crq_dir=$_inv_temp_dir/cache-resource-qos
    mkdir -p "$crq_dir" || return 1
    crq_cache_stream=$crq_dir/cache.jsonl; : > "$crq_cache_stream" || return 1
    for crq_idx in /sys/devices/system/cpu/cpu[0-9]*/cache/index*; do
        [ -d "$crq_idx" ] || continue
        crq_cpu=${crq_idx%/cache/index*}; crq_cpu=${crq_cpu##*cpu}
        crq_index=${crq_idx##*index}
        crq_level=$(cat "$crq_idx/level" 2>/dev/null)
        crq_type=$(cat "$crq_idx/type" 2>/dev/null)
        crq_size=$(cat "$crq_idx/size" 2>/dev/null)
        crq_shared=$(cat "$crq_idx/shared_cpu_list" 2>/dev/null)
        crq_id=$(cat "$crq_idx/id" 2>/dev/null)
        crq_ways=$(cat "$crq_idx/ways_of_associativity" 2>/dev/null)
        crq_sets=$(cat "$crq_idx/number_of_sets" 2>/dev/null)
        crq_line=$(cat "$crq_idx/coherency_line_size" 2>/dev/null)
        crq_alloc=$(cat "$crq_idx/allocation_policy" 2>/dev/null)
        crq_write=$(cat "$crq_idx/write_policy" 2>/dev/null)
        jq -n --arg cpu "$crq_cpu" --arg idx "$crq_index" --arg lvl "$crq_level" --arg typ "$crq_type" --arg size "$crq_size" --arg shared "$crq_shared" --arg cid "$crq_id" --arg ways "$crq_ways" --arg sets "$crq_sets" --arg line "$crq_line" --arg alloc "$crq_alloc" --arg write "$crq_write" '{cpu:($cpu|tonumber),index:($idx|tonumber),level:(try ($lvl|tonumber) catch null),type:$typ,size:$size,shared_cpu_list:$shared,cache_id:(try ($cid|tonumber) catch null),ways_of_associativity:(try ($ways|tonumber) catch null),number_of_sets:(try ($sets|tonumber) catch null),coherency_line_size:(try ($line|tonumber) catch null),allocation_policy:(if $alloc=="" then null else $alloc end),write_policy:(if $write=="" then null else $write end)}' >> "$crq_cache_stream" || return 1
    done
    jq -s 'sort_by(.level,.type,.cache_id,.cpu)' "$crq_cache_stream" > "$crq_dir/cache.json" || return 1
    crq_node_stream=$crq_dir/nodes.jsonl; : > "$crq_node_stream" || return 1
    for crq_node in /sys/devices/system/node/node[0-9]*; do
        [ -d "$crq_node" ] || continue
        crq_nid=${crq_node##*node}
        crq_cpus=$(cat "$crq_node/cpulist" 2>/dev/null)
        crq_distance=$(cat "$crq_node/distance" 2>/dev/null)
        jq -n --arg node "$crq_nid" --arg cpus "$crq_cpus" --arg distance "$crq_distance" '{node:($node|tonumber),cpulist:$cpus,distance_raw:$distance}' >> "$crq_node_stream" || return 1
    done
    jq -s 'sort_by(.node)' "$crq_node_stream" > "$crq_dir/nodes.json" || return 1
    crq_flags=$(awk -F: '/^flags[[:space:]]*:/{sub(/^[[:space:]]+/,"",$2);print $2;exit}' /proc/cpuinfo 2>/dev/null)
    crq_resctrl_fs=false; grep -qw resctrl /proc/filesystems 2>/dev/null && crq_resctrl_fs=true
    crq_mounted=false; [ -d /sys/fs/resctrl/info ] && crq_mounted=true
    crq_info_stream=$crq_dir/resctrl.jsonl; : > "$crq_info_stream" || return 1
    crq_group_stream=$crq_dir/resctrl-groups.jsonl; : > "$crq_group_stream" || return 1
    if [ "$crq_mounted" = true ]; then
        find /sys/fs/resctrl/info -maxdepth 3 -type f -readable 2>/dev/null | sort | while IFS= read -r crq_file; do
            crq_key=${crq_file#/sys/fs/resctrl/info/}; crq_val=$(cat "$crq_file" 2>/dev/null)
            jq -n --arg path "$crq_key" --arg value "$crq_val" '{path:$path,value:$value}' >> "$crq_info_stream"
        done
        for crq_group_dir in /sys/fs/resctrl /sys/fs/resctrl/* /sys/fs/resctrl/*/mon_groups/*; do
            [ -d "$crq_group_dir" ] || continue
            case "$crq_group_dir" in
                /sys/fs/resctrl/info|/sys/fs/resctrl/mon_data|/sys/fs/resctrl/mon_groups|*/mon_data|*/mon_groups) continue ;;
            esac
            [ -e "$crq_group_dir/cpus" ] || [ -e "$crq_group_dir/tasks" ] || [ -e "$crq_group_dir/schemata" ] || continue
            if [ "$crq_group_dir" = /sys/fs/resctrl ]; then crq_group_name=root; crq_group_kind=root
            else
                crq_group_name=${crq_group_dir#/sys/fs/resctrl/}
                case "$crq_group_name" in */mon_groups/*) crq_group_kind=monitor ;; *) crq_group_kind=control ;; esac
            fi
            crq_group_cpus=$(cat "$crq_group_dir/cpus" 2>/dev/null)
            crq_group_cpus_list=$(cat "$crq_group_dir/cpus_list" 2>/dev/null)
            crq_group_tasks=$(cat "$crq_group_dir/tasks" 2>/dev/null)
            crq_group_schemata=$(cat "$crq_group_dir/schemata" 2>/dev/null)
            crq_group_mode=$(cat "$crq_group_dir/mode" 2>/dev/null)
            jq -n --arg name "$crq_group_name" --arg kind "$crq_group_kind" --arg cpus "$crq_group_cpus" --arg cpus_list "$crq_group_cpus_list" --arg tasks "$crq_group_tasks" --arg schemata "$crq_group_schemata" --arg mode "$crq_group_mode" '{name:$name,kind:$kind,cpus_mask:(if $cpus=="" then null else $cpus end),cpus_list:(if $cpus_list=="" then null else $cpus_list end),tasks:[$tasks|split("\n")[]|select(length>0)|(tonumber? // .)],task_count:([$tasks|split("\n")[]|select(length>0)]|length),schemata_raw:(if $schemata=="" then null else $schemata end),mode:(if $mode=="" then null else $mode end)}' >> "$crq_group_stream" || return 1
        done
    fi
    jq -s '.' "$crq_info_stream" > "$crq_dir/resctrl.json" || return 1
    jq -s '.' "$crq_group_stream" > "$crq_dir/resctrl-groups.json" || return 1
    jq -n --arg flags "$crq_flags" --argjson fs "$crq_resctrl_fs" --argjson mounted "$crq_mounted" --slurpfile caches "$crq_dir/cache.json" --slurpfile nodes "$crq_dir/nodes.json" --slurpfile info "$crq_dir/resctrl.json" --slurpfile groups "$crq_dir/resctrl-groups.json" '{cpu_flags:$flags,cache_entries:$caches[0],numa_nodes:$nodes[0],resctrl:{filesystem_supported:$fs,mounted:$mounted,info_files:$info[0],groups:$groups[0]},capabilities:{rdt_a:($flags|test("(^| )rdt_a( |$)")),cat_l3:($flags|test("(^| )cat_l3( |$)")),cat_l2:($flags|test("(^| )cat_l2( |$)")),cqm_llc:($flags|test("(^| )cqm_llc( |$)")),mbm_total:($flags|test("(^| )cqm_mbm_total( |$)")),mbm_local:($flags|test("(^| )cqm_mbm_local( |$)")),mba:($flags|test("(^| )mba( |$)"))}}' > "$crq_dir/data.json" || return 1
    inv_strings_json "$crq_dir/notes.json" "resctrl is observed only; this collector never mounts or configures it." "NUMA distance and cache sharing are kernel-reported topology evidence." || return 1
    inv_strings_json "$crq_dir/errors.json" || return 1
    inv_emit cache-resource-qos "$crq_dir/data.json" "$crq_dir/notes.json" "$crq_dir/errors.json"
}
print_cache_resource_qos_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "Cache / NUMA / Resource QoS View"
    inv_view_kv "Cache entries" "$(jq -r '.data.cache_entries|length' "$f")"
    inv_view_kv "NUMA nodes" "$(jq -r '.data.numa_nodes|length' "$f")"
    inv_view_kv "resctrl supported" "$(jq -r '.data.resctrl.filesystem_supported' "$f")"
    inv_view_kv "resctrl mounted" "$(jq -r '.data.resctrl.mounted' "$f")"
    inv_view_kv "resctrl groups" "$(jq -r '(.data.resctrl.groups//[])|length' "$f")"
    inv_view_section "Resource-control capabilities"
    jq -r '.data.capabilities|to_entries[]|"\(.key)=\(.value)"' "$f" | while IFS= read -r line; do inv_view_item "$line"; done
    inv_view_section "Shared last-level cache entries"
    jq -r '.data.cache_entries as $c | ($c|map(.level)|max) as $m | $c | map(select(.level==$m)) | unique_by(.cache_id,.type,.shared_cpu_list)[]?|"L\(.level) \(.type) size=\(.size) shared=\(.shared_cpu_list) id=\(.cache_id // "n/a")"' "$f" | while IFS= read -r line; do inv_view_item "$line"; done
    inv_view_metadata "$f"
}

# ============================================================
# print_cache_resource_qos_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_cache_resource_qos_summary() { print_cache_resource_qos_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_cache_resource_qos_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_cache_resource_qos.summary-source.json
        collect_cache_resource_qos > "$main_summary_file" || return $?
        print_cache_resource_qos_view "$main_summary_file"
        return $?
    fi
    collect_cache_resource_qos
}
