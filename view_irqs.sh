...
# ============================================================
# view_irqs.sh
# Collects IRQ affinity/effective affinity, interrupt counters and irqbalance state.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_irqs.sh
#   ./view_irqs.sh --file collect_irqs.json
#   ./view_irqs.sh --help
#
# Output:
...
app_rc=0
app_compact=0
app_package_specs="jq:jq:required pgrep:procps:optional"
...
usage() {
    cat <<'EOF'
view_irqs.sh - standalone irqs inventory view

Usage:
  sudo ./view_irqs.sh
  ./view_irqs.sh --file collect_irqs.json
  sudo ./view_irqs.sh --json > collect_irqs.json

Default behavior:
  With no arguments, collect the irqs aspect live and
  render its colorized human-readable view.

View input:
  --file FILE       Render a previously saved collect_irqs.json envelope.
...
# ============================================================
# collect_irqs
# Collects configured/effective IRQ affinity plus irqbalance evidence.
#
# Version:
#   1.0.0
#
...
# ============================================================
collect_irqs() {
    cir_dir=$_inv_temp_dir/irqs
    cir_stream=$cir_dir/irqs.jsonl
    mkdir -p "$cir_dir" || return 1
    : > "$cir_stream"
    for cir_irq in /proc/irq/[0-9]*; do
        [ -d "$cir_irq" ] || continue
        jq -n --arg irq "${cir_irq##*/}" --arg smp_affinity "$(cat "$cir_irq/smp_affinity" 2>/dev/null)" --arg smp_affinity_list "$(cat "$cir_irq/smp_affinity_list" 2>/dev/null)" --arg effective_affinity "$(cat "$cir_irq/effective_affinity" 2>/dev/null)" --arg effective_affinity_list "$(cat "$cir_irq/effective_affinity_list" 2>/dev/null)" --arg node "$(cat "$cir_irq/node" 2>/dev/null)" --arg spurious "$(cat "$cir_irq/spurious" 2>/dev/null)" '{irq:($irq|tonumber),smp_affinity:$smp_affinity,smp_affinity_list:$smp_affinity_list,effective_affinity:$effective_affinity,effective_affinity_list:$effective_affinity_list,node:$node,spurious_raw:$spurious}' >> "$cir_stream" || return 1
    done
    jq -s '.' "$cir_stream" > "$cir_dir/irqs.json" || return 1
    inv_command_json "$cir_dir/irqbalance_service.json" 6 systemctl show irqbalance.service -p LoadState -p ActiveState -p SubState -p ExecStart || return 1
    inv_command_json "$cir_dir/irqbalance_process.json" 5 pgrep -a irqbalance || return 1
    inv_command_json "$cir_dir/irqbalance_config.json" 5 sh -c 'for f in /etc/default/irqbalance /etc/sysconfig/irqbalance /etc/irqbalance.env; do [ -r "$f" ] && { printf "### %s\n" "$f"; cat "$f"; }; done' || return 1
    jq -n --arg default_affinity "$(cat /proc/irq/default_smp_affinity 2>/dev/null)" --arg interrupts "$(cat /proc/interrupts 2>/dev/null)" --arg softirqs "$(cat /proc/softirqs 2>/dev/null)" --slurpfile irqs "$cir_dir/irqs.json" --slurpfile service "$cir_dir/irqbalance_service.json" --slurpfile process "$cir_dir/irqbalance_process.json" --slurpfile config "$cir_dir/irqbalance_config.json" '{default_smp_affinity:$default_affinity,irqs:$irqs[0],interrupts_raw:$interrupts,softirqs_raw:$softirqs,irqbalance:{service:$service[0],process:$process[0],configuration:$config[0]}}' > "$cir_dir/data.json" || return 1
    inv_strings_json "$cir_dir/notes.json" || return 1
    inv_strings_json "$cir_dir/errors.json" || return 1
    inv_emit irqs "$cir_dir/data.json" "$cir_dir/notes.json" "$cir_dir/errors.json"
}
...
print_irqs_view() {
    piv_file=$1
    inv_summary_validate_file "$piv_file" || return $?
    inv_view_header "$piv_file" "IRQ View"

    inv_view_section "IRQ overview"
    inv_view_kv "IRQ entries" "$(jq -r '.data.irqs|length' "$piv_file")"
    inv_view_kv "Default affinity" "$(jq -r '.data.default_smp_affinity // "unknown"' "$piv_file")"
    inv_view_kv "irqbalance service" "rc=$(jq -r '.data.irqbalance.service.rc // "unavailable"' "$piv_file")"
    inv_view_kv "irqbalance process" "$(jq -r '(.data.irqbalance.process.stdout // "") | if .=="" then "not observed" else . end' "$piv_file")"

    inv_view_section "Effective IRQ distribution"
    jq -r '.data.irqs
        | map({cpu:(if (.effective_affinity_list//"")=="" then "none" else .effective_affinity_list end)})
        | group_by(.cpu)
        | sort_by(if .[0].cpu=="none" then 999999 else (.[0].cpu|tonumber) end)[]
        | [(.[0].cpu),(length|tostring)]
        | @tsv' "$piv_file" |
    while IFS="$(printf '\t')" read -r piv_cpu piv_count; do
        inv_view_item "effective CPUs=$piv_cpu : $piv_count IRQ(s)"
    done

    inv_view_section "Latency-relevant device IRQs"
    jq -r '
        .data as $d
        | ($d.irqs | map({key:(.irq|tostring), value:(if (.effective_affinity_list//"")=="" then "none" else .effective_affinity_list end)}) | from_entries) as $aff
        | $d.interrupts_raw
        | split("\n")[]
        | select(test("nvme|enp|ahci|xhci|nouveau|snd_hda|rtw|vfio|AMD-Vi";"i"))
        | capture("^\\s*(?<irq>[0-9]+):(?<body>.*)$") as $m
        | ($m.body | sub("^.*\\s[0-9]+-(edge|fasteoi)\\s+";"") | gsub("^\\s+|\\s+$";"")) as $source
        | [$m.irq, ($aff[$m.irq] // "none"), $source]
        | @tsv' "$piv_file" |
    while IFS="$(printf '\t')" read -r piv_irq piv_eff piv_source; do
        inv_view_item "IRQ $piv_irq effective=$piv_eff source=$piv_source"
    done

    inv_view_metadata "$piv_file"
}

# ============================================================
# print_irqs_summary
# Presents IRQ affinity and irqbalance state.
#
# Version:
#   1.1.0
#
# Usage:
#   print_irqs_summary collectorEnvelope.json
#
# Returns:
#   0 on success
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_irqs_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_irqs.summary-source.json
        collect_irqs > "$main_summary_file" || return $?
        print_irqs_view "$main_summary_file"
        return $?
    fi
    collect_irqs
}
