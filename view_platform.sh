...
# ============================================================
# view_platform.sh
# Collects platform, OS, firmware identity, EFI state and Proxmox version.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_platform.sh
#   ./view_platform.sh --file collect_platform.json
#   ./view_platform.sh --help
#
# Output:
...
app_name="view_platform"
app_version=0.9.2
app_rc=0
app_compact=0
app_package_specs="jq:jq:required mokutil:mokutil:optional"
...
usage() {
    cat <<'EOF'
view_platform.sh - standalone platform inventory view

Usage:
  sudo ./view_platform.sh
  ./view_platform.sh --file collect_platform.json
  sudo ./view_platform.sh --json > collect_platform.json

Default behavior:
  With no arguments, collect the platform aspect live and
  render its colorized human-readable view.

View input:
  --file FILE       Render a previously saved collect_platform.json envelope.
...
# ============================================================
# collect_platform
# Collects OS, kernel, DMI, EFI, Secure Boot and PVE version data.
#
# Version:
#   1.0.0
#
...
# ============================================================
collect_platform() {
    cpl_dir=$_inv_temp_dir/platform
    mkdir -p "$cpl_dir" || return 1
    inv_file_map_json "$cpl_dir/dmi.json" "bios_vendor=/sys/class/dmi/id/bios_vendor" "bios_version=/sys/class/dmi/id/bios_version" "bios_date=/sys/class/dmi/id/bios_date" "board_vendor=/sys/class/dmi/id/board_vendor" "board_name=/sys/class/dmi/id/board_name" "board_version=/sys/class/dmi/id/board_version" "product_name=/sys/class/dmi/id/product_name" "product_version=/sys/class/dmi/id/product_version" "sys_vendor=/sys/class/dmi/id/sys_vendor" || return 1
    inv_command_json "$cpl_dir/pveversion.json" 12 pveversion -v || return 1
    inv_command_json "$cpl_dir/mokutil.json" 6 mokutil --sb-state || return 1
    inv_command_json "$cpl_dir/uname.json" 3 uname -a || return 1
    cpl_os_release=$(cat /etc/os-release 2>/dev/null)
    cpl_uptime=$(cat /proc/uptime 2>/dev/null)
    if [ -d /sys/firmware/efi ]; then cpl_efi=true; else cpl_efi=false; fi
    cpl_euid=$(id -u 2>/dev/null)
    jq -n --arg os_release "$cpl_os_release" --arg uptime "$cpl_uptime" --arg euid "$cpl_euid" --argjson efi "$cpl_efi" --slurpfile dmi "$cpl_dir/dmi.json" --slurpfile pve "$cpl_dir/pveversion.json" --slurpfile mok "$cpl_dir/mokutil.json" --slurpfile uname "$cpl_dir/uname.json" '{os_release_raw:$os_release,uptime_raw:$uptime,euid:($euid|tonumber? // $euid),efi_present:$efi,dmi:$dmi[0],pveversion:$pve[0],secure_boot_probe:$mok[0],uname:$uname[0]}' > "$cpl_dir/data.json" || return 1
    inv_strings_json "$cpl_dir/notes.json" "Firmware switches that are not exposed by the OS are intentionally not inferred." || return 1
    inv_strings_json "$cpl_dir/errors.json" || return 1
    inv_emit platform "$cpl_dir/data.json" "$cpl_dir/notes.json" "$cpl_dir/errors.json"
}
...
# ============================================================
# print_platform_view
# Presents a colorized platform/firmware view.
#
# Version:
#   1.0.0
#
# Usage:
#   print_platform_view collectorEnvelope.json
#
# Returns:
#   0 on success
...
print_platform_view() {
    ppv_file=$1
    inv_summary_validate_file "$ppv_file" || return $?
    ppv_os=$(jq -r 'first(.data.os_release_raw|split("\n")[]|select(startswith("PRETTY_NAME="))|sub("^PRETTY_NAME=\\\"?";"")|sub("\\\"$";"")) // "unknown"' "$ppv_file")
    ppv_proxmox=$(jq -r '(.data.pveversion.stdout // "" | split("\n")[0]) // "unknown"' "$ppv_file")
    ppv_kernel=$(jq -r '.data.uname.stdout // "unknown"' "$ppv_file")
    ppv_uptime=$(jq -r '.data.uptime_raw // "unknown"' "$ppv_file")
    inv_view_header "$ppv_file" "Platform / Firmware View"
    inv_view_section "Overview"
    inv_view_kv "Operating system" "$ppv_os"
    inv_view_kv "Proxmox" "$ppv_proxmox"
    inv_view_kv "Kernel" "$ppv_kernel"
    inv_view_kv "Raw uptime" "$ppv_uptime"
    inv_view_kv "Effective UID" "$(jq -r '.data.euid|tostring' "$ppv_file")"
    inv_view_section "System identity"
    inv_view_kv "System vendor" "$(jq -r '.data.dmi.sys_vendor // "unknown"' "$ppv_file")"
    inv_view_kv "Product" "$(jq -r '[.data.dmi.product_name,.data.dmi.product_version]|map(select(.!=null and .!=""))|join(" ")' "$ppv_file")"
    inv_view_kv "Board" "$(jq -r '[.data.dmi.board_vendor,.data.dmi.board_name,.data.dmi.board_version]|map(select(.!=null and .!=""))|join(" ")' "$ppv_file")"
    inv_view_kv "BIOS" "$(jq -r '[.data.dmi.bios_vendor,.data.dmi.bios_version,.data.dmi.bios_date]|map(select(.!=null and .!=""))|join(" / ")' "$ppv_file")"
    inv_view_section "Boot environment"
    inv_view_kv "EFI boot" "$(jq -r 'if .data.efi_present then "yes" else "no" end' "$ppv_file")"
    inv_view_kv "Secure Boot" "$(jq -r '.data.secure_boot_probe.stdout // "unavailable"' "$ppv_file")"
    inv_view_metadata "$ppv_file"
}

# ============================================================
# print_platform_summary
# Presents platform inventory in a concise human-readable form.
#
# Version:
#   1.1.0
#
# Usage:
#   print_platform_summary collectorEnvelope.json
#
# Returns:
#   0 on success
#   2 when the input envelope is invalid
# ============================================================
print_platform_summary() {
    print_platform_view "$1"
}


# ============================================================
# main
# Dispatches the standalone live/file view and raw-JSON modes.
#
# Version:
#   1.0.0
#
# Returns:
#   Collector return code.
# ============================================================
main() {
    case "$_inv_action" in
        help) usage; return 0 ;;
        ...
        summary_file|view_file)
            ...
            print_platform_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_platform.summary-source.json
        collect_platform > "$main_summary_file" || return $?
        print_platform_view "$main_summary_file"
        return $?
    fi
    collect_platform
}
