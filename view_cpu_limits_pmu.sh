...
# Collects CPU thermal-throttling, thermal-pressure, frequency-residency and performance-monitoring-unit capability evidence.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_cpu_limits_pmu.sh
#   ./view_cpu_limits_pmu.sh --file collect_runtime_pressure.json
#   ./view_cpu_limits_pmu.sh --help
#
# Output:
...
usage() {
    cat <<'EOF'
view_cpu_limits_pmu.sh - collect CPU performance-limit, thermal-throttle and PMU capability evidence

Usage:
  ./view_cpu_limits_pmu.sh [--compact|--summary|--view]
  ./view_cpu_limits_pmu.sh --help
  ./view_cpu_limits_pmu.sh --version
...
collect_cpu_limits_pmu() {
    clp_dir=$_inv_temp_dir/cpu-limits-pmu; mkdir -p "$clp_dir" || return 1
    clp_cpu_stream=$clp_dir/cpus.jsonl; : > "$clp_cpu_stream" || return 1
    for clp_cpu_dir in /sys/devices/system/cpu/cpu[0-9]*; do
        [ -d "$clp_cpu_dir" ] || continue
        clp_cpu=${clp_cpu_dir##*cpu}
        clp_tp=$(cat "$clp_cpu_dir/thermal_pressure" 2>/dev/null)
        clp_cur=$(cat "$clp_cpu_dir/cpufreq/scaling_cur_freq" 2>/dev/null)
        clp_hwcur=$(cat "$clp_cpu_dir/cpufreq/cpuinfo_cur_freq" 2>/dev/null)
        clp_min=$(cat "$clp_cpu_dir/cpufreq/scaling_min_freq" 2>/dev/null)
        clp_max=$(cat "$clp_cpu_dir/cpufreq/scaling_max_freq" 2>/dev/null)
        clp_bios=$(cat "$clp_cpu_dir/cpufreq/bios_limit" 2>/dev/null)
        clp_tis=$(cat "$clp_cpu_dir/cpufreq/stats/time_in_state" 2>/dev/null)
        clp_core_tc=$(cat "$clp_cpu_dir/thermal_throttle/core_throttle_count" 2>/dev/null)
        clp_pkg_tc=$(cat "$clp_cpu_dir/thermal_throttle/package_throttle_count" 2>/dev/null)
        clp_core_total=$(cat "$clp_cpu_dir/thermal_throttle/core_throttle_total_time_ms" 2>/dev/null)
        clp_pkg_total=$(cat "$clp_cpu_dir/thermal_throttle/package_throttle_total_time_ms" 2>/dev/null)
        jq -n --arg cpu "$clp_cpu" --arg tp "$clp_tp" --arg cur "$clp_cur" --arg hwcur "$clp_hwcur" --arg minf "$clp_min" --arg maxf "$clp_max" --arg bios "$clp_bios" --arg tis "$clp_tis" --arg ctc "$clp_core_tc" --arg ptc "$clp_pkg_tc" --arg ctm "$clp_core_total" --arg ptm "$clp_pkg_total" '{cpu:($cpu|tonumber),thermal_pressure:(try ($tp|tonumber) catch null),frequency_khz:{scaling_cur:(try ($cur|tonumber) catch null),cpuinfo_cur:(try ($hwcur|tonumber) catch null),scaling_min:(try ($minf|tonumber) catch null),scaling_max:(try ($maxf|tonumber) catch null),bios_limit:(try ($bios|tonumber) catch null)},time_in_state_raw:(if $tis=="" then null else $tis end),time_in_state:[($tis|split("\n")[]|select(test("^[0-9]+[[:space:]]+[0-9]+$"))|capture("^(?<frequency_khz>[0-9]+)[[:space:]]+(?<ticks>[0-9]+)$")|{frequency_khz:(.frequency_khz|tonumber),ticks:(.ticks|tonumber)})],thermal_throttle:{core_count:(try ($ctc|tonumber) catch null),package_count:(try ($ptc|tonumber) catch null),core_total_time_ms:(try ($ctm|tonumber) catch null),package_total_time_ms:(try ($ptm|tonumber) catch null)}} | .time_in_state_total_ticks=([.time_in_state[].ticks]|add // 0)' >> "$clp_cpu_stream" || return 1
    done
    jq -s 'sort_by(.cpu)' "$clp_cpu_stream" > "$clp_dir/cpus.json" || return 1
    clp_pmu_stream=$clp_dir/pmu.jsonl; : > "$clp_pmu_stream" || return 1
    for clp_src in /sys/bus/event_source/devices/*; do
        [ -d "$clp_src" ] || continue
        clp_name=${clp_src##*/}; clp_type=$(cat "$clp_src/type" 2>/dev/null); clp_cpus=$(cat "$clp_src/cpumask" 2>/dev/null)
        clp_events=$(find "$clp_src/events" -maxdepth 1 -type f -printf '%f\n' 2>/dev/null | sort | paste -sd ',' -)
        clp_formats=$(find "$clp_src/format" -maxdepth 1 -type f -printf '%f\n' 2>/dev/null | sort | paste -sd ',' -)
        jq -n --arg name "$clp_name" --arg type "$clp_type" --arg cpus "$clp_cpus" --arg events "$clp_events" --arg formats "$clp_formats" '{name:$name,type:(try ($type|tonumber) catch null),cpumask:(if $cpus=="" then null else $cpus end),event_names:($events|split(",")|map(select(length>0))),format_names:($formats|split(",")|map(select(length>0)))}' >> "$clp_pmu_stream" || return 1
    done
    jq -s 'sort_by(.name)' "$clp_pmu_stream" > "$clp_dir/pmu.json" || return 1
    clp_paranoid=$(cat /proc/sys/kernel/perf_event_paranoid 2>/dev/null)
    clp_nmi=$(cat /proc/sys/kernel/nmi_watchdog 2>/dev/null)
    jq -n --arg paranoid "$clp_paranoid" --arg nmi "$clp_nmi" --slurpfile cpus "$clp_dir/cpus.json" --slurpfile pmu "$clp_dir/pmu.json" '{cpus:$cpus[0],pmu_sources:$pmu[0],perf_event_paranoid:(try ($paranoid|tonumber) catch null),nmi_watchdog:(try ($nmi|tonumber) catch null)}' > "$clp_dir/data.json" || return 1
    inv_strings_json "$clp_dir/notes.json" "Thermal-throttle counters are cumulative when the platform exposes them." "PMU event names describe capability only; no perf event is started." || return 1
    inv_strings_json "$clp_dir/errors.json" || return 1
    inv_emit cpu-limits-pmu "$clp_dir/data.json" "$clp_dir/notes.json" "$clp_dir/errors.json"
}
print_cpu_limits_pmu_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "CPU Performance Limits / PMU View"
    inv_view_kv "CPUs" "$(jq -r '.data.cpus|length' "$f")"
    inv_view_kv "PMU sources" "$(jq -r '.data.pmu_sources|length' "$f")"
    inv_view_kv "perf_event_paranoid" "$(jq -r '.data.perf_event_paranoid // "unknown"' "$f")"
    inv_view_section "Thermal/throttle evidence"
    jq -r '.data.cpus[]|select(.thermal_pressure!=null or .thermal_throttle.core_count!=null or .thermal_throttle.package_count!=null)|"CPU \(.cpu) pressure=\(.thermal_pressure // "n/a") core-throttle=\(.thermal_throttle.core_count // "n/a") package-throttle=\(.thermal_throttle.package_count // "n/a")"' "$f" | while IFS= read -r line; do inv_view_item "$line"; done
    inv_view_section "PMU sources"
    jq -r '.data.pmu_sources[]?|"\(.name) type=\(.type // "n/a") events=\(.event_names|length)"' "$f" | head -30 | while IFS= read -r line; do inv_view_item "$line"; done
    inv_view_metadata "$f"
}

# ============================================================
# print_cpu_limits_pmu_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_cpu_limits_pmu_summary() { print_cpu_limits_pmu_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_cpu_limits_pmu_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_cpu_limits_pmu.summary-source.json
        collect_cpu_limits_pmu > "$main_summary_file" || return $?
        print_cpu_limits_pmu_view "$main_summary_file"
        return $?
    fi
    collect_cpu_limits_pmu
}
