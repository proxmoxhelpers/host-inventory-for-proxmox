...
# Passively samples multi-second CPU, PSI, IRQ/softirq, IPI, storage, network, QEMU and paging activity without synthetic load.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_latency_sample.sh
#   ./view_latency_sample.sh --file collect_runtime_pressure.json
#   ./view_latency_sample.sh --help
#
# Output:
...
app_name="view_latency_sample"
app_version=0.9.2
app_rc=0
app_compact=0
app_duration=${HFIP_LATENCY_SAMPLE_DURATION:-5}
app_interval=${HFIP_LATENCY_SAMPLE_INTERVAL:-1}
app_package_specs="jq:jq:required turbostat:linux-cpupower:optional"
...
inv_parse_options() {
    ...
    while [ "$#" -gt 0 ]; do
        ...
        case "$ipo_arg" in
            ...
            --interval)
                shift
                [ "$#" -gt 0 ] || { inv_message error "ERROR: --interval requires a positive integer."; return 2; }
                case "$1" in *[!0-9]*|""|0) inv_message error "ERROR: --interval requires a positive integer."; return 2 ;; esac
                app_interval=$1
                ;;
            --duration)
                shift
                [ "$#" -gt 0 ] || { inv_message error "ERROR: --duration requires a positive integer."; return 2; }
                case "$1" in *[!0-9]*|""|0) inv_message error "ERROR: --duration requires a positive integer."; return 2 ;; esac
                app_duration=$1
                ;;
...
usage() {
    cat <<'EOF'
view_latency_sample.sh - standalone passive multi-second latency / pressure sample view

Usage:
  sudo ./view_latency_sample.sh
  ./view_latency_sample.sh --file collect_latency_sample.json
  sudo ./view_latency_sample.sh --json > collect_latency_sample.json
...
cls_snapshot() {
    cls_tag=$1
    cp /proc/stat "$cls_dir/$cls_tag.stat" 2>/dev/null || : > "$cls_dir/$cls_tag.stat"
    cp /proc/vmstat "$cls_dir/$cls_tag.vmstat" 2>/dev/null || : > "$cls_dir/$cls_tag.vmstat"
    cp /proc/softirqs "$cls_dir/$cls_tag.softirqs" 2>/dev/null || : > "$cls_dir/$cls_tag.softirqs"
    cp /proc/interrupts "$cls_dir/$cls_tag.interrupts" 2>/dev/null || : > "$cls_dir/$cls_tag.interrupts"
    cp /proc/schedstat "$cls_dir/$cls_tag.schedstat" 2>/dev/null || : > "$cls_dir/$cls_tag.schedstat"

    cls_block="$cls_dir/$cls_tag.block"
    : > "$cls_block"
    for cls_bd in /sys/block/*; do
        [ -r "$cls_bd/stat" ] || continue
        cls_bn=${cls_bd##*/}
        printf '%s %s\n' "$cls_bn" "$(cat "$cls_bd/stat" 2>/dev/null)" >> "$cls_block"
    done

    awk -F'[: ]+' 'NR>2 && $1!="" {
      print $1 "\t" $3 "\t" $4 "\t" $5 "\t" $6 "\t" $11 "\t" $12 "\t" $13 "\t" $14
    }' /proc/net/dev 2>/dev/null > "$cls_dir/$cls_tag.net"

    cls_q="$cls_dir/$cls_tag.qemu"
    : > "$cls_q"
    for cls_pf in /run/qemu-server/[0-9]*.pid; do
        [ -f "$cls_pf" ] || continue
        cls_vmid=${cls_pf##*/}; cls_vmid=${cls_vmid%.pid}
        cls_pid=$(cat "$cls_pf" 2>/dev/null)
        case "$cls_pid" in *[!0-9]*|"") continue ;; esac
        [ -r "/proc/$cls_pid/stat" ] || continue
        cls_ut=$(awk '{print $14}' "/proc/$cls_pid/stat" 2>/dev/null)
        cls_st=$(awk '{print $15}' "/proc/$cls_pid/stat" 2>/dev/null)
        set -- $(cat "/proc/$cls_pid/schedstat" 2>/dev/null)
        cls_rt=${1:-0}; cls_wait=${2:-0}; cls_slices=${3:-0}
        cls_mig=$(awk '$1=="se.nr_migrations"{print $3}' "/proc/$cls_pid/sched" 2>/dev/null)
        printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$cls_vmid" "$cls_pid" \
          "${cls_ut:-0}" "${cls_st:-0}" "${cls_rt:-0}" "${cls_wait:-0}" "${cls_slices:-0}" "${cls_mig:-0}" >> "$cls_q"
    done

    cls_freq="$cls_dir/$cls_tag.freq"
    : > "$cls_freq"
    for cls_cp in /sys/devices/system/cpu/cpu[0-9]*; do
        [ -d "$cls_cp" ] || continue
        cls_cpu=${cls_cp##*/}; cls_cpu=${cls_cpu#cpu}
        cls_f=$(cat "$cls_cp/cpufreq/scaling_cur_freq" 2>/dev/null)
        [ -n "$cls_f" ] || continue
        printf '%s\t%s\n' "$cls_cpu" "$cls_f" >> "$cls_freq"
    done
}
...
cls_parse_deltas() {
    awk 'NR==FNR {
      if ($1 ~ /^cpu[0-9]+$/) {
        cpu=$1; for(i=2;i<=11;i++) a[cpu,i]=$i
      }
      next
    }
    $1 ~ /^cpu[0-9]+$/ {
      cpu=$1; total=0; idle=0;
      for(i=2;i<=11;i++){d=$i-a[cpu,i]; if(d<0)d=0; vals[i]=d; total+=d}
      idle=vals[5]+vals[6];
      busy=total-idle;
      printf "%s\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",
        substr(cpu,4),vals[2]+vals[3],vals[4],vals[5],vals[6],vals[7],vals[8],busy,total
    }' "$cls_dir/t0.stat" "$cls_dir/t1.stat" > "$cls_dir/percpu.tsv"

    jq -Rn '[inputs | split("\t") |
      {cpu:(.[0]|tonumber),user_nice_ticks:(.[1]|tonumber),system_ticks:(.[2]|tonumber),
       idle_ticks:(.[3]|tonumber),iowait_ticks:(.[4]|tonumber),irq_ticks:(.[5]|tonumber),
       softirq_ticks:(.[6]|tonumber),busy_ticks:(.[7]|tonumber),total_ticks:(.[8]|tonumber),
       busy_percent:(if (.[8]|tonumber)>0 then ((.[7]|tonumber)*100/(.[8]|tonumber)) else 0 end)}]' \
      < "$cls_dir/percpu.tsv" > "$cls_dir/percpu.json" || return 1

    awk 'NR==FNR {a[$1]=$2; next}
      $1 ~ /^(ctxt|processes)$/ {print $1 "\t" ($2-(a[$1]+0))}' \
      "$cls_dir/t0.stat" "$cls_dir/t1.stat" > "$cls_dir/statdelta.tsv"
    jq -Rn 'reduce (inputs|split("\t")) as $r ({}; .[$r[0]]=($r[1]|tonumber))' \
      < "$cls_dir/statdelta.tsv" > "$cls_dir/statdelta.json" || return 1

    awk 'NR==FNR {a[$1]=$2; next}
      $1 ~ /^(pgfault|pgmajfault|pswpin|pswpout|pgscan_|pgsteal_|compact_|allocstall|pgmigrate_)/ {
        print $1 "\t" ($2-(a[$1]+0))
      }' "$cls_dir/t0.vmstat" "$cls_dir/t1.vmstat" > "$cls_dir/vmdelta.tsv"
    jq -Rn 'reduce (inputs|split("\t")) as $r ({}; .[$r[0]]=($r[1]|tonumber))' \
      < "$cls_dir/vmdelta.tsv" > "$cls_dir/vmdelta.json" || return 1

    awk 'NR==FNR {
      if ($1 ~ /:$/) {n=$1; sub(/:$/,"",n); for(i=2;i<=NF;i++) a[n,i]=$i}
      next
    }
    $1 ~ /:$/ {
      n=$1; sub(/:$/,"",n); total=0; csv="";
      for(i=2;i<=NF;i++){d=$i-(a[n,i]+0); total+=d; csv=csv (i==2?"":",") d}
      print n "\t" total "\t" csv
    }' "$cls_dir/t0.softirqs" "$cls_dir/t1.softirqs" > "$cls_dir/softirq.tsv"
    jq -Rn '[inputs|split("\t")|
      {name:.[0],total_delta:(.[1]|tonumber),per_cpu_delta:(.[2]|split(",")|map(tonumber))}]' \
      < "$cls_dir/softirq.tsv" > "$cls_dir/softirq.json" || return 1

    awk 'NR==1 {next}
      NR==FNR {
        if ($1 ~ /^(NMI|LOC|SPU|PMI|IWI|RTR|RES|CAL|TLB|TRM|THR|DFR|MCE|MCP|HYP|ERR|MIS|PIN|NPI|PIW):$/) {
          n=$1; sub(/:$/,"",n); for(i=2;i<=NF;i++){if($i !~ /^[0-9]+$/)break; a[n,i]=$i}
        } next
      }
      $1 ~ /^(NMI|LOC|SPU|PMI|IWI|RTR|RES|CAL|TLB|TRM|THR|DFR|MCE|MCP|HYP|ERR|MIS|PIN|NPI|PIW):$/ {
        n=$1; sub(/:$/,"",n); total=0; csv="";
        for(i=2;i<=NF;i++){if($i !~ /^[0-9]+$/)break; d=$i-(a[n,i]+0); total+=d; csv=csv (i==2?"":",") d}
        print n "\t" total "\t" csv
      }' "$cls_dir/t0.interrupts" "$cls_dir/t1.interrupts" > "$cls_dir/ipi.tsv"
    jq -Rn '[inputs|split("\t")|
      {name:.[0],total_delta:(.[1]|tonumber),per_cpu_delta:(.[2]|split(",")|map(tonumber))}]' \
      < "$cls_dir/ipi.tsv" > "$cls_dir/ipi.json" || return 1

    awk 'NR==FNR {for(i=2;i<=NF;i++)a[$1,i]=$i; next}
      {
        name=$1;
        print name "\t" ($2-(a[name,2]+0)) "\t" ($4-(a[name,4]+0)) "\t" \
          ($6-(a[name,6]+0)) "\t" ($8-(a[name,8]+0)) "\t" ($11-(a[name,11]+0))
      }' "$cls_dir/t0.block" "$cls_dir/t1.block" > "$cls_dir/block.tsv"
    jq -Rn '[inputs|split("\t")|
      {device:.[0],reads_completed_delta:(.[1]|tonumber),sectors_read_delta:(.[2]|tonumber),
       writes_completed_delta:(.[3]|tonumber),sectors_written_delta:(.[4]|tonumber),
       io_ticks_delta:(.[5]|tonumber)}]' < "$cls_dir/block.tsv" > "$cls_dir/block.json" || return 1

    awk -F'\t' 'NR==FNR {for(i=2;i<=9;i++)a[$1,i]=$i; next}
      {
        name=$1;
        printf "%s",name;
        for(i=2;i<=9;i++) printf "\t%d",($i-(a[name,i]+0));
        printf "\n"
      }' "$cls_dir/t0.net" "$cls_dir/t1.net" > "$cls_dir/net.tsv"
    jq -Rn '[inputs|split("\t")|
      {interface:.[0],rx_bytes_delta:(.[1]|tonumber),rx_packets_delta:(.[2]|tonumber),
       rx_errors_delta:(.[3]|tonumber),rx_drops_delta:(.[4]|tonumber),
       tx_bytes_delta:(.[5]|tonumber),tx_packets_delta:(.[6]|tonumber),
       tx_errors_delta:(.[7]|tonumber),tx_drops_delta:(.[8]|tonumber)}]' \
      < "$cls_dir/net.tsv" > "$cls_dir/net.json" || return 1

    awk -F'\t' 'NR==FNR {for(i=3;i<=8;i++)a[$1,i]=$i; next}
      {
        vm=$1;
        print vm "\t" $2 "\t" ($3-(a[vm,3]+0)) "\t" ($4-(a[vm,4]+0)) "\t" \
          ($5-(a[vm,5]+0)) "\t" ($6-(a[vm,6]+0)) "\t" ($7-(a[vm,7]+0)) "\t" ($8-(a[vm,8]+0))
      }' "$cls_dir/t0.qemu" "$cls_dir/t1.qemu" > "$cls_dir/qemu.tsv"
    jq -Rn '[inputs|split("\t")|
      {vmid:.[0],pid:(.[1]|tonumber),utime_ticks_delta:(.[2]|tonumber),
       stime_ticks_delta:(.[3]|tonumber),sched_runtime_ns_delta:(.[4]|tonumber),
       sched_wait_ns_delta:(.[5]|tonumber),timeslices_delta:(.[6]|tonumber),
       migrations_delta:(.[7]|tonumber)}]' < "$cls_dir/qemu.tsv" > "$cls_dir/qemu.json" || return 1

    awk -F'\t' 'NR==FNR {a[$1]=$2; next}
      {print $1 "\t" $2 "\t" ($2-(a[$1]+0))}' "$cls_dir/t0.freq" "$cls_dir/t1.freq" \
      > "$cls_dir/freq.tsv"
    jq -Rn '[inputs|split("\t")|
      {cpu:(.[0]|tonumber),end_khz:(.[1]|tonumber),delta_khz:(.[2]|tonumber)}]' \
      < "$cls_dir/freq.tsv" > "$cls_dir/freq.json" || return 1
}

collect_latency_sample() {
    cls_dir=$_inv_temp_dir/latency-sample
    mkdir -p "$cls_dir" || return 1
    cls_start=$(inv_now_iso)
    cls_pressure="$cls_dir/pressure.jsonl"; : > "$cls_pressure" || return 1

    cls_snapshot t0
    cls_elapsed=0
    while [ "$cls_elapsed" -lt "$app_duration" ]; do
        cls_cpu=$(cat /proc/pressure/cpu 2>/dev/null)
        cls_mem=$(cat /proc/pressure/memory 2>/dev/null)
        cls_io=$(cat /proc/pressure/io 2>/dev/null)
        cls_load=$(cat /proc/loadavg 2>/dev/null)
        jq -n --arg elapsed "$cls_elapsed" --arg cpu "$cls_cpu" --arg memory "$cls_mem" \
          --arg io "$cls_io" --arg load "$cls_load" \
          '{elapsed_seconds:($elapsed|tonumber),cpu_psi:$cpu,memory_psi:$memory,io_psi:$io,loadavg:$load}' \
          >> "$cls_pressure" || return 1
        cls_remaining=$(( app_duration - cls_elapsed ))
        cls_sleep=$app_interval
        [ "$cls_sleep" -le "$cls_remaining" ] || cls_sleep=$cls_remaining
        [ "$cls_sleep" -gt 0 ] || break
        sleep "$cls_sleep"
        cls_elapsed=$(( cls_elapsed + cls_sleep ))
    done
    cls_cpu=$(cat /proc/pressure/cpu 2>/dev/null); cls_mem=$(cat /proc/pressure/memory 2>/dev/null)
    cls_io=$(cat /proc/pressure/io 2>/dev/null); cls_load=$(cat /proc/loadavg 2>/dev/null)
    jq -n --arg elapsed "$cls_elapsed" --arg cpu "$cls_cpu" --arg memory "$cls_mem" \
      --arg io "$cls_io" --arg load "$cls_load" \
      '{elapsed_seconds:($elapsed|tonumber),cpu_psi:$cpu,memory_psi:$memory,io_psi:$io,loadavg:$load}' \
      >> "$cls_pressure" || return 1
    ...
    cls_parse_deltas || return 1
    jq -s '.' "$cls_pressure" > "$cls_dir/pressure.json" || return 1
    inv_command_json "$cls_dir/turbostat_smi.json" 8 turbostat --quiet --show SMI --interval 1 --num_iterations 1 || return 1

    jq -n --arg start "$cls_start" --arg sample_end "$cls_end" --arg duration "$app_duration" \
      --arg interval "$app_interval" --arg sched0 "$(cat "$cls_dir/t0.schedstat" 2>/dev/null)" \
      --arg sched1 "$(cat "$cls_dir/t1.schedstat" 2>/dev/null)" \
      --slurpfile pressure "$cls_dir/pressure.json" --slurpfile cpu "$cls_dir/percpu.json" \
      --slurpfile stat "$cls_dir/statdelta.json" --slurpfile vm "$cls_dir/vmdelta.json" \
      --slurpfile soft "$cls_dir/softirq.json" --slurpfile ipi "$cls_dir/ipi.json" \
      --slurpfile block "$cls_dir/block.json" --slurpfile net "$cls_dir/net.json" \
      --slurpfile qemu "$cls_dir/qemu.json" --slurpfile freq "$cls_dir/freq.json" \
      --slurpfile smi "$cls_dir/turbostat_smi.json" '
      {sample:{start:$start,end:$sample_end,duration_seconds:($duration|tonumber),
         interval_seconds:($interval|tonumber),sample_count:($pressure[0]|length)},
       pressure_samples:$pressure[0],system_deltas:$stat[0],vmstat_deltas:$vm[0],
       per_cpu:$cpu[0],frequency_end_and_delta:$freq[0],
       softirq_deltas:$soft[0],ipi_deltas:$ipi[0],
       block_deltas:$block[0],network_deltas:$net[0],qemu_deltas:$qemu[0],
       schedstat:{t0:$sched0,t1:$sched1},turbostat_smi:$smi[0]}' > "$cls_dir/data.json" || return 1

    inv_strings_json "$cls_dir/notes.json" \
      "This is a passive multi-second sample and generates no synthetic workload." \
      "Deltas and observed maxima describe only this collection window." \
      "turbostat SMI evidence is optional and adds its own short read-only sample when turbostat is available." || return 1
    inv_strings_json "$cls_dir/errors.json" || return 1
    inv_emit latency-sample "$cls_dir/data.json" "$cls_dir/notes.json" "$cls_dir/errors.json"
}

print_latency_sample_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "Passive Multi-second Latency / Pressure Sample View"
    inv_view_kv "Duration" "$(jq -r '.data.sample.duration_seconds' "$f") seconds"
    inv_view_kv "Interval" "$(jq -r '.data.sample.interval_seconds' "$f") seconds"
    inv_view_kv "Samples" "$(jq -r '.data.sample.sample_count' "$f")"
    inv_view_section "Scheduler / paging deltas"
    inv_view_kv "Context switches" "$(jq -r '.data.system_deltas.ctxt // 0' "$f")"
    inv_view_kv "Processes created" "$(jq -r '.data.system_deltas.processes // 0' "$f")"
    inv_view_kv "Major faults" "$(jq -r '.data.vmstat_deltas.pgmajfault // 0' "$f")"
    inv_view_kv "Swap in/out" "$(jq -r '"\(.data.vmstat_deltas.pswpin // 0) / \(.data.vmstat_deltas.pswpout // 0)"' "$f")"
    inv_view_section "Highest observed activity deltas"
    jq -r '.data.softirq_deltas|sort_by(.total_delta)|reverse|.[0:6][]?|
      "softirq \(.name)=\(.total_delta)"' "$f" | while IFS= read -r line; do inv_view_item "$line"; done
    jq -r '.data.block_deltas|sort_by((.reads_completed_delta+.writes_completed_delta))|reverse|.[0:6][]?|
      "block \(.device) read=\(.reads_completed_delta) write=\(.writes_completed_delta)"' "$f" |
      while IFS= read -r line; do inv_view_item "$line"; done
    jq -r '.data.qemu_deltas[]?|
      "VM \(.vmid) runtime-ns=\(.sched_runtime_ns_delta) wait-ns=\(.sched_wait_ns_delta) migrations=\(.migrations_delta)"' "$f" |
      while IFS= read -r line; do inv_view_item "$line"; done
    inv_view_metadata "$f"
}

# ============================================================
# print_latency_sample_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_latency_sample_summary() { print_latency_sample_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_latency_sample_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_latency_sample.summary-source.json
        collect_latency_sample > "$main_summary_file" || return $?
        print_latency_sample_view "$main_summary_file"
        return $?
    fi
    collect_latency_sample
}
