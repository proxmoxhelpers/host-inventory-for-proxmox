...
# ============================================================
# view_pm_qos.sh
# Collects CPU and device PM-QoS latency constraints, runtime-power state and cpu_dma_latency holder evidence.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_pm_qos.sh
#   ./view_pm_qos.sh --file collect_runtime_pressure.json
#   ./view_pm_qos.sh --help
#
# Output:
...
usage() {
    cat <<'EOF'
view_pm_qos.sh - standalone pm-qos / runtime power latency view

Usage:
  sudo ./view_pm_qos.sh
  ./view_pm_qos.sh --file collect_pm_qos.json
  sudo ./view_pm_qos.sh --json > collect_pm_qos.json
...
collect_pm_qos() {
    cpq_dir=$_inv_temp_dir/pm-qos
    mkdir -p "$cpq_dir" || return 1

    cpq_cpu="$cpq_dir/cpu.jsonl"; : > "$cpq_cpu" || return 1
    for cpq_path in /sys/devices/system/cpu/cpu[0-9]*/power/pm_qos_resume_latency_us; do
        [ -e "$cpq_path" ] || continue
        cpq_cpu_name=${cpq_path%/power/pm_qos_resume_latency_us}; cpq_cpu_name=${cpq_cpu_name##*/}
        cpq_cpu_id=${cpq_cpu_name#cpu}
        cpq_val=$(cat "$cpq_path" 2>/dev/null)
        jq -n --arg cpu "$cpq_cpu_id" --arg value "$cpq_val" \
          '{cpu:($cpu|tonumber),present:true,value_us:(try ($value|tonumber) catch null)}' >> "$cpq_cpu" || return 1
    done
    jq -s '.' "$cpq_cpu" > "$cpq_dir/cpu.json" || return 1

    cpq_devices="$cpq_dir/devices.jsonl"; : > "$cpq_devices" || return 1
    for cpq_bus in pci usb; do
        for cpq_dev in /sys/bus/"$cpq_bus"/devices/*; do
            [ -d "$cpq_dev" ] || continue
            cpq_name=${cpq_dev##*/}
            cpq_control=$(cat "$cpq_dev/power/control" 2>/dev/null)
            cpq_status=$(cat "$cpq_dev/power/runtime_status" 2>/dev/null)
            cpq_auto=$(cat "$cpq_dev/power/autosuspend_delay_ms" 2>/dev/null)
            cpq_wakeup=$(cat "$cpq_dev/power/wakeup" 2>/dev/null)
            cpq_resume=$(cat "$cpq_dev/power/pm_qos_resume_latency_us" 2>/dev/null)
            cpq_tol=$(cat "$cpq_dev/power/pm_qos_latency_tolerance_us" 2>/dev/null)
            cpq_active=$(cat "$cpq_dev/power/runtime_active_time" 2>/dev/null)
            cpq_suspended=$(cat "$cpq_dev/power/runtime_suspended_time" 2>/dev/null)
            jq -n --arg bus "$cpq_bus" --arg device "$cpq_name" --arg control "$cpq_control" \
              --arg status "$cpq_status" --arg autosuspend "$cpq_auto" --arg wakeup "$cpq_wakeup" \
              --arg resume "$cpq_resume" --arg tolerance "$cpq_tol" --arg active "$cpq_active" \
              --arg suspended "$cpq_suspended" '
              {bus:$bus,device:$device,
               control:($control|select(length>0)),runtime_status:($status|select(length>0)),
               autosuspend_delay_ms:(try ($autosuspend|tonumber) catch null),
               wakeup:($wakeup|select(length>0)),
               pm_qos_resume_latency_us:(try ($resume|tonumber) catch null),
               pm_qos_latency_tolerance_us:(try ($tolerance|tonumber) catch null),
               runtime_active_time_ms:(try ($active|tonumber) catch null),
               runtime_suspended_time_ms:(try ($suspended|tonumber) catch null)}' >> "$cpq_devices" || return 1
        done
    done
    jq -s '.' "$cpq_devices" > "$cpq_dir/devices.json" || return 1

    cpq_holders="$cpq_dir/holders.jsonl"; : > "$cpq_holders" || return 1
    if [ -e /dev/cpu_dma_latency ]; then
        for cpq_fd in /proc/[0-9]*/fd/[0-9]*; do
            [ -L "$cpq_fd" ] || continue
            cpq_target=$(readlink "$cpq_fd" 2>/dev/null) || continue
            [ "$cpq_target" = "/dev/cpu_dma_latency" ] || continue
            cpq_pid=${cpq_fd#/proc/}; cpq_pid=${cpq_pid%%/*}
            cpq_fdno=${cpq_fd##*/}
            cpq_comm=$(cat "/proc/$cpq_pid/comm" 2>/dev/null)
            jq -n --arg pid "$cpq_pid" --arg fd "$cpq_fdno" --arg comm "$cpq_comm" \
              '{pid:($pid|tonumber),fd:($fd|tonumber),comm:$comm}' >> "$cpq_holders" || return 1
        done
    fi
    jq -s '.' "$cpq_holders" > "$cpq_dir/holders.json" || return 1
    cpq_dma=false; [ -e /dev/cpu_dma_latency ] && cpq_dma=true

    jq -n --argjson dma "$cpq_dma" --slurpfile cpu "$cpq_dir/cpu.json" \
      --slurpfile dev "$cpq_dir/devices.json" --slurpfile holders "$cpq_dir/holders.json" \
      '{cpu_dma_latency_present:$dma,cpu_dma_latency_holders:$holders[0],
        cpu_resume_latency:$cpu[0],devices:$dev[0]}' > "$cpq_dir/data.json" || return 1
    inv_strings_json "$cpq_dir/notes.json" \
      "PM-QoS values are read-only snapshots; the collector does not open or modify /dev/cpu_dma_latency." || return 1
    inv_strings_json "$cpq_dir/errors.json" || return 1
    inv_emit pm-qos "$cpq_dir/data.json" "$cpq_dir/notes.json" "$cpq_dir/errors.json"
}

print_pm_qos_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "PM-QoS / Runtime Power Latency View"
    inv_view_kv "/dev/cpu_dma_latency" "$(jq -r 'if .data.cpu_dma_latency_present then "present" else "absent" end' "$f")"
    inv_view_kv "Open holders" "$(jq -r '.data.cpu_dma_latency_holders|length' "$f")"
    inv_view_kv "CPU QoS interfaces" "$(jq -r '.data.cpu_resume_latency|length' "$f")"
    inv_view_section "Device runtime/QoS records"
    jq -r '.data.devices[]? | select(.pm_qos_resume_latency_us!=null or .runtime_status!=null) |
      [.bus,.device,(.runtime_status//"unknown"),(.pm_qos_resume_latency_us//"n/a")]|@tsv' "$f" |
      head -40 | while IFS="$(printf '\t')" read -r bus dev state lat; do
        inv_view_item "$bus/$dev runtime=$state resume-latency-us=$lat"
      done
    inv_view_metadata "$f"
}

# ============================================================
# print_pm_qos_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_pm_qos_summary() { print_pm_qos_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_pm_qos_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_pm_qos.summary-source.json
        collect_pm_qos > "$main_summary_file" || return $?
        print_pm_qos_view "$main_summary_file"
        return $?
    fi
    collect_pm_qos
}
