...
# Collects DRM connector, EDID, active-mode, refresh/VRR and display-link evidence visible to the host.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_display_timing.sh
#   ./view_display_timing.sh --file collect_runtime_pressure.json
#   ./view_display_timing.sh --help
#
# Output:
...
app_name="view_display_timing"
app_version=0.9.2
app_rc=0
app_compact=0
app_package_specs="jq:jq:required sha256sum:coreutils:optional parse-edid:read-edid:optional"
...
usage() {
    cat <<'EOF'
view_display_timing.sh - collect host-visible display connector, EDID, mode and variable-refresh evidence

Usage:
  ./view_display_timing.sh [--compact|--summary|--view]
  ./view_display_timing.sh --help
  ./view_display_timing.sh --version
...
collect_display_timing() {
    cdt_dir=$_inv_temp_dir/display-timing; mkdir -p "$cdt_dir" || return 1
    cdt_stream=$cdt_dir/connectors.jsonl; : > "$cdt_stream" || return 1
    for cdt_conn in /sys/class/drm/card*-*; do
        [ -d "$cdt_conn" ] || continue
        cdt_name=${cdt_conn##*/}; cdt_status=$(cat "$cdt_conn/status" 2>/dev/null); cdt_enabled=$(cat "$cdt_conn/enabled" 2>/dev/null); cdt_modes=$(cat "$cdt_conn/modes" 2>/dev/null); cdt_mode=$(cat "$cdt_conn/mode" 2>/dev/null); cdt_link=$(cat "$cdt_conn/link_status" 2>/dev/null); cdt_vrr=$(cat "$cdt_conn/vrr_capable" 2>/dev/null); cdt_bpc=$(cat "$cdt_conn/max_bpc" 2>/dev/null); cdt_dpms=$(cat "$cdt_conn/dpms" 2>/dev/null)
        cdt_edid_present=false; cdt_hash=; cdt_size=; cdt_parse=
        if [ -s "$cdt_conn/edid" ]; then
            cdt_edid_present=true; cdt_size=$(wc -c < "$cdt_conn/edid" 2>/dev/null | tr -d ' '); command -v sha256sum >/dev/null 2>&1 && cdt_hash=$(sha256sum "$cdt_conn/edid" 2>/dev/null | awk '{print $1}'); command -v parse-edid >/dev/null 2>&1 && cdt_parse=$(parse-edid < "$cdt_conn/edid" 2>/dev/null)
        fi
        cdt_dev=$(readlink -f "$cdt_conn/device" 2>/dev/null); cdt_bdf=${cdt_dev##*/}; case "$cdt_bdf" in ????\:??\:??.?) ;; *) cdt_bdf= ;; esac
        jq -n --arg name "$cdt_name" --arg status "$cdt_status" --arg enabled "$cdt_enabled" --arg modes "$cdt_modes" --arg mode "$cdt_mode" --arg link "$cdt_link" --arg vrr "$cdt_vrr" --arg bpc "$cdt_bpc" --arg dpms "$cdt_dpms" --arg bdf "$cdt_bdf" --argjson ep "$cdt_edid_present" --arg es "$cdt_size" --arg eh "$cdt_hash" --arg parsed "$cdt_parse" '{connector:$name,status:(if $status=="" then null else $status end),enabled:(if $enabled=="" then null else $enabled end),modes:($modes|split("\n")|map(select(length>0))),active_mode:(if $mode=="" then null else $mode end),link_status:(if $link=="" then null else $link end),vrr_capable:(if $vrr=="1" then true elif $vrr=="0" then false else null end),max_bpc:(if $bpc=="" then null else $bpc end),dpms:(if $dpms=="" then null else $dpms end),pci_bdf:(if $bdf=="" then null else $bdf end),edid:{present:$ep,size_bytes:(try ($es|tonumber) catch null),sha256:(if $eh=="" then null else $eh end),parse_edid_raw:(if $parsed=="" then null else $parsed end)}}' >> "$cdt_stream" || return 1
    done
    jq -s 'sort_by(.connector)' "$cdt_stream" > "$cdt_dir/connectors.json" || return 1
    cdt_vt=$(for f in /sys/class/vtconsole/vtcon*/bind; do [ -f "$f" ] || continue; printf '%s=%s\n' "${f%/bind}" "$(cat "$f" 2>/dev/null)"; done)
    jq -n --arg vt "$cdt_vt" --slurpfile conns "$cdt_dir/connectors.json" '{connectors:$conns[0],connected_count:([$conns[0][]|select(.status=="connected")]|length),vtconsole_bind_raw:$vt}' > "$cdt_dir/data.json" || return 1
    inv_strings_json "$cdt_dir/notes.json" "Display timing reflects host-visible DRM state only; passthrough guest display timing is outside this collector." || return 1
    inv_strings_json "$cdt_dir/errors.json" || return 1
    inv_emit display-timing "$cdt_dir/data.json" "$cdt_dir/notes.json" "$cdt_dir/errors.json"
}
print_display_timing_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "Display Timing / EDID View"
    inv_view_kv "Connectors" "$(jq -r '.data.connectors|length' "$f")"
    inv_view_kv "Connected" "$(jq -r '.data.connected_count' "$f")"
    inv_view_section "Connectors"
    jq -r '.data.connectors[]?|"\(.connector) status=\(.status // "n/a") active=\(.active_mode // "n/a") VRR=\(.vrr_capable // "unknown") BDF=\(.pci_bdf // "n/a") EDID=\(.edid.sha256 // "n/a")"' "$f" | while IFS= read -r line; do inv_view_item "$line"; done
    inv_view_metadata "$f"
}

# ============================================================
# print_display_timing_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_display_timing_summary() { print_display_timing_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_display_timing_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_display_timing.summary-source.json
        collect_display_timing > "$main_summary_file" || return $?
        print_display_timing_view "$main_summary_file"
        return $?
    fi
    collect_display_timing
}
