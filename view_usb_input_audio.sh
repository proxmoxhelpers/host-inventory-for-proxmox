...
# Collects USB, input-device and ALSA/audio topology including PCI-controller ancestry.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_usb_input_audio.sh
#   ./view_usb_input_audio.sh --file collect_usb_input_audio.json
#   ./view_usb_input_audio.sh --help
#
# Output:
...
app_name="view_usb_input_audio"
app_version=0.9.2
app_rc=0
app_compact=0
app_package_specs="jq:jq:required lsusb:usbutils:optional aplay:alsa-utils:optional"
...
usage() {
    cat <<'EOF'
view_usb_input_audio.sh - standalone USB / input / audio view

Usage:
  sudo ./view_usb_input_audio.sh
  ./view_usb_input_audio.sh --file collect_usb_input_audio.json
  sudo ./view_usb_input_audio.sh --json > collect_usb_input_audio.json
...
collect_usb_input_audio() {
    cua_dir=$_inv_temp_dir/usb-input-audio
    mkdir -p "$cua_dir" || return 1
    inv_command_json "$cua_dir/lsusb.json" 10 lsusb || return 1
    inv_command_json "$cua_dir/lsusb-tree.json" 10 lsusb -t || return 1
    inv_command_json "$cua_dir/aplay.json" 10 aplay -l || return 1
    cua_usb=$cua_dir/usb.jsonl; : > "$cua_usb"
    for cua_path in /sys/bus/usb/devices/*; do
        [ -d "$cua_path" ] || continue
        [ -r "$cua_path/idVendor" ] || continue
        cua_name=${cua_path##*/}; cua_vendor=$(cat "$cua_path/idVendor" 2>/dev/null); cua_product=$(cat "$cua_path/idProduct" 2>/dev/null); cua_man=$(cat "$cua_path/manufacturer" 2>/dev/null); cua_prod=$(cat "$cua_path/product" 2>/dev/null); cua_speed=$(cat "$cua_path/speed" 2>/dev/null); cua_bcd=$(cat "$cua_path/bcdUSB" 2>/dev/null); cua_ctrl=$(cat "$cua_path/power/control" 2>/dev/null); cua_auto=$(cat "$cua_path/power/autosuspend" 2>/dev/null); cua_wake=$(cat "$cua_path/power/wakeup" 2>/dev/null)
        cua_real=$(readlink -f "$cua_path" 2>/dev/null); cua_bdf=$(printf '%s\n' "$cua_real" | grep -oE '[0-9a-fA-F]{4}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}\.[0-7]' | tail -1)
        jq -n --arg sysfs "$cua_name" --arg vendor "$cua_vendor" --arg product "$cua_product" --arg manufacturer "$cua_man" --arg product_name "$cua_prod" --arg speed "$cua_speed" --arg bcd "$cua_bcd" --arg bdf "$cua_bdf" --arg control "$cua_ctrl" --arg autosuspend "$cua_auto" --arg wakeup "$cua_wake" '{sysfs:$sysfs,vid:$vendor,pid:$product,manufacturer:(if $manufacturer=="" then null else $manufacturer end),product:(if $product_name=="" then null else $product_name end),speed_mbps:(if $speed=="" then null else ($speed|tonumber? // $speed) end),usb_version:(if $bcd=="" then null else $bcd end),pci_ancestor:(if $bdf=="" then null else $bdf end),power:{control:(if $control=="" then null else $control end),autosuspend:(if $autosuspend=="" then null else $autosuspend end),wakeup:(if $wakeup=="" then null else $wakeup end)}}' >> "$cua_usb" || return 1
    done
    jq -s 'sort_by(.sysfs)' "$cua_usb" > "$cua_dir/usb.json" || return 1
    cua_input=$cua_dir/input.jsonl; : > "$cua_input"
    for cua_event in /sys/class/input/event*; do
        [ -e "$cua_event" ] || continue
        cua_name=${cua_event##*/}; cua_dev=$cua_event/device; cua_label=$(cat "$cua_dev/name" 2>/dev/null); cua_phys=$(cat "$cua_dev/phys" 2>/dev/null); cua_uniq=$(cat "$cua_dev/uniq" 2>/dev/null); cua_real=$(readlink -f "$cua_dev" 2>/dev/null); cua_bdf=$(printf '%s\n' "$cua_real" | grep -oE '[0-9a-fA-F]{4}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}\.[0-7]' | tail -1)
        jq -n --arg event "$cua_name" --arg name "$cua_label" --arg phys "$cua_phys" --arg uniq "$cua_uniq" --arg bdf "$cua_bdf" '{event:$event,name:(if $name=="" then null else $name end),phys:(if $phys=="" then null else $phys end),uniq:(if $uniq=="" then null else $uniq end),pci_ancestor:(if $bdf=="" then null else $bdf end)}' >> "$cua_input" || return 1
    done
    jq -s '.' "$cua_input" > "$cua_dir/input.json" || return 1
    cua_sound=$cua_dir/sound.jsonl; : > "$cua_sound"
    for cua_card in /sys/class/sound/card[0-9]*; do
        [ -e "$cua_card" ] || continue
        cua_name=${cua_card##*/}; cua_id=$(cat "$cua_card/id" 2>/dev/null); cua_real=$(readlink -f "$cua_card/device" 2>/dev/null); cua_bdf=$(printf '%s\n' "$cua_real" | grep -oE '[0-9a-fA-F]{4}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}\.[0-7]' | tail -1)
        jq -n --arg card "$cua_name" --arg id "$cua_id" --arg bdf "$cua_bdf" '{card:$card,id:(if $id=="" then null else $id end),pci_ancestor:(if $bdf=="" then null else $bdf end)}' >> "$cua_sound" || return 1
    done
    jq -s '.' "$cua_sound" > "$cua_dir/sound.json" || return 1
    cua_pcm_runtime=$cua_dir/pcm-runtime.jsonl; : > "$cua_pcm_runtime"
    for cua_pcm_path in /sys/class/sound/pcmC*D*[pc]; do
        [ -e "$cua_pcm_path" ] || continue
        cua_pcm_name=${cua_pcm_path##*/}
        cua_pcm_card=$(printf '%s\n' "$cua_pcm_name" | sed -n 's/^pcmC\([0-9][0-9]*\)D[0-9][0-9]*[pc]$/\1/p')
        cua_pcm_device=$(printf '%s\n' "$cua_pcm_name" | sed -n 's/^pcmC[0-9][0-9]*D\([0-9][0-9]*\)[pc]$/\1/p')
        cua_pcm_dir=$(printf '%s\n' "$cua_pcm_name" | sed -n 's/^.*\([pc]\)$/\1/p')
        [ -n "$cua_pcm_card" ] && [ -n "$cua_pcm_device" ] && [ -n "$cua_pcm_dir" ] || continue
        if [ "$cua_pcm_dir" = p ]; then cua_pcm_kind=playback; else cua_pcm_kind=capture; fi
        cua_pcm_real=$(readlink -f "$cua_pcm_path/device" 2>/dev/null)
        cua_pcm_bdf=$(printf '%s\n' "$cua_pcm_real" | grep -oE '[0-9a-fA-F]{4}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}\.[0-7]' | tail -1)
        cua_pcm_id=$(cat "$cua_pcm_path/id" 2>/dev/null)
        cua_pcm_dev=$(cat "$cua_pcm_path/dev" 2>/dev/null)
        cua_sub_jsonl=$cua_dir/substreams-${cua_pcm_card}-${cua_pcm_device}-${cua_pcm_dir}.jsonl; : > "$cua_sub_jsonl"
        cua_proc_pcm=/proc/asound/card${cua_pcm_card}/pcm${cua_pcm_device}${cua_pcm_dir}
        for cua_sub in "$cua_proc_pcm"/sub*; do
            [ -d "$cua_sub" ] || continue
            cua_sub_name=${cua_sub##*/}
            cua_sub_info=$(cat "$cua_sub/info" 2>/dev/null)
            cua_sub_hw=$(cat "$cua_sub/hw_params" 2>/dev/null)
            cua_sub_status=$(cat "$cua_sub/status" 2>/dev/null)
            jq -n --arg substream "$cua_sub_name" --arg info "$cua_sub_info" --arg hw_params "$cua_sub_hw" --arg status "$cua_sub_status"               '{substream:$substream,info_raw:(if $info=="" then null else $info end),hw_params_raw:(if $hw_params=="" then null else $hw_params end),status_raw:(if $status=="" then null else $status end),stream_open:(($hw_params!="") or ($status!=""))}' >> "$cua_sub_jsonl" || return 1
        done
        jq -s '.' "$cua_sub_jsonl" > "$cua_dir/substreams.json" || return 1
        jq -n --slurpfile subs "$cua_dir/substreams.json" --arg pcm "$cua_pcm_name" --arg card "$cua_pcm_card" --arg device "$cua_pcm_device" --arg direction "$cua_pcm_kind" --arg id "$cua_pcm_id" --arg dev "$cua_pcm_dev" --arg bdf "$cua_pcm_bdf"           '{pcm:$pcm,card:($card|tonumber),device:($device|tonumber),direction:$direction,id:(if $id=="" then null else $id end),dev:(if $dev=="" then null else $dev end),pci_ancestor:(if $bdf=="" then null else $bdf end),substreams:$subs[0]}' >> "$cua_pcm_runtime" || return 1
    done
    jq -s 'sort_by(.card,.device,.direction)' "$cua_pcm_runtime" > "$cua_dir/pcm-runtime.json" || return 1
    inv_flat_dir_json "$cua_dir/hda-params.json" /sys/module/snd_hda_intel/parameters || return 1
    cua_cards=$(cat /proc/asound/cards 2>/dev/null); cua_pcm=$(cat /proc/asound/pcm 2>/dev/null); cua_input_raw=$(cat /proc/bus/input/devices 2>/dev/null)
    jq -n --slurpfile usb "$cua_dir/usb.json" --slurpfile input "$cua_dir/input.json" --slurpfile sound "$cua_dir/sound.json" --slurpfile pcm_runtime "$cua_dir/pcm-runtime.json" --slurpfile hda "$cua_dir/hda-params.json" --slurpfile lsusb "$cua_dir/lsusb.json" --slurpfile tree "$cua_dir/lsusb-tree.json" --slurpfile aplay "$cua_dir/aplay.json" --arg cards "$cua_cards" --arg pcm "$cua_pcm" --arg input_raw "$cua_input_raw" '{usb_devices:$usb[0],input_devices:$input[0],sound_cards:$sound[0],pcm_devices:$pcm_runtime[0],snd_hda_intel_parameters:$hda[0],raw:{lsusb:$lsusb[0],lsusb_tree:$tree[0],aplay:$aplay[0],proc_asound_cards:$cards,proc_asound_pcm:$pcm,proc_input_devices:$input_raw}}' > "$cua_dir/data.json" || return 1
    inv_strings_json "$cua_dir/notes.json" "USB and input PCI ancestors are resolved from sysfs paths when possible; this is topology evidence, not a latency measurement." || return 1
    inv_strings_json "$cua_dir/errors.json" || return 1
    inv_emit usb-input-audio "$cua_dir/data.json" "$cua_dir/notes.json" "$cua_dir/errors.json"
}

print_usb_input_audio_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "USB / Input / Audio Topology View"
    inv_view_kv "USB devices" "$(jq -r '.data.usb_devices|length' "$f")"
    inv_view_kv "Input events" "$(jq -r '.data.input_devices|length' "$f")"
    inv_view_kv "ALSA cards" "$(jq -r '.data.sound_cards|length' "$f")"
    inv_view_kv "PCM devices" "$(jq -r '(.data.pcm_devices//[])|length' "$f")"
    inv_view_section "USB devices"
    jq -r '.data.usb_devices[]|[.sysfs,(.manufacturer//""),(.product//""),(.speed_mbps//"?"|tostring),(.pci_ancestor//"n/a"),(.power.control//"n/a")]|@tsv' "$f" | while IFS="$(printf '\t')" read -r s m p speed bdf ctl; do inv_view_item "$s $m $p speed=${speed}Mb/s controller=$bdf power=$ctl"; done
    inv_view_section "Input devices"
    jq -r '.data.input_devices[]|[.event,(.name//"unknown"),(.phys//""),(.pci_ancestor//"n/a")]|@tsv' "$f" | while IFS="$(printf '\t')" read -r e n p b; do inv_view_item "$e $n phys=$p controller=$b"; done
    inv_view_section "Audio cards"
    jq -r '.data.sound_cards[]|[.card,(.id//"unknown"),(.pci_ancestor//"n/a")]|@tsv' "$f" | while IFS="$(printf '\t')" read -r c i b; do inv_view_item "$c id=$i controller=$b"; done
    inv_view_section "PCM devices / existing stream state"
    jq -r '(.data.pcm_devices//[])[]|[.pcm,.direction,(.id//"unknown"),(.pci_ancestor//"n/a"),([.substreams[]?|select(.stream_open==true)]|length|tostring)]|@tsv' "$f" | while IFS="$(printf '\t')" read -r p d i b o; do inv_view_item "$p $d id=$i controller=$b open-substreams=$o"; done
    inv_view_metadata "$f"
}

# ============================================================
# print_usb_input_audio_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_usb_input_audio_summary() { print_usb_input_audio_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_usb_input_audio_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_usb_input_audio.summary-source.json
        collect_usb_input_audio > "$main_summary_file" || return $?
        print_usb_input_audio_view "$main_summary_file"
        return $?
    fi
    collect_usb_input_audio
}
