...
# Passively samples host scheduler, PSI, VM pressure and context-switch activity without synthetic load.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_runtime_pressure.sh
#   ./view_runtime_pressure.sh --file collect_runtime_pressure.json
#   ./view_runtime_pressure.sh --help
#
# Output:
...
app_name="view_runtime_pressure"
app_version=0.9.2
app_rc=0
app_compact=0
app_package_specs="jq:jq:required vmstat:procps:optional mpstat:sysstat:optional"
...
usage() {
    cat <<'EOF'
view_runtime_pressure.sh - standalone runtime pressure / scheduler view

Usage:
  sudo ./view_runtime_pressure.sh
  ./view_runtime_pressure.sh --file collect_runtime_pressure.json
  sudo ./view_runtime_pressure.sh --json > collect_runtime_pressure.json
...
collect_runtime_pressure() {
    crp_dir=$_inv_temp_dir/runtime-pressure
    mkdir -p "$crp_dir" || return 1
    crp_stat0=$(cat /proc/stat 2>/dev/null); crp_vm0=$(cat /proc/vmstat 2>/dev/null); crp_load0=$(cat /proc/loadavg 2>/dev/null); crp_cpu0=$(cat /proc/pressure/cpu 2>/dev/null); crp_mem0=$(cat /proc/pressure/memory 2>/dev/null); crp_io0=$(cat /proc/pressure/io 2>/dev/null); crp_sched=$(cat /proc/schedstat 2>/dev/null)
    sleep 1
    crp_stat1=$(cat /proc/stat 2>/dev/null); crp_vm1=$(cat /proc/vmstat 2>/dev/null); crp_load1=$(cat /proc/loadavg 2>/dev/null); crp_cpu1=$(cat /proc/pressure/cpu 2>/dev/null); crp_mem1=$(cat /proc/pressure/memory 2>/dev/null); crp_io1=$(cat /proc/pressure/io 2>/dev/null)
    printf '%s\n' "$crp_stat0" > "$crp_dir/stat0"; printf '%s\n' "$crp_stat1" > "$crp_dir/stat1"; printf '%s\n' "$crp_vm0" > "$crp_dir/vm0"; printf '%s\n' "$crp_vm1" > "$crp_dir/vm1"
    crp_ctxt0=$(awk '$1=="ctxt"{print $2}' "$crp_dir/stat0"); crp_ctxt1=$(awk '$1=="ctxt"{print $2}' "$crp_dir/stat1"); crp_proc0=$(awk '$1=="processes"{print $2}' "$crp_dir/stat0"); crp_proc1=$(awk '$1=="processes"{print $2}' "$crp_dir/stat1")
    crp_pgf0=$(awk '$1=="pgfault"{print $2}' "$crp_dir/vm0"); crp_pgf1=$(awk '$1=="pgfault"{print $2}' "$crp_dir/vm1"); crp_maj0=$(awk '$1=="pgmajfault"{print $2}' "$crp_dir/vm0"); crp_maj1=$(awk '$1=="pgmajfault"{print $2}' "$crp_dir/vm1"); crp_in0=$(awk '$1=="pswpin"{print $2}' "$crp_dir/vm0"); crp_in1=$(awk '$1=="pswpin"{print $2}' "$crp_dir/vm1"); crp_out0=$(awk '$1=="pswpout"{print $2}' "$crp_dir/vm0"); crp_out1=$(awk '$1=="pswpout"{print $2}' "$crp_dir/vm1")
    crp_delta_ctxt=$(( ${crp_ctxt1:-0} - ${crp_ctxt0:-0} )); crp_delta_proc=$(( ${crp_proc1:-0} - ${crp_proc0:-0} )); crp_delta_pgf=$(( ${crp_pgf1:-0} - ${crp_pgf0:-0} )); crp_delta_maj=$(( ${crp_maj1:-0} - ${crp_maj0:-0} )); crp_delta_in=$(( ${crp_in1:-0} - ${crp_in0:-0} )); crp_delta_out=$(( ${crp_out1:-0} - ${crp_out0:-0} ))
    inv_command_json "$crp_dir/vmstat.json" 5 vmstat 1 2 || return 1
    inv_command_json "$crp_dir/mpstat.json" 5 mpstat -P ALL 1 1 || return 1
    jq -n --arg load0 "$crp_load0" --arg load1 "$crp_load1" --arg cpu0 "$crp_cpu0" --arg cpu1 "$crp_cpu1" --arg mem0 "$crp_mem0" --arg mem1 "$crp_mem1" --arg io0 "$crp_io0" --arg io1 "$crp_io1" --arg sched "$crp_sched" --arg ctxt "$crp_delta_ctxt" --arg proc "$crp_delta_proc" --arg pgf "$crp_delta_pgf" --arg maj "$crp_delta_maj" --arg pin "$crp_delta_in" --arg pout "$crp_delta_out" --slurpfile vmstat "$crp_dir/vmstat.json" --slurpfile mpstat "$crp_dir/mpstat.json" '{sample_seconds:1,loadavg:{t0:$load0,t1:$load1},pressure:{cpu:{t0:$cpu0,t1:$cpu1},memory:{t0:$mem0,t1:$mem1},io:{t0:$io0,t1:$io1}},deltas:{context_switches:($ctxt|tonumber),processes_created:($proc|tonumber),pgfault:($pgf|tonumber),pgmajfault:($maj|tonumber),pswpin:($pin|tonumber),pswpout:($pout|tonumber)},schedstat_raw:$sched,vmstat:$vmstat[0],mpstat:$mpstat[0]}' > "$crp_dir/data.json" || return 1
    inv_strings_json "$crp_dir/notes.json" "This is a passive one-second host-pressure sample; it does not generate workload." "PSI and delta counters describe the sampled interval only." || return 1
    inv_strings_json "$crp_dir/errors.json" || return 1
    inv_emit runtime-pressure "$crp_dir/data.json" "$crp_dir/notes.json" "$crp_dir/errors.json"
}

print_runtime_pressure_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "Runtime Pressure / Scheduler View"
    inv_view_kv "Passive sample" "$(jq -r '.data.sample_seconds|tostring' "$f") seconds"
    inv_view_section "Load / pressure"
    inv_view_kv "Loadavg t1" "$(jq -r '.data.loadavg.t1' "$f")"
    inv_view_kv "CPU PSI" "$(jq -r '.data.pressure.cpu.t1 // "not-exposed"' "$f")"
    inv_view_kv "Memory PSI" "$(jq -r '.data.pressure.memory.t1 // "not-exposed"' "$f")"
    inv_view_kv "I/O PSI" "$(jq -r '.data.pressure.io.t1 // "not-exposed"' "$f")"
    inv_view_section "One-second deltas"
    jq -r '.data.deltas|to_entries[]|[.key,.value]|@tsv' "$f" | while IFS="$(printf '\t')" read -r k v; do inv_view_item "$k=$v"; done
    inv_view_metadata "$f"
}

# ============================================================
# print_runtime_pressure_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_runtime_pressure_summary() { print_runtime_pressure_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_runtime_pressure_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_runtime_pressure.summary-source.json
        collect_runtime_pressure > "$main_summary_file" || return $?
        print_runtime_pressure_view "$main_summary_file"
        return $?
    fi
    collect_runtime_pressure
}
