...
# Collects sockets, dies, cores, SMT siblings, caches and NUMA topology.
#
# Version:
#   0.9.2
#
...
app_name="view_cpu_topology"
app_version=0.9.2
app_rc=0
app_compact=0
app_package_specs="jq:jq:required lscpu:util-linux:optional lstopo-no-graphics:hwloc:optional"
...
usage() {
    cat <<'EOF'
view_cpu_topology.sh - standalone cpu topology inventory view
...
Default behavior:
  With no arguments, collect the cpu topology aspect live and
  render its colorized human-readable view.

View input:
  --file FILE       Render a previously saved collect_cpu_topology.json envelope.
...
# Builds per-CPU topology records and NUMA-node records from sysfs.
#
# Version:
#   1.0.0
#
...
# ============================================================
collect_cpu_topology() {
    cct_dir=$_inv_temp_dir/cpu-topology
    cct_cpu_stream=$cct_dir/cpus.jsonl
    cct_node_stream=$cct_dir/nodes.jsonl
    mkdir -p "$cct_dir" || return 1
    : > "$cct_cpu_stream"
    : > "$cct_node_stream"
    for cct_cpu in $(printf '%s\n' /sys/devices/system/cpu/cpu[0-9]* | sed 's|.*/cpu||' | sort -n); do
        cct_cpu_path=/sys/devices/system/cpu/cpu$cct_cpu
        [ -d "$cct_cpu_path" ] || continue
        cct_top=$cct_cpu_path/topology
        cct_node=
        for cct_node_path in "$cct_cpu_path"/node[0-9]*; do [ -e "$cct_node_path" ] || continue; cct_node=${cct_node_path##*/}; break; done
        cct_cache_stream=$cct_dir/cache.$cct_cpu.jsonl
        : > "$cct_cache_stream"
        for cct_cache_path in "$cct_cpu_path"/cache/index*; do
            [ -d "$cct_cache_path" ] || continue
            jq -n --arg index "${cct_cache_path##*/}" --arg level "$(cat "$cct_cache_path/level" 2>/dev/null)" --arg type "$(cat "$cct_cache_path/type" 2>/dev/null)" --arg size "$(cat "$cct_cache_path/size" 2>/dev/null)" --arg shared_cpu_list "$(cat "$cct_cache_path/shared_cpu_list" 2>/dev/null)" '{index:$index,level:$level,type:$type,size:$size,shared_cpu_list:$shared_cpu_list}' >> "$cct_cache_stream" || return 1
        done
        jq -s '.' "$cct_cache_stream" > "$cct_dir/cache.$cct_cpu.json" || return 1
        jq -n --arg cpu "$cct_cpu" --arg online "$(cat "$cct_cpu_path/online" 2>/dev/null)" --arg core_id "$(cat "$cct_top/core_id" 2>/dev/null)" --arg package_id "$(cat "$cct_top/physical_package_id" 2>/dev/null)" --arg die_id "$(cat "$cct_top/die_id" 2>/dev/null)" --arg cluster_id "$(cat "$cct_top/cluster_id" 2>/dev/null)" --arg core_type "$(cat "$cct_top/core_type" 2>/dev/null)" --arg thread_siblings_list "$(cat "$cct_top/thread_siblings_list" 2>/dev/null)" --arg core_cpus_list "$(cat "$cct_top/core_cpus_list" 2>/dev/null)" --arg package_cpus_list "$(cat "$cct_top/package_cpus_list" 2>/dev/null)" --arg node "$cct_node" --slurpfile cache "$cct_dir/cache.$cct_cpu.json" '{cpu:($cpu|tonumber),online:$online,core_id:$core_id,physical_package_id:$package_id,die_id:$die_id,cluster_id:$cluster_id,core_type:$core_type,thread_siblings_list:$thread_siblings_list,core_cpus_list:$core_cpus_list,package_cpus_list:$package_cpus_list,node:(if $node=="" then null else $node end),cache:$cache[0]}' >> "$cct_cpu_stream" || return 1
    done
    jq -s '.' "$cct_cpu_stream" > "$cct_dir/cpus.json" || return 1
    for cct_node_path in /sys/devices/system/node/node[0-9]*; do
        [ -d "$cct_node_path" ] || continue
        jq -n --arg node "${cct_node_path##*/}" --arg cpulist "$(cat "$cct_node_path/cpulist" 2>/dev/null)" --arg cpumap "$(cat "$cct_node_path/cpumap" 2>/dev/null)" --arg distance "$(cat "$cct_node_path/distance" 2>/dev/null)" '{node:$node,cpulist:$cpulist,cpumap:$cpumap,distance:$distance}' >> "$cct_node_stream" || return 1
    done
    jq -s '.' "$cct_node_stream" > "$cct_dir/nodes.json" || return 1
    inv_command_json "$cct_dir/lscpu.json" 8 lscpu -J || return 1
    inv_command_json "$cct_dir/lscpu_extended.json" 8 lscpu -e || return 1
    inv_command_json "$cct_dir/lstopo.json" 12 lstopo-no-graphics --of console || return 1
    jq -n --arg present "$(cat /sys/devices/system/cpu/present 2>/dev/null)" --arg possible "$(cat /sys/devices/system/cpu/possible 2>/dev/null)" --arg online "$(cat /sys/devices/system/cpu/online 2>/dev/null)" --arg offline "$(cat /sys/devices/system/cpu/offline 2>/dev/null)" --arg smt_active "$(cat /sys/devices/system/cpu/smt/active 2>/dev/null)" --arg smt_control "$(cat /sys/devices/system/cpu/smt/control 2>/dev/null)" --slurpfile cpus "$cct_dir/cpus.json" --slurpfile nodes "$cct_dir/nodes.json" --slurpfile lscpu "$cct_dir/lscpu.json" --slurpfile lscpu_ext "$cct_dir/lscpu_extended.json" --slurpfile lstopo "$cct_dir/lstopo.json" '{present:$present,possible:$possible,online:$online,offline:$offline,smt_active:$smt_active,smt_control:$smt_control,cpus:$cpus[0],numa_nodes:$nodes[0],raw:{lscpu_json:$lscpu[0],lscpu_extended:$lscpu_ext[0],lstopo:$lstopo[0]}}' > "$cct_dir/data.json" || return 1
    inv_strings_json "$cct_dir/notes.json" "A missing core_type is reported as an empty value; it is not interpreted as proof of homogeneous cores." || return 1
    inv_strings_json "$cct_dir/errors.json" || return 1
    inv_emit cpu-topology "$cct_dir/data.json" "$cct_dir/notes.json" "$cct_dir/errors.json"
}



# ============================================================
# print_cpu_topology_view
# Presents a colorized CPU topology view.
#
# Version:
#   1.1.0
# ============================================================
print_cpu_topology_view() {
    pctv_file=$1
    inv_summary_validate_file "$pctv_file" || return $?
    inv_view_header "$pctv_file" "CPU Topology View"

    inv_view_section "Topology overview"
    inv_view_kv "Logical CPUs" "$(jq -r '.data.cpus|length' "$pctv_file")"
    inv_view_kv "Physical cores" "$(jq -r '[.data.cpus[]|(.physical_package_id+":"+ .core_id)]|unique|length' "$pctv_file")"
    inv_view_kv "Packages" "$(jq -r '[.data.cpus[].physical_package_id]|unique|length' "$pctv_file")"
    inv_view_kv "NUMA nodes" "$(jq -r '.data.numa_nodes|length' "$pctv_file")"
    inv_view_kv "Present CPUs" "$(jq -r '.data.present // "unknown"' "$pctv_file")"
    inv_view_kv "Possible CPUs" "$(jq -r '.data.possible // "unknown"' "$pctv_file")"
    inv_view_kv "Online CPUs" "$(jq -r '.data.online // "unknown"' "$pctv_file")"
    inv_view_kv "Raw offline mask" "$(jq -r '.data.offline // "none"' "$pctv_file")"
    inv_view_kv "SMT active" "$(jq -r '.data.smt_active // "unknown"' "$pctv_file")"
    inv_view_kv "SMT control" "$(jq -r '.data.smt_control // "unknown"' "$pctv_file")"

    inv_view_section "Physical core / SMT map"
    jq -r '.data.cpus
        | group_by(.thread_siblings_list)
        | map(.[0])
        | sort_by((.cpu|tonumber))[]
        | [(.cpu|tostring),(.core_id//"?"),(.physical_package_id//"?"),(.thread_siblings_list//"?"),(.node//"no NUMA node"),
           (if (.online // "") == "" then "online (fixed)" elif .online=="1" then "online" else "offline" end)]
        | @tsv' "$pctv_file" |
    while IFS="$(printf '\t')" read -r pctv_cpu pctv_core pctv_pkg pctv_sibs pctv_node pctv_on; do
        inv_view_item "CPU $pctv_cpu -> core=$pctv_core package=$pctv_pkg siblings=$pctv_sibs node=$pctv_node $pctv_on"
    done

    inv_view_section "Cache topology"
    jq -r '[.data.cpus[].cache[] | {level,type,size,shared_cpu_list}] | unique_by([.level,.type,.size,.shared_cpu_list]) | sort_by(.level,.type,.shared_cpu_list)[] | [.level,.type,.size,.shared_cpu_list] | @tsv' "$pctv_file" |
    while IFS="$(printf '\t')" read -r pctv_level pctv_type pctv_size pctv_shared; do
        inv_view_item "L$pctv_level $pctv_type $pctv_size shared CPUs=$pctv_shared"
    done

    inv_view_section "NUMA nodes"
    jq -r '.data.numa_nodes[]? | [.node, (.cpulist // "?"), (.distance // "")] | @tsv' "$pctv_file" |
    while IFS="$(printf '\t')" read -r pctv_node pctv_cpus pctv_dist; do
        inv_view_item "$pctv_node CPUs=$pctv_cpus distance=$pctv_dist"
    done

    inv_view_metadata "$pctv_file"
}

# ============================================================
# print_cpu_topology_summary
# Presents CPU, SMT, cache-sharing and NUMA topology.
#
# Version:
#   1.1.0
#
# Usage:
#   print_cpu_topology_summary collectorEnvelope.json
#
# Returns:
#   0 on success
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_cpu_topology_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_cpu_topology.summary-source.json
        collect_cpu_topology > "$main_summary_file" || return $?
        print_cpu_topology_view "$main_summary_file"
        return $?
    fi
    collect_cpu_topology
}
