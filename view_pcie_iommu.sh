...
#
# Usage:
#   sudo ./view_pcie_iommu.sh
#   ./view_pcie_iommu.sh --file collect_pcie_iommu.json
#   ./view_pcie_iommu.sh --help
#
# Output:
...
usage() {
    cat <<'EOF'
view_pcie_iommu.sh - standalone pcie iommu inventory view

Usage:
  sudo ./view_pcie_iommu.sh
  ./view_pcie_iommu.sh --file collect_pcie_iommu.json
  sudo ./view_pcie_iommu.sh --json > collect_pcie_iommu.json

Default behavior:
  With no arguments, collect the pcie iommu aspect live and
  render its colorized human-readable view.

View input:
  --file FILE       Render a previously saved collect_pcie_iommu.json envelope.
