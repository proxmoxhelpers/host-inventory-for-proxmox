...
# Collects KVM/QEMU/vhost/VFIO module parameters and virtualization runtime capabilities.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./collect_virtualization_stack.sh [--compact]
#   ./collect_virtualization_stack.sh --help
#   ./collect_virtualization_stack.sh --version
#
# Output:
...
app_name="collect_virtualization_stack"
app_version=0.9.2
app_rc=0
app_compact=0
...
usage() {
    cat <<'EOF'
collect_virtualization_stack.sh - collect KVM/QEMU/vhost/VFIO virtualization-stack evidence

Usage:
  ./collect_virtualization_stack.sh [--compact|--summary|--view]
  ./collect_virtualization_stack.sh --help
  ./collect_virtualization_stack.sh --version

The collector is read-only. Optional QEMU/module facilities that are absent are
...
collect_virtualization_stack() {
    cvs_dir=$_inv_temp_dir/virtualization-stack
    mkdir -p "$cvs_dir/modules" || return 1

    cvs_module_list="kvm kvm_amd kvm_intel vfio vfio_iommu_type1 vfio_pci vhost vhost_net vhost_vsock irqbypass amd_iommu_v2"
    cvs_modules_stream=$cvs_dir/module-params.jsonl
    : > "$cvs_modules_stream" || return 1
    for cvs_module in $cvs_module_list; do
        cvs_param_dir=/sys/module/$cvs_module/parameters
        cvs_param_json=$cvs_dir/modules/$cvs_module.json
        if [ -d "$cvs_param_dir" ]; then
            inv_flat_dir_json "$cvs_param_json" "$cvs_param_dir" || return 1
            cvs_loaded=true
        else
            printf '{}
' > "$cvs_param_json" || return 1
            cvs_loaded=false
        fi
        jq -n --arg module_name "$cvs_module" --argjson loaded "$cvs_loaded" --slurpfile parameters "$cvs_param_json" '{module:$module_name,loaded:$loaded,parameters:$parameters[0]}' >> "$cvs_modules_stream" || return 1
    done
    jq -s '.' "$cvs_modules_stream" > "$cvs_dir/modules.json" || return 1

    cvs_kvm_exists=false
    cvs_kvm_mode=
    cvs_kvm_uid=
    cvs_kvm_gid=
    if [ -e /dev/kvm ]; then
        cvs_kvm_exists=true
        cvs_kvm_mode=$(stat -c '%a' /dev/kvm 2>/dev/null)
        cvs_kvm_uid=$(stat -c '%u' /dev/kvm 2>/dev/null)
        cvs_kvm_gid=$(stat -c '%g' /dev/kvm 2>/dev/null)
    fi

    cvs_flags=$(sed -n 's/^flags[[:space:]]*:[[:space:]]*//p' /proc/cpuinfo 2>/dev/null | head -1)
    cvs_vflags_stream=$cvs_dir/virt-flags.jsonl
    : > "$cvs_vflags_stream" || return 1
    for cvs_flag in $cvs_flags; do
        case "$cvs_flag" in
            vmx|svm|npt|avic|v_vmsave_vmload|vgif|nrip_save|tsc_scale|vmcb_clean|decodeassists|pausefilter|pfthreshold|flushbyasid|sev|sev_es|sev_snp|v_spec_ctrl)
                jq -n --arg v "$cvs_flag" '$v' >> "$cvs_vflags_stream" || return 1
                ;;
        esac
    done
    jq -s 'unique' "$cvs_vflags_stream" > "$cvs_dir/virt-flags.json" || return 1

    inv_command_json "$cvs_dir/modules-raw.json" 8 sh -c 'grep -E "^(kvm|kvm_amd|kvm_intel|vfio|vfio_iommu_type1|vfio_pci|vhost|vhost_net|vhost_vsock|irqbypass|amd_iommu_v2|virtio) " /proc/modules 2>/dev/null || true' || return 1
    inv_command_json "$cvs_dir/qemu-version.json" 8 qemu-system-x86_64 --version || return 1
    inv_command_json "$cvs_dir/kvm-version.json" 8 kvm --version || return 1
    inv_command_json "$cvs_dir/pveversion.json" 8 pveversion --verbose || return 1
    inv_command_json "$cvs_dir/dmesg.json" 8 sh -c 'dmesg 2>/dev/null | grep -Ei "KVM|SVM|VMX|NPT|AVIC|SEV|VFIO|vhost|virtio|halt.poll|IOMMU" || true' || return 1

    cvs_io_uring_disabled=$(cat /proc/sys/kernel/io_uring_disabled 2>/dev/null)
    cvs_io_uring_group=$(cat /proc/sys/kernel/io_uring_group 2>/dev/null)
    cvs_debugfs=false
    [ -d /sys/kernel/debug/kvm ] && cvs_debugfs=true
    cvs_debug_count=0
    if [ "$cvs_debugfs" = true ]; then cvs_debug_count=$(find /sys/kernel/debug/kvm -maxdepth 1 -type f 2>/dev/null | wc -l | tr -d ' '); fi

    jq -n \
      --argjson kvm_exists "$cvs_kvm_exists" --arg kvm_mode "$cvs_kvm_mode" --arg kvm_uid "$cvs_kvm_uid" --arg kvm_gid "$cvs_kvm_gid" \
      --slurpfile modules "$cvs_dir/modules.json" --slurpfile flags "$cvs_dir/virt-flags.json" \
      --slurpfile modules_raw "$cvs_dir/modules-raw.json" --slurpfile qemu "$cvs_dir/qemu-version.json" --slurpfile kvmver "$cvs_dir/kvm-version.json" --slurpfile pve "$cvs_dir/pveversion.json" --slurpfile dmesg "$cvs_dir/dmesg.json" \
      --arg io_disabled "$cvs_io_uring_disabled" --arg io_group "$cvs_io_uring_group" --argjson debugfs "$cvs_debugfs" --arg debug_count "$cvs_debug_count" \
      '{kvm_device:{exists:$kvm_exists,mode:(if $kvm_mode=="" then null else $kvm_mode end),uid:(if $kvm_uid=="" then null else $kvm_uid end),gid:(if $kvm_gid=="" then null else $kvm_gid end)},module_parameters:$modules[0],cpu_virtualization_flags:$flags[0],io_uring:{disabled:(if $io_disabled=="" then null else $io_disabled end),group:(if $io_group=="" then null else $io_group end)},debugfs_kvm:{present:$debugfs,top_level_file_count:($debug_count|tonumber? // 0)},raw:{loaded_modules:$modules_raw[0],qemu_system:$qemu[0],kvm_command:$kvmver[0],pveversion:$pve[0],dmesg_relevant:$dmesg[0]}}' > "$cvs_dir/data.json" || return 1

    inv_strings_json "$cvs_dir/notes.json" \
      "KVM/QEMU/module parameters are observations only; the collector does not change virtualization settings." \
      "CPU virtualization flags indicate exposed CPU capabilities, not a literal firmware-toggle state." \
      "Debugfs KVM statistics are not enabled or mounted by this collector." || return 1
    inv_strings_json "$cvs_dir/errors.json" || return 1
    inv_emit virtualization-stack "$cvs_dir/data.json" "$cvs_dir/notes.json" "$cvs_dir/errors.json"
}

print_virtualization_stack_view() {
    pvs_file=$1
    inv_summary_validate_file "$pvs_file" || return $?
    inv_view_header "$pvs_file" "KVM / Virtualization Stack View"

    inv_view_section "KVM device / CPU capability"
    inv_view_kv "/dev/kvm" "$(jq -r 'if .data.kvm_device.exists then "present mode="+(.data.kvm_device.mode//"?")+" uid="+(.data.kvm_device.uid//"?")+" gid="+(.data.kvm_device.gid//"?") else "absent" end' "$pvs_file")"
    inv_view_kv "Virtualization flags" "$(jq -r '.data.cpu_virtualization_flags|join(", ")|if .=="" then "none observed" else . end' "$pvs_file")"
    inv_view_kv "io_uring disabled" "$(jq -r '.data.io_uring.disabled // "not-exposed"' "$pvs_file")"
    inv_view_kv "io_uring group" "$(jq -r '.data.io_uring.group // "not-exposed"' "$pvs_file")"
    inv_view_kv "KVM debugfs" "$(jq -r 'if .data.debugfs_kvm.present then "present" else "not-present" end' "$pvs_file")"

    inv_view_section "Loaded virtualization modules / parameters"
    jq -r '.data.module_parameters[]? | select(.loaded==true) | [.module,((.parameters|to_entries|length)|tostring),(.parameters|to_entries|map(.key+"="+(.value|tostring))|join(" "))] | @tsv' "$pvs_file" |
    while IFS="$(printf '	')" read -r pvs_module pvs_count pvs_params; do
        inv_view_item "$pvs_module parameters=$pvs_count${pvs_params:+ $pvs_params}"
    done

    inv_view_section "Userspace virtualization tooling"
    inv_view_kv "qemu-system-x86_64" "$(jq -r 'if .data.raw.qemu_system.available then ((.data.raw.qemu_system.stdout//"")|split("\n")[0]) else "unavailable" end' "$pvs_file")"
    inv_view_kv "kvm command" "$(jq -r 'if .data.raw.kvm_command.available then ((.data.raw.kvm_command.stdout//"")|split("\n")[0]) else "unavailable" end' "$pvs_file")"
    inv_view_kv "pveversion" "$(jq -r 'if .data.raw.pveversion.available then ((.data.raw.pveversion.stdout//"")|split("\n")[0]) else "unavailable" end' "$pvs_file")"

    inv_view_metadata "$pvs_file"
}

# ============================================================
# print_virtualization_stack_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_virtualization_stack_summary() { print_virtualization_stack_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_virtualization_stack_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_virtualization_stack.summary-source.json
        collect_virtualization_stack > "$main_summary_file" || return $?
        print_virtualization_stack_view "$main_summary_file"
        return $?
    fi
    collect_virtualization_stack
}
