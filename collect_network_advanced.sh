...
# Collects advanced physical NIC RSS, EEE, pause, statistics and queue-steering evidence.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./collect_network_advanced.sh [--compact]
#   ./collect_network_advanced.sh --help
#   ./collect_network_advanced.sh --version
#
# Output:
...
app_name="collect_network_advanced"
app_version=0.9.2
app_rc=0
app_compact=0
app_package_specs="jq:jq:required ethtool:ethtool:optional"
...
usage() {
    cat <<'EOF'
collect_network_advanced.sh - collect advanced network RSS, EEE, pause, statistics and steering evidence

Usage:
  ./collect_network_advanced.sh [--compact|--summary|--view]
  ./collect_network_advanced.sh --help
  ./collect_network_advanced.sh --version
...
collect_network_advanced() {
    cna_dir=$_inv_temp_dir/network-advanced
    mkdir -p "$cna_dir" || return 1
    cna_stream=$cna_dir/interfaces.jsonl; : > "$cna_stream" || return 1
    for cna_path in /sys/class/net/*; do
        [ -d "$cna_path" ] || continue
        cna_name=${cna_path##*/}
        [ -L "$cna_path/device" ] || continue
        cna_bdf=$(readlink -f "$cna_path/device" 2>/dev/null | grep -oE '[0-9a-fA-F]{4}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}\.[0-7]' | tail -1)
        for cna_cmd in driver link channels ring coalesce features rss pause eee stats; do :; done
        inv_command_json "$cna_dir/$cna_name-driver.json" 8 ethtool -i "$cna_name" || return 1
        inv_command_json "$cna_dir/$cna_name-link.json" 8 ethtool "$cna_name" || return 1
        inv_command_json "$cna_dir/$cna_name-channels.json" 8 ethtool -l "$cna_name" || return 1
        inv_command_json "$cna_dir/$cna_name-ring.json" 8 ethtool -g "$cna_name" || return 1
        inv_command_json "$cna_dir/$cna_name-coalesce.json" 8 ethtool -c "$cna_name" || return 1
        inv_command_json "$cna_dir/$cna_name-features.json" 8 ethtool -k "$cna_name" || return 1
        inv_command_json "$cna_dir/$cna_name-rss.json" 8 ethtool -x "$cna_name" || return 1
        inv_command_json "$cna_dir/$cna_name-pause.json" 8 ethtool -a "$cna_name" || return 1
        inv_command_json "$cna_dir/$cna_name-eee.json" 8 ethtool --show-eee "$cna_name" || return 1
        inv_command_json "$cna_dir/$cna_name-stats.json" 12 ethtool -S "$cna_name" || return 1
        cna_qstream=$cna_dir/$cna_name-queues.jsonl; : > "$cna_qstream"
        for cna_q in "$cna_path"/queues/*; do
            [ -d "$cna_q" ] || continue
            cna_qname=${cna_q##*/}
            cna_rps=$(cat "$cna_q/rps_cpus" 2>/dev/null); cna_flow=$(cat "$cna_q/rps_flow_cnt" 2>/dev/null); cna_xps=$(cat "$cna_q/xps_cpus" 2>/dev/null); cna_xrx=$(cat "$cna_q/xps_rxqs" 2>/dev/null)
            jq -n --arg queue "$cna_qname" --arg rps "$cna_rps" --arg flow "$cna_flow" --arg xps "$cna_xps" --arg xrx "$cna_xrx" '{queue:$queue,rps_cpus:(if $rps=="" then null else $rps end),rps_flow_cnt:(if $flow=="" then null else $flow end),xps_cpus:(if $xps=="" then null else $xps end),xps_rxqs:(if $xrx=="" then null else $xrx end)}' >> "$cna_qstream" || return 1
        done
        jq -s '.' "$cna_qstream" > "$cna_dir/$cna_name-queues.json" || return 1
        jq -n --arg name "$cna_name" --arg bdf "$cna_bdf" --slurpfile driver "$cna_dir/$cna_name-driver.json" --slurpfile link "$cna_dir/$cna_name-link.json" --slurpfile channels "$cna_dir/$cna_name-channels.json" --slurpfile ring "$cna_dir/$cna_name-ring.json" --slurpfile coal "$cna_dir/$cna_name-coalesce.json" --slurpfile feat "$cna_dir/$cna_name-features.json" --slurpfile rss "$cna_dir/$cna_name-rss.json" --slurpfile pause "$cna_dir/$cna_name-pause.json" --slurpfile eee "$cna_dir/$cna_name-eee.json" --slurpfile stats "$cna_dir/$cna_name-stats.json" --slurpfile queues "$cna_dir/$cna_name-queues.json" '{name:$name,bdf:(if $bdf=="" then null else $bdf end),driver:$driver[0],link:$link[0],channels:$channels[0],ring:$ring[0],coalesce:$coal[0],features:$feat[0],rss:$rss[0],pause:$pause[0],eee:$eee[0],statistics:$stats[0],queues:$queues[0]}' >> "$cna_stream" || return 1
    done
    jq -s 'sort_by(.name)' "$cna_stream" > "$cna_dir/interfaces.json" || return 1
    inv_file_map_json "$cna_dir/sysctls.json" "netdev_budget=/proc/sys/net/core/netdev_budget" "netdev_budget_usecs=/proc/sys/net/core/netdev_budget_usecs" "netdev_max_backlog=/proc/sys/net/core/netdev_max_backlog" "rps_sock_flow_entries=/proc/sys/net/core/rps_sock_flow_entries" "busy_read=/proc/sys/net/core/busy_read" "busy_poll=/proc/sys/net/core/busy_poll" || return 1
    cna_softnet=$(cat /proc/net/softnet_stat 2>/dev/null)
    jq -n --slurpfile interfaces "$cna_dir/interfaces.json" --slurpfile sysctls "$cna_dir/sysctls.json" --arg softnet "$cna_softnet" '{interfaces:$interfaces[0],net_core:$sysctls[0],softnet_stat_raw:$softnet}' > "$cna_dir/data.json" || return 1
    inv_strings_json "$cna_dir/notes.json" "NIC statistics are cumulative snapshots; RSS/RPS/XPS describe different steering layers and are not conflated." || return 1
    inv_strings_json "$cna_dir/errors.json" || return 1
    inv_emit network-advanced "$cna_dir/data.json" "$cna_dir/notes.json" "$cna_dir/errors.json"
}

print_network_advanced_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "Advanced Network View"
    inv_view_section "Physical NICs"
    jq -r '.data.interfaces[]|[.name,(.bdf//"n/a"),(.driver.rc//"unavailable"|tostring),(.rss.rc//"unavailable"|tostring),(.eee.rc//"unavailable"|tostring),(.pause.rc//"unavailable"|tostring),(.statistics.rc//"unavailable"|tostring),(.queues|length|tostring)]|@tsv' "$f" | while IFS="$(printf '\t')" read -r n b d r e p s q; do inv_view_item "$n BDF=$b queues=$q driver-rc=$d RSS-rc=$r EEE-rc=$e pause-rc=$p stats-rc=$s"; done
    inv_view_section "net.core steering/backlog knobs"
    jq -r '.data.net_core|to_entries[]|[.key,.value]|@tsv' "$f" | while IFS="$(printf '\t')" read -r k v; do inv_view_item "$k=$v"; done
    inv_view_metadata "$f"
}

# ============================================================
# print_network_advanced_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_network_advanced_summary() { print_network_advanced_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_network_advanced_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_network_advanced.summary-source.json
        collect_network_advanced > "$main_summary_file" || return $?
        print_network_advanced_view "$main_summary_file"
        return $?
    fi
    collect_network_advanced
}
