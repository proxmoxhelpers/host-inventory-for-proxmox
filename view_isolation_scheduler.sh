...
# Collects CPU isolation, workqueue masks, cgroup/systemd affinity and scheduler state.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_isolation_scheduler.sh
#   ./view_isolation_scheduler.sh --file collect_isolation_scheduler.json
#   ./view_isolation_scheduler.sh --help
#
# Output:
...
app_name="view_isolation_scheduler"
app_version=0.9.2
app_rc=0
app_compact=0
...
usage() {
    cat <<'EOF'
view_isolation_scheduler.sh - standalone isolation scheduler inventory view

Usage:
  sudo ./view_isolation_scheduler.sh
  ./view_isolation_scheduler.sh --file collect_isolation_scheduler.json
  sudo ./view_isolation_scheduler.sh --json > collect_isolation_scheduler.json

Default behavior:
  With no arguments, collect the isolation scheduler aspect live and
  render its colorized human-readable view.

View input:
  --file FILE       Render a previously saved collect_isolation_scheduler.json envelope.
...
# ============================================================
# collect_isolation_scheduler
# Collects kernel isolation parameters and runtime CPU-affinity controls.
#
# Version:
#   1.0.0
#
...
# ============================================================
collect_isolation_scheduler() {
    cis_dir=$_inv_temp_dir/isolation
    cis_wq_stream=$cis_dir/workqueues.jsonl
    mkdir -p "$cis_dir" || return 1
    : > "$cis_wq_stream"
    for cis_wq in /sys/devices/virtual/workqueue/*; do
        [ -d "$cis_wq" ] || continue
        inv_flat_dir_json "$cis_dir/workqueue-values.json" "$cis_wq" '^(cpumask|cpumask_requested|cpumask_isolated|affinity_scope|nice)$' || return 1
        jq -n --arg workqueue "${cis_wq##*/}" --slurpfile values "$cis_dir/workqueue-values.json" '{workqueue:$workqueue,values:$values[0]}' >> "$cis_wq_stream" || return 1
    done
    jq -s '.' "$cis_wq_stream" > "$cis_dir/workqueues.json" || return 1
    inv_file_map_json "$cis_dir/cgroup.json" "controllers=/sys/fs/cgroup/cgroup.controllers" "subtree_control=/sys/fs/cgroup/cgroup.subtree_control" "cpuset_cpus=/sys/fs/cgroup/cpuset.cpus" "cpuset_cpus_effective=/sys/fs/cgroup/cpuset.cpus.effective" "cpuset_mems=/sys/fs/cgroup/cpuset.mems" "cpuset_mems_effective=/sys/fs/cgroup/cpuset.mems.effective" "cpuset_cpus_partition=/sys/fs/cgroup/cpuset.cpus.partition" || return 1
    inv_command_json "$cis_dir/system_slice.json" 6 systemctl show system.slice -p AllowedCPUs -p EffectiveCPUs -p AllowedMemoryNodes -p EffectiveMemoryNodes || return 1
    inv_command_json "$cis_dir/user_slice.json" 6 systemctl show user.slice -p AllowedCPUs -p EffectiveCPUs -p AllowedMemoryNodes -p EffectiveMemoryNodes || return 1
    inv_command_json "$cis_dir/machine_slice.json" 6 systemctl show machine.slice -p AllowedCPUs -p EffectiveCPUs -p AllowedMemoryNodes -p EffectiveMemoryNodes || return 1
    inv_command_json "$cis_dir/ps_threads.json" 12 ps -eLo pid,tid,psr,cls,rtprio,pri,ni,comm --sort=psr || return 1
    jq -n --arg cmdline "$(cat /proc/cmdline 2>/dev/null)" --arg isolated "$(cat /sys/devices/system/cpu/isolated 2>/dev/null)" --arg nohz_full "$(cat /sys/devices/system/cpu/nohz_full 2>/dev/null)" --arg rcu_nocb "$(cat /sys/kernel/rcu_nocb_mask 2>/dev/null)" --arg autogroup "$(cat /proc/sys/kernel/sched_autogroup_enabled 2>/dev/null)" --arg numa_balancing "$(cat /proc/sys/kernel/numa_balancing 2>/dev/null)" --arg rt_runtime "$(cat /proc/sys/kernel/sched_rt_runtime_us 2>/dev/null)" --arg rt_period "$(cat /proc/sys/kernel/sched_rt_period_us 2>/dev/null)" --slurpfile workqueues "$cis_dir/workqueues.json" --slurpfile cgroup "$cis_dir/cgroup.json" --slurpfile system_slice "$cis_dir/system_slice.json" --slurpfile user_slice "$cis_dir/user_slice.json" --slurpfile machine_slice "$cis_dir/machine_slice.json" --slurpfile ps "$cis_dir/ps_threads.json" '{cmdline_raw:$cmdline,kernel_runtime:{isolated_cpus:$isolated,nohz_full_cpus:$nohz_full,rcu_nocb_mask:$rcu_nocb},global_workqueues:$workqueues[0],cgroup_v2_root:$cgroup[0],systemd_cpu_affinity:{system_slice:$system_slice[0],user_slice:$user_slice[0],machine_slice:$machine_slice[0]},scheduler_sysctls:{sched_autogroup_enabled:$autogroup,numa_balancing:$numa_balancing,sched_rt_runtime_us:$rt_runtime,sched_rt_period_us:$rt_period},raw:{ps_threads:$ps[0]}}' > "$cis_dir/data.json" || return 1
    inv_strings_json "$cis_dir/notes.json" "The raw kernel command line is preserved; policy interpretation of isolcpus/nohz_full/irqaffinity is deferred to the harmonizer." || return 1
    inv_strings_json "$cis_dir/errors.json" || return 1
    inv_emit isolation-scheduler "$cis_dir/data.json" "$cis_dir/notes.json" "$cis_dir/errors.json"
}



# ============================================================
# print_isolation_scheduler_view
# Presents a colorized isolation / scheduler view.
#
# Version:
#   1.1.0
# ============================================================
print_isolation_scheduler_view() {
    pisv_file=$1
    inv_summary_validate_file "$pisv_file" || return $?
    inv_view_header "$pisv_file" "Isolation / Scheduler View"

    inv_view_section "Isolation overview"
    inv_view_kv "Isolated CPUs" "$(jq -r '(.data.kernel_runtime.isolated_cpus // "") | if .=="" or test("^\\s*\\(null\\)\\s*$") then "none" else . end' "$pisv_file")"
    inv_view_kv "nohz_full CPUs" "$(jq -r '(.data.kernel_runtime.nohz_full_cpus // "") | if .=="" or test("^\\s*\\(null\\)\\s*$") then "none" else . end' "$pisv_file")"
    inv_view_kv "RCU nocb mask" "$(jq -r '(.data.kernel_runtime.rcu_nocb_mask // "") | if .=="" or test("^\\s*\\(null\\)\\s*$") then "none" else . end' "$pisv_file")"
    inv_view_kv "cgroup CPUs effective" "$(jq -r '.data.cgroup_v2_root.cpuset_cpus_effective // "unknown"' "$pisv_file")"

    inv_view_section "Scheduler sysctls"
    jq -r '.data.scheduler_sysctls | to_entries[] | [.key,.value] | @tsv' "$pisv_file" |
    while IFS="$(printf '\t')" read -r pisv_key pisv_value; do
        inv_view_item "$pisv_key = $pisv_value"
    done

    inv_view_section "Global workqueue CPU masks"
    jq -r '.data.global_workqueues
        | map({name:.workqueue, mask:(.values.cpumask // .values.cpumask_requested // "not-exposed")})
        | group_by(.mask)
        | sort_by(.[0].mask)[]
        | [(.[0].mask), (length|tostring), (map(.name)|join(", "))]
        | @tsv' "$pisv_file" |
    while IFS="$(printf '\t')" read -r pisv_mask pisv_count pisv_names; do
        inv_view_item "mask=$pisv_mask ($pisv_count): $pisv_names"
    done

    inv_view_metadata "$pisv_file"
}

# ============================================================
# print_isolation_scheduler_summary
# Presents CPU isolation, workqueue and scheduler state.
#
# Version:
#   1.1.0
#
# Usage:
#   print_isolation_scheduler_summary collectorEnvelope.json
#
# Returns:
#   0 on success
...
# ============================================================
print_isolation_scheduler_summary() {
    print_isolation_scheduler_view "$1"
}
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_isolation_scheduler_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_isolation_scheduler.summary-source.json
        collect_isolation_scheduler > "$main_summary_file" || return $?
        print_isolation_scheduler_view "$main_summary_file"
        return $?
    fi
    collect_isolation_scheduler
}
