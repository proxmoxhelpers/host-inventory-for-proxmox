...
# Collects advanced PCIe locality, power, reset, AER, SR-IOV and capability evidence.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./collect_pcie_advanced.sh [--compact]
#   ./collect_pcie_advanced.sh --help
#   ./collect_pcie_advanced.sh --version
#
# Output:
...
app_name="collect_pcie_advanced"
app_version=0.9.2
app_rc=0
app_compact=0
...
usage() {
    cat <<'EOF'
collect_pcie_advanced.sh - collect advanced PCIe locality, power-management and error capability evidence

Usage:
  ./collect_pcie_advanced.sh [--compact|--summary|--view]
  ./collect_pcie_advanced.sh --help
  ./collect_pcie_advanced.sh --version
...
collect_pcie_advanced() {
    cpa_dir=$_inv_temp_dir/pcie-advanced
    mkdir -p "$cpa_dir" || return 1
    cpa_stream=$cpa_dir/devices.jsonl; : > "$cpa_stream" || return 1
    for cpa_path in /sys/bus/pci/devices/*; do
        [ -d "$cpa_path" ] || continue
        cpa_bdf=${cpa_path##*/}
        cpa_local_cpus=$(cat "$cpa_path/local_cpus" 2>/dev/null)
        cpa_local_cpulist=$(cat "$cpa_path/local_cpulist" 2>/dev/null)
        cpa_node=$(cat "$cpa_path/numa_node" 2>/dev/null)
        cpa_d3=$(cat "$cpa_path/d3cold_allowed" 2>/dev/null)
        cpa_power_control=$(cat "$cpa_path/power/control" 2>/dev/null)
        cpa_runtime=$(cat "$cpa_path/power/runtime_status" 2>/dev/null)
        cpa_wakeup=$(cat "$cpa_path/power/wakeup" 2>/dev/null)
        cpa_reset=false; [ -e "$cpa_path/reset" ] && cpa_reset=true
        cpa_reset_method=$(cat "$cpa_path/reset_method" 2>/dev/null)
        cpa_totalvfs=$(cat "$cpa_path/sriov_totalvfs" 2>/dev/null)
        cpa_numvfs=$(cat "$cpa_path/sriov_numvfs" 2>/dev/null)
        cpa_cur_speed=$(cat "$cpa_path/current_link_speed" 2>/dev/null)
        cpa_cur_width=$(cat "$cpa_path/current_link_width" 2>/dev/null)
        cpa_max_speed=$(cat "$cpa_path/max_link_speed" 2>/dev/null)
        cpa_max_width=$(cat "$cpa_path/max_link_width" 2>/dev/null)
        cpa_aer=$cpa_dir/aer-${cpa_bdf}.json
        cpa_aer_stream=$cpa_dir/aer-${cpa_bdf}.jsonl; : > "$cpa_aer_stream"
        for cpa_file in "$cpa_path"/aer_*; do
            [ -f "$cpa_file" ] || continue
            cpa_key=${cpa_file##*/}; cpa_val=$(cat "$cpa_file" 2>/dev/null)
            jq -n --arg key "$cpa_key" --arg value "$cpa_val" '{($key):$value}' >> "$cpa_aer_stream"
        done
        jq -s 'reduce .[] as $x ({}; . * $x)' "$cpa_aer_stream" > "$cpa_aer" || return 1
        cpa_detail=""; if command -v lspci >/dev/null 2>&1; then cpa_detail=$(lspci -s "$cpa_bdf" -vv 2>/dev/null); fi
        cpa_sysfs=$(readlink -f "$cpa_path" 2>/dev/null)
        cpa_ancestry=$(printf '%s\n' "$cpa_sysfs" | tr '/' '\n' | grep -E '^[0-9a-fA-F]{4}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}\.[0-7]$' | jq -R -s 'split("\n")|map(select(length>0))' 2>/dev/null)
        [ -n "$cpa_ancestry" ] || cpa_ancestry='[]'
        cpa_pcie_type=$(printf '%s\n' "$cpa_detail" | grep -m1 -E 'Express \(v[0-9]+\) (Endpoint|Root Port|Upstream Port|Downstream Port|Legacy Endpoint|Root Complex Integrated Endpoint)' 2>/dev/null)
        cpa_aspm=$(printf '%s\n' "$cpa_detail" | grep -E 'LnkCtl:.*ASPM|LnkCap:.*ASPM|L1SubCap:|L1SubCtl' 2>/dev/null | paste -sd ' | ' -)
        cpa_ltr=$(printf '%s\n' "$cpa_detail" | grep -E 'Latency Tolerance Reporting|LTR' 2>/dev/null | paste -sd ' | ' -)
        cpa_acs=$(printf '%s\n' "$cpa_detail" | grep -E 'Access Control Services|ACSCtl|ACSCap' 2>/dev/null | paste -sd ' | ' -)
        cpa_ari=$(printf '%s\n' "$cpa_detail" | grep -E 'Alternative Routing-ID|ARICtl|ARICap' 2>/dev/null | paste -sd ' | ' -)
        cpa_dpc=$(printf '%s\n' "$cpa_detail" | grep -E 'Downstream Port Containment|DpcCap|DpcCtl|DPC' 2>/dev/null | paste -sd ' | ' -)
        cpa_payload=$(printf '%s\n' "$cpa_detail" | grep -E 'DevCap:.*MaxPayload|DevCtl:.*MaxPayload.*MaxReadReq' 2>/dev/null | paste -sd ' | ' -)
        cpa_rebar=$(printf '%s\n' "$cpa_detail" | grep -E 'Resizable BAR|BAR [0-9]: current size|supported:' 2>/dev/null | paste -sd ' | ' -)
        cpa_ats=$(printf '%s\n' "$cpa_detail" | grep -E 'Address Translation Service|ATSCap|ATSCtl' 2>/dev/null | paste -sd ' | ' -)
        cpa_pri=$(printf '%s\n' "$cpa_detail" | grep -E 'Page Request Interface|PRICtl|PRICap' 2>/dev/null | paste -sd ' | ' -)
        cpa_pasid=$(printf '%s\n' "$cpa_detail" | grep -E 'Process Address Space ID|PASIDCap|PASIDCtl' 2>/dev/null | paste -sd ' | ' -)
        cpa_tph=$(printf '%s\n' "$cpa_detail" | grep -E 'TPH Requester|TPHComp|TPH' 2>/dev/null | paste -sd ' | ' -)
        jq -n --arg bdf "$cpa_bdf" --arg local_cpus "$cpa_local_cpus" --arg local_cpulist "$cpa_local_cpulist" --arg node "$cpa_node" --arg d3 "$cpa_d3" --arg power_control "$cpa_power_control" --arg runtime "$cpa_runtime" --arg wakeup "$cpa_wakeup" --argjson reset "$cpa_reset" --arg reset_method "$cpa_reset_method" --arg totalvfs "$cpa_totalvfs" --arg numvfs "$cpa_numvfs" --arg cur_speed "$cpa_cur_speed" --arg cur_width "$cpa_cur_width" --arg max_speed "$cpa_max_speed" --arg max_width "$cpa_max_width" --arg aspm "$cpa_aspm" --arg ltr "$cpa_ltr" --arg acs "$cpa_acs" --arg ari "$cpa_ari" --arg dpc "$cpa_dpc" --arg payload "$cpa_payload" --arg pcie_type "$cpa_pcie_type" --arg sysfs_path "$cpa_sysfs" --argjson ancestry "$cpa_ancestry" --arg rebar "$cpa_rebar" --arg ats "$cpa_ats" --arg pri "$cpa_pri" --arg pasid "$cpa_pasid" --arg tph "$cpa_tph" --arg detail "$cpa_detail" --slurpfile aer "$cpa_aer" '{bdf:$bdf,local_cpus:(if $local_cpus=="" then null else $local_cpus end),local_cpulist:(if $local_cpulist=="" then null else $local_cpulist end),numa_node:(if $node=="" then null else ($node|tonumber? // $node) end),d3cold_allowed:(if $d3=="" then null else $d3 end),power:{control:(if $power_control=="" then null else $power_control end),runtime_status:(if $runtime=="" then null else $runtime end),wakeup:(if $wakeup=="" then null else $wakeup end)},reset:{present:$reset,methods:(if $reset_method=="" then null else $reset_method end)},sriov:{totalvfs:(if $totalvfs=="" then null else ($totalvfs|tonumber? // $totalvfs) end),numvfs:(if $numvfs=="" then null else ($numvfs|tonumber? // $numvfs) end)},link:{current_speed:(if $cur_speed=="" then null else $cur_speed end),current_width:(if $cur_width=="" then null else $cur_width end),max_speed:(if $max_speed=="" then null else $max_speed end),max_width:(if $max_width=="" then null else $max_width end)},aer:$aer[0],sysfs_device_path:(if $sysfs_path=="" then null else $sysfs_path end),upstream_path_bdfs:$ancestry,pcie_port_type:(if $pcie_type=="" then null else $pcie_type end),capability_evidence:{aspm:(if $aspm=="" then null else $aspm end),ltr:(if $ltr=="" then null else $ltr end),acs:(if $acs=="" then null else $acs end),ari:(if $ari=="" then null else $ari end),dpc:(if $dpc=="" then null else $dpc end),payload_read_request:(if $payload=="" then null else $payload end),resizable_bar:(if $rebar=="" then null else $rebar end),ats:(if $ats=="" then null else $ats end),pri:(if $pri=="" then null else $pri end),pasid:(if $pasid=="" then null else $pasid end),tph:(if $tph=="" then null else $tph end)},lspci_vv_raw:$detail}' >> "$cpa_stream" || return 1
    done
    jq -s 'sort_by(.bdf)' "$cpa_stream" > "$cpa_dir/devices.json" || return 1
    jq -n --slurpfile devices "$cpa_dir/devices.json" '{devices:$devices[0]}' > "$cpa_dir/data.json" || return 1
    inv_strings_json "$cpa_dir/notes.json" "PCI locality masks are kernel-provided affinity hints/locality evidence; they do not prove physical CPU-lane attachment." "AER counters are cumulative snapshots when exposed." || return 1
    inv_strings_json "$cpa_dir/errors.json" || return 1
    inv_emit pcie-advanced "$cpa_dir/data.json" "$cpa_dir/notes.json" "$cpa_dir/errors.json"
}

print_pcie_advanced_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "Advanced PCIe View"
    inv_view_kv "PCI devices" "$(jq -r '.data.devices|length' "$f")"
    inv_view_section "Devices with advanced evidence"
    jq -r '.data.devices[]|[.bdf,(.local_cpulist//"n/a"),(.numa_node|tostring),(.power.runtime_status//"n/a"),(.d3cold_allowed//"n/a"),(.reset.methods//"n/a"),(.sriov.totalvfs//"n/a"|tostring),(.capability_evidence.aspm//"n/a"),(.capability_evidence.resizable_bar//"n/a")]|@tsv' "$f" | while IFS="$(printf '\t')" read -r bdf cpus node pwr d3 reset vfs aspm rebar; do inv_view_item "$bdf localCPUs=$cpus node=$node power=$pwr d3cold=$d3 reset=$reset sriov-total=$vfs"; [ "$aspm" != n/a ] && inv_view_item "  ASPM: $aspm"; [ "$rebar" != n/a ] && inv_view_item "  ReBAR: $rebar"; done
    inv_view_section "AER counters exposed"
    jq -r '[.data.devices[]|select((.aer|length)>0)]|length' "$f" | while IFS= read -r n; do inv_view_kv "Devices with AER sysfs" "$n"; done
    inv_view_metadata "$f"
}

# ============================================================
# print_pcie_advanced_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_pcie_advanced_summary() { print_pcie_advanced_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_pcie_advanced_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_pcie_advanced.summary-source.json
        collect_pcie_advanced > "$main_summary_file" || return $?
        print_pcie_advanced_view "$main_summary_file"
        return $?
    fi
    collect_pcie_advanced
}
