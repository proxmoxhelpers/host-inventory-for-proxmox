...
#
# Usage:
#   sudo ./view_gpu_vfio.sh
#   ./view_gpu_vfio.sh --file collect_gpu_vfio.json
#   ./view_gpu_vfio.sh --help
#
# Output:
...
usage() {
    cat <<'EOF'
view_gpu_vfio.sh - standalone gpu vfio inventory view

Usage:
  sudo ./view_gpu_vfio.sh
  ./view_gpu_vfio.sh --file collect_gpu_vfio.json
  sudo ./view_gpu_vfio.sh --json > collect_gpu_vfio.json

Default behavior:
  With no arguments, collect the gpu vfio aspect live and
  render its colorized human-readable view.

View input:
  --file FILE       Render a previously saved collect_gpu_vfio.json envelope.
