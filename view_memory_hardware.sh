...
# Collects DIMM/SMBIOS memory hardware and EDAC corrected/uncorrected error evidence.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_memory_hardware.sh
#   ./view_memory_hardware.sh --file collect_memory_hardware.json
#   ./view_memory_hardware.sh --help
#
# Output:
...
app_name="view_memory_hardware"
app_version=0.9.2
app_rc=0
app_compact=0
app_package_specs="jq:jq:required dmidecode:dmidecode:optional decode-dimms:i2c-tools:optional"
...
usage() {
    cat <<'EOF'
view_memory_hardware.sh - standalone memory hardware / EDAC view

Usage:
  sudo ./view_memory_hardware.sh
  ./view_memory_hardware.sh --file collect_memory_hardware.json
  sudo ./view_memory_hardware.sh --json > collect_memory_hardware.json
...
collect_memory_hardware() {
    cmh_dir=$_inv_temp_dir/memory-hardware
    mkdir -p "$cmh_dir" || return 1
    inv_command_json "$cmh_dir/dmidecode.json" 15 dmidecode -t 16 -t 17 -t 19 -t 20 || return 1
    inv_command_json "$cmh_dir/decode-dimms.json" 20 decode-dimms || return 1
    cmh_stream=$cmh_dir/edac.jsonl; : > "$cmh_stream"
    if [ -d /sys/devices/system/edac/mc ]; then
        for cmh_mc in /sys/devices/system/edac/mc/mc*; do
            [ -d "$cmh_mc" ] || continue
            cmh_name=${cmh_mc##*/}; cmh_mcname=$(cat "$cmh_mc/mc_name" 2>/dev/null); cmh_ce=$(cat "$cmh_mc/ce_count" 2>/dev/null); cmh_ue=$(cat "$cmh_mc/ue_count" 2>/dev/null)
            cmh_dstream=$cmh_dir/$cmh_name-dimms.jsonl; : > "$cmh_dstream"
            for cmh_dimm in "$cmh_mc"/dimm*; do
                [ -d "$cmh_dimm" ] || continue
                cmh_dname=${cmh_dimm##*/}; cmh_label=$(cat "$cmh_dimm/dimm_label" 2>/dev/null); cmh_size=$(cat "$cmh_dimm/size" 2>/dev/null); cmh_dce=$(cat "$cmh_dimm/dimm_ce_count" 2>/dev/null); cmh_due=$(cat "$cmh_dimm/dimm_ue_count" 2>/dev/null); cmh_mode=$(cat "$cmh_dimm/edac_mode" 2>/dev/null)
                jq -n --arg name "$cmh_dname" --arg label "$cmh_label" --arg size "$cmh_size" --arg ce "$cmh_dce" --arg ue "$cmh_due" --arg mode "$cmh_mode" '{name:$name,label:(if $label=="" then null else $label end),size:(if $size=="" then null else $size end),ce_count:(if $ce=="" then null else ($ce|tonumber? // $ce) end),ue_count:(if $ue=="" then null else ($ue|tonumber? // $ue) end),edac_mode:(if $mode=="" then null else $mode end)}' >> "$cmh_dstream" || return 1
            done
            jq -s '.' "$cmh_dstream" > "$cmh_dir/$cmh_name-dimms.json" || return 1
            jq -n --arg name "$cmh_name" --arg mc_name "$cmh_mcname" --arg ce "$cmh_ce" --arg ue "$cmh_ue" --slurpfile dimms "$cmh_dir/$cmh_name-dimms.json" '{name:$name,mc_name:(if $mc_name=="" then null else $mc_name end),ce_count:(if $ce=="" then null else ($ce|tonumber? // $ce) end),ue_count:(if $ue=="" then null else ($ue|tonumber? // $ue) end),dimms:$dimms[0]}' >> "$cmh_stream" || return 1
        done
    fi
    jq -s '.' "$cmh_stream" > "$cmh_dir/edac.json" || return 1
    inv_file_map_json "$cmh_dir/edac-controls.json" "log_ce=/sys/devices/system/edac/mc/log_ce" "log_ue=/sys/devices/system/edac/mc/log_ue" "panic_on_ue=/sys/devices/system/edac/mc/panic_on_ue" || return 1
    jq -n --slurpfile dmi "$cmh_dir/dmidecode.json" --slurpfile spd "$cmh_dir/decode-dimms.json" --slurpfile edac "$cmh_dir/edac.json" --slurpfile controls "$cmh_dir/edac-controls.json" '{dmidecode_memory:$dmi[0],decode_dimms:$spd[0],edac_controllers:$edac[0],edac_controls:$controls[0]}' > "$cmh_dir/data.json" || return 1
    inv_strings_json "$cmh_dir/notes.json" "EDAC counters are cumulative when exposed; absence of EDAC sysfs does not prove ECC is unsupported." || return 1
    inv_strings_json "$cmh_dir/errors.json" || return 1
    inv_emit memory-hardware "$cmh_dir/data.json" "$cmh_dir/notes.json" "$cmh_dir/errors.json"
}

print_memory_hardware_view() {
    f=$1; inv_summary_validate_file "$f" || return $?
    inv_view_header "$f" "Memory Hardware / EDAC View"
    inv_view_kv "dmidecode rc" "$(jq -r '.data.dmidecode_memory.rc // "unavailable"' "$f")"
    inv_view_kv "decode-dimms rc" "$(jq -r '.data.decode_dimms.rc // "unavailable"' "$f")"
    inv_view_kv "EDAC controllers" "$(jq -r '.data.edac_controllers|length' "$f")"
    inv_view_section "EDAC"
    jq -r '.data.edac_controllers[]|[.name,(.mc_name//"?"),(.ce_count//"n/a"|tostring),(.ue_count//"n/a"|tostring),(.dimms|length|tostring)]|@tsv' "$f" | while IFS="$(printf '\t')" read -r n m ce ue d; do inv_view_item "$n $m CE=$ce UE=$ue DIMMs=$d"; done
    inv_view_metadata "$f"
}

# ============================================================
# print_memory_hardware_summary
# Factual human-readable summary.
# Version: 1.0.0
# ============================================================
print_memory_hardware_summary() { print_memory_hardware_view "$1"; }
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_memory_hardware_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_memory_hardware.summary-source.json
        collect_memory_hardware > "$main_summary_file" || return $?
        print_memory_hardware_view "$main_summary_file"
        return $?
    fi
    collect_memory_hardware
}
