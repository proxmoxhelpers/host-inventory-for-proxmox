...
# ============================================================
# view_memory.sh
# Collects memory, NUMA, HugeTLB, THP, KSM, swap and VM memory sysctls.
#
# Version:
#   0.9.2
#
# Usage:
#   sudo ./view_memory.sh
#   ./view_memory.sh --file collect_memory.json
#   ./view_memory.sh --help
#
# Output:
...
app_name="view_memory"
app_version=0.9.2
app_rc=0
app_compact=0
app_package_specs="jq:jq:required swapon:util-linux:optional"
...
usage() {
    cat <<'EOF'
view_memory.sh - standalone memory inventory view

Usage:
  sudo ./view_memory.sh
  ./view_memory.sh --file collect_memory.json
  sudo ./view_memory.sh --json > collect_memory.json

Default behavior:
  With no arguments, collect the memory aspect live and
  render its colorized human-readable view.

View input:
  --file FILE       Render a previously saved collect_memory.json envelope.
...
# ============================================================
# collect_memory
# Collects host memory-management state relevant to deterministic VM latency.
#
# Version:
#   1.0.0
#
...
# ============================================================
collect_memory() {
    cm_dir=$_inv_temp_dir/memory
    cm_huge_stream=$cm_dir/huge.jsonl
    cm_node_stream=$cm_dir/nodes.jsonl
    mkdir -p "$cm_dir" || return 1
    : > "$cm_huge_stream"
    : > "$cm_node_stream"
    for cm_huge in /sys/kernel/mm/hugepages/hugepages-*; do
        [ -d "$cm_huge" ] || continue
        inv_flat_dir_json "$cm_dir/huge-values.json" "$cm_huge" || return 1
        jq -n --arg size "${cm_huge##*/}" --slurpfile values "$cm_dir/huge-values.json" '{size:$size,values:$values[0]}' >> "$cm_huge_stream" || return 1
    done
    jq -s '.' "$cm_huge_stream" > "$cm_dir/huge.json" || return 1
    for cm_node in /sys/devices/system/node/node[0-9]*; do
        [ -d "$cm_node" ] || continue
        jq -n --arg node "${cm_node##*/}" --arg cpulist "$(cat "$cm_node/cpulist" 2>/dev/null)" --arg meminfo "$(cat "$cm_node/meminfo" 2>/dev/null)" '{node:$node,cpulist:$cpulist,meminfo_raw:$meminfo}' >> "$cm_node_stream" || return 1
    done
    jq -s '.' "$cm_node_stream" > "$cm_dir/nodes.json" || return 1
    inv_flat_dir_json "$cm_dir/thp.json" /sys/kernel/mm/transparent_hugepage || return 1
    inv_flat_dir_json "$cm_dir/ksm.json" /sys/kernel/mm/ksm || return 1
    inv_flat_dir_json "$cm_dir/zswap.json" /sys/module/zswap/parameters || return 1
    inv_command_json "$cm_dir/swapon.json" 6 swapon --show --bytes --output NAME,TYPE,SIZE,USED,PRIO || return 1
    jq -n --arg meminfo "$(cat /proc/meminfo 2>/dev/null)" --arg proc_swaps "$(cat /proc/swaps 2>/dev/null)" --arg swappiness "$(cat /proc/sys/vm/swappiness 2>/dev/null)" --arg page_cluster "$(cat /proc/sys/vm/page-cluster 2>/dev/null)" --arg overcommit_memory "$(cat /proc/sys/vm/overcommit_memory 2>/dev/null)" --arg overcommit_ratio "$(cat /proc/sys/vm/overcommit_ratio 2>/dev/null)" --arg compaction_proactiveness "$(cat /proc/sys/vm/compaction_proactiveness 2>/dev/null)" --arg zone_reclaim_mode "$(cat /proc/sys/vm/zone_reclaim_mode 2>/dev/null)" --arg dirty_background_ratio "$(cat /proc/sys/vm/dirty_background_ratio 2>/dev/null)" --arg dirty_ratio "$(cat /proc/sys/vm/dirty_ratio 2>/dev/null)" --slurpfile huge "$cm_dir/huge.json" --slurpfile nodes "$cm_dir/nodes.json" --slurpfile thp "$cm_dir/thp.json" --slurpfile ksm "$cm_dir/ksm.json" --slurpfile zswap "$cm_dir/zswap.json" --slurpfile swapon "$cm_dir/swapon.json" '{meminfo_raw:$meminfo,numa_nodes:$nodes[0],hugetlb:$huge[0],transparent_hugepages:$thp[0],ksm:$ksm[0],swap:{proc_swaps_raw:$proc_swaps,swapon:$swapon[0],swappiness:$swappiness,page_cluster:$page_cluster},vm_sysctls:{overcommit_memory:$overcommit_memory,overcommit_ratio:$overcommit_ratio,compaction_proactiveness:$compaction_proactiveness,zone_reclaim_mode:$zone_reclaim_mode,dirty_background_ratio:$dirty_background_ratio,dirty_ratio:$dirty_ratio},zswap:$zswap[0]}' > "$cm_dir/data.json" || return 1
    inv_strings_json "$cm_dir/notes.json" || return 1
    inv_strings_json "$cm_dir/errors.json" || return 1
    inv_emit memory "$cm_dir/data.json" "$cm_dir/notes.json" "$cm_dir/errors.json"
}
...
# ============================================================
# print_memory_view
# Presents a colorized memory view.
#
# Version:
#   1.1.0
# ============================================================
print_memory_view() {
    pmv_file=$1
    inv_summary_validate_file "$pmv_file" || return $?
    inv_view_header "$pmv_file" "Memory View"

    inv_view_section "Memory overview"
    inv_view_kv "MemTotal" "$(jq -r 'try (.data.meminfo_raw|capture("(?m)^MemTotal:\\s+(?<v>[^\\n]+)").v) catch "unknown"' "$pmv_file")"
    inv_view_kv "MemAvailable" "$(jq -r 'try (.data.meminfo_raw|capture("(?m)^MemAvailable:\\s+(?<v>[^\\n]+)").v) catch "unknown"' "$pmv_file")"
    inv_view_kv "NUMA nodes" "$(jq -r '.data.numa_nodes|length' "$pmv_file")"
    inv_view_kv "THP enabled" "$(jq -r '.data.transparent_hugepages.enabled // "unknown"' "$pmv_file")"
    inv_view_kv "THP defrag" "$(jq -r '.data.transparent_hugepages.defrag // "unknown"' "$pmv_file")"
    inv_view_kv "KSM run" "$(jq -r '.data.ksm.run // "unknown"' "$pmv_file")"
    inv_view_kv "KSM pages shared" "$(jq -r '.data.ksm.pages_shared // "unknown"' "$pmv_file")"
    inv_view_kv "KSM pages sharing" "$(jq -r '.data.ksm.pages_sharing // "unknown"' "$pmv_file")"
    inv_view_kv "Swappiness" "$(jq -r '.data.swap.swappiness // "unknown"' "$pmv_file")"
    inv_view_kv "zswap enabled" "$(jq -r '.data.zswap.enabled // "unknown"' "$pmv_file")"

    inv_view_section "Swap"
    pmv_swap=$(jq -r '.data.swap.proc_swaps_raw // "" | split("\n")[1:] | map(select(length>0)) | join(" | ")' "$pmv_file")
    if [ -n "$pmv_swap" ]; then
        inv_view_item "$pmv_swap"
    else
        inv_view_item "No active swap entries reported."
    fi

    inv_view_section "HugeTLB pools"
    jq -r '.data.hugetlb[] | [.size,(.values.nr_hugepages//"?"),(.values.free_hugepages//"?"),(.values.resv_hugepages//"?"),(.values.surplus_hugepages//"?")] | @tsv' "$pmv_file" |
    while IFS="$(printf '\t')" read -r pmv_size pmv_nr pmv_free pmv_resv pmv_sur; do
        inv_view_item "$pmv_size total=$pmv_nr free=$pmv_free reserved=$pmv_resv surplus=$pmv_sur"
    done

    inv_view_metadata "$pmv_file"
}

# ============================================================
# print_memory_summary
# Presents RAM, HugeTLB, THP, KSM and swap state.
#
# Version:
#   1.1.0
#
# Usage:
#   print_memory_summary collectorEnvelope.json
#
# Returns:
#   0 on success
...
main() {
    case "$_inv_action" in
        ...
        summary_file|view_file)
            ...
            print_memory_view "$_inv_summary_file"
            return $?
            ;;
    esac
    ...
    if [ "$_inv_action" = summary ] || [ "$_inv_action" = view ]; then
        main_summary_file=$_inv_temp_dir/collect_memory.summary-source.json
        collect_memory > "$main_summary_file" || return $?
        print_memory_view "$main_summary_file"
        return $?
    fi
    collect_memory
}
